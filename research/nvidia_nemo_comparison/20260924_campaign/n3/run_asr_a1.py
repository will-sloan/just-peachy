...
def source_binding(root):
    ...
    for path in (Path(__file__),Path(__file__).with_name('reference_asr.py'),
                 ...
                 Path(__file__).with_name('a1_onnx_v2.py')):
...
def main():
    ...
    p.add_argument('--runtime',choices=['sherpa','native','reference','onnx'],required=True)
    ...
    lock=dict(schema='just-peachy.n3.asr-run.v1',variant=args.variant,runtime=args.runtime,
        ...
        a1_frontend_policy='eval_no_dither' if args.variant=='A1' else None,
        onnx_runtime_packages={name:version(name) for name in ('numpy','scipy','onnxruntime')} if args.runtime=='onnx' else None)
    ...
    try:
        ...
        if args.runtime=='sherpa':
            ...
            if args.variant!='A1' or args.binding is None or args.right_context!=1:raise ValueError('A1 ONNX requires its exact nominal bundle manifest')
            from a1_onnx_v2 import OnnxRecognizer
            owner=OnnxRecognizer(args.binding.parent)
            result['onnx_dependency_check']=dict(torch_imported='torch' in sys.modules,nemo_imported='nemo' in sys.modules,
                encoder_providers=owner.service.encoder.get_providers(),decoder_providers=owner.service.decoder.get_providers())
            if result['onnx_dependency_check']['torch_imported'] or result['onnx_dependency_check']['nemo_imported']:
                raise RuntimeError('Portable application runtime imported a reference framework')
        else:
            ...
            if args.variant=='A1':owner.service._audio_buffer.preprocessor.eval()
