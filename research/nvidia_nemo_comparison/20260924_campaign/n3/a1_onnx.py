"""CPU A1 service using NumPy/SciPy and ONNX Runtime; README_A1_PORTABLE.md."""
        ...
        for t in range(int(length[0])):
            for _ in range(10):
                logits,_,h,c=self.decoder.run(None,dict(encoder_outputs=encoded[:,:,t:t+1],
                    targets=np.array([[self.last_token]],np.int64),target_length=np.ones(1,np.int64),
...
class OnnxRecognizer:
    ...
    def stream(self):
        from reference_asr import ReferenceStream
