"""Actual source-paced Controller/Tk panel on a private desktop. See README_A1_CONTROLLER.md."""
...
def audio_jobs(regression,screen,tap='O0'):
    ...
    for variant in ('A1',):
        for example,scene,mode in SCENES[:3]:
            ...
            jobs.append(dict(cell_id=variant+'_'+example,variant=variant,combination='D0_E0',
...
def aggregate_cell_reports(admission,runs,*,require_all=True):
    ...
    return dict(schema='n2-actual-source-paced-gui-panel-v2',scope=SCOPE,
        ...
        requested_cells=len(expected),full_panel_cells=3,completed=len(cells),missing_cells=missing,
...
class ActualPanelTests(unittest.TestCase):
    def test_actual_saved_audio_panel(self):
        ...
        report=dict(schema='n3-actual-source-paced-gui-panel-v1',scope=SCOPE,desktop=desktop,status='RUNNING',cells=[],
                    ...
                    full_panel=admission['full_panel'],requested_cells=len(admission['jobs']),full_panel_cells=3)
        for job in admission['jobs']:
...
def main():
    ...
    parser.add_argument('--cpu',type=int,choices=[4,14],default=4);parser.add_argument('--tap',choices=['O0','O1'],default='O0')
    parser.add_argument('--cell',action='append',help='Explicit smoke subset; full panel is three cells')
    ...
    admission=dict(schema='n2-actual-gui-admission-v1',scope=SCOPE,source=str(source),common_source=str(common),output=str(output),
        ...
        full_panel=len(jobs)==3,job_count=len(jobs),cpu=args.cpu,threads=1,saved_audio_only=True,gain=1.,
