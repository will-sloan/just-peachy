        ...
        for job in admission['jobs']:
            ...
            try:
                ...
                if job['mode']=='selected_closed':
                    ...
                    self.assertTrue(all(r['label'].endswith(' Â· assumed') and r['display_profile_id'] in {p['id'] for p in c.store.list()} for r in choices))
                    self.assertTrue(any(' Â· assumed' in r['label'] for r in audit.receipts),'No actual assumed-name Tk receipt')
                else:
