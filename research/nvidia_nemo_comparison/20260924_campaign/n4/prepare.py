"""Verify all accepted files; freeze the full N4 bank and 16-row matrix. README.md."""
...
def prepare(args):
    ...
    catalog = load(catalog_path)['backends']
    mapping = {('A0','D0','E0'):'baseline', ('A0','D1','E0'):'nemotron_hybrid',
               ('A0','D0','E1'):'titanet', ('A0','D1','E1'):'nemotron_titanet',
               ('A2','D1','E0'):'nemotron_600m', ('A3','D1','E0'):'nemotron_35_600m'}
    matrix = []
    for a,d,e in product(('A0','A1','A2','A3'), ('D0','D1'), ('E0','E1')):
        backend = next((b for b in catalog if b['key'] == mapping.get((a,d,e))), None)
    ...
    receipt = dict(schema='n4-data-preparation-v1', status='ACTUALLY_RUN_MODEL_FREE',
        inputs=[bind(audit_path), bind(corpus_path), bind(truth_path), bind(catalog_path), bind(__file__), bind(HERE/'common.py')],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=Path, default=LOCAL/'releases/n3-common-v2/prototype')
