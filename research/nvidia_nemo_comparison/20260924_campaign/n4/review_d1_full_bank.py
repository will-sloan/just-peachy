"""Review all full-bank D1 cells. See README_D1_FULL_BANK.md."""
import argparse
import gzip
import json
...
def review(args):
    ...
    for binding in initial+bindings:verify(binding)
    ...
    receipt = dict(status='PASS_D1_FULL_BANK_COMPONENTS_ONLY', reviewed_utc=supervisor.now(),
        ...
        reviewer=bind(Path(__file__)), component_cells=960, paired_files=480, rows=rows,
