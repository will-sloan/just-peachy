# Complete planned V3 panel evidence coverage

Purpose: `review_application_panel_v3.py` reconstructs a qualified production
panel and reviews every declared cell through the V3 joined-cell reader. Expected
...
Inputs: a completed V3 application run containing its immutable ADMISSION,
RUN_OWNER, RESULT, worker command, cells and per-cell progress; the unchanged V3
...
REVIEW.json with `PASS_COMPLETE_V3_PANEL_EVIDENCE_COVERAGE_ONLY`, or FAILED.json
...
The development probe has ten model-free tests: complete 40/240 populations;
...
& $jpPython -B "$jpCode\probe_application_panel_review_v3.py" --output "$jpLocal\n4\application-panel-v3-review-probe-v1"
```
...
"%JP_PY%" -B "%JP_CODE%\probe_application_panel_review_v3.py" --output "%JP_LOCAL%\n4\application-panel-v3-review-probe-v1"
```
...
```powershell
& $jpPython -B "$jpCode\review_application_panel_v3.py" --run "$jpLocal\n4\paced-panel-run-v3" --output "$jpLocal\n4\paced-panel-evidence-review-v1"
```

```bat
"%JP_PY%" -B "%JP_CODE%\review_application_panel_v3.py" --run "%JP_LOCAL%\n4\paced-panel-run-v3" --output "%JP_LOCAL%\n4\paced-panel-evidence-review-v1"
```

V3 binds the explicit delivery-observed planner, runner, transport and composed
cell readers. Each compact cell also retains the independent delivery envelope,
full raw append/lateness diagnostic summary, source sample count, shared origin
and last append time. It checks the reconstructed row's exact job/contract/cell ID
and requires delivery counts/origins to match the composed joins. Changed rows,
counts, origins, summary records or promoted deadline claims fail compaction.
An older V2 run schema is rejected. Delivery overhead is not subtracted from GUI
latency, and complete population coverage does not grant timing or N4 acceptance.

The ten development checks exercise 40 and 240-cell population fixtures and the
saved V3 synthetic joined-cell result. They do not execute a positive production
plan or review an actual panel. The probe writes its exact owner and four source
snapshots before fragile dependency admission and preserves every failed attempt.
Private source transcripts, profile identities, traces and full observations stay
outside Git. The original readers and their qualifications remain unchanged.
