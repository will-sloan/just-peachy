"""Frozen-loop regression and clock-corruption checks. README_D1_FRAME_CLOCK_V3.md."""
...
from review_d1_frame_clock_v3 import scan_events
...
class LateSpeechNative(FrameStepNative):
    def _update(self, target, final):
        update = super()._update(target, final)
        values = self.np.zeros_like(update.probabilities)
        for offset in range(len(values)):
            if update.frame_start + offset >= 5000:
                values[offset, 3] = .9
        return replace(update, probabilities=values)


class LateQueryClockTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        fixtures.TestD1ActualLoop.setUpClass()
        f = fixtures.TestD1ActualLoop()
        capture = f.setup_capture(frames=65 * 16000 + 5)
        f.resident.native = LateSpeechNative(f.Update, f.np)
        cls.summary = capture.run_capture(f.encoder)
        cls.rows = [json.loads(line) for line in f.log.getvalue().splitlines()]
        cls.wave, cls.namespace = f.wave, f.encoder.namespace
        from app.n2_pipeline import ActivityTimeline
    ...
    def test_native_event_clock_separate_from_rounded_waveform_support(self):
        from review_d1_frame_clock_v2 import scan_events as previous
        with self.assertRaisesRegex(ValueError, 'Numeric clock/source'):
            previous(self.rows, self.wave, self.summary, self.namespace, self.timeline)
        self.assertGreater(self.scan()['embeddings'], 25)

    def test_native_query_event_clock_corruption_rejected(self):
        for kind in ['component_d1_embedding_call', 'research_embedding']:
            with self.subTest(kind=kind):
                rows = deepcopy(self.rows)
                next(r for r in rows if r['event_type'] == kind)['source_time_sec'] += 2e-6
                with self.assertRaisesRegex(ValueError, 'Numeric clock/source'):
                    self.scan(rows)

    def test_rounded_embedding_support_remains_exact(self):
        rows = deepcopy(self.rows)
        p = next(r for r in rows if r['event_type'] == 'research_embedding')['payload']
        p['clean_intervals'][0][1] -= 1 / 16000
