...
from private_application_process import PrivateApplicationProcess
...
class PrivateProcessTests(unittest.TestCase):
    def setUp(self):
        if OUTPUT is None: self.skipTest('Use probe_private_application_process.py with a fresh private output')
    ...
    def launch(self, mode):
        self.child = PrivateApplicationProcess(self.root/'lifetime', executable_binding=bind(sys.executable),
            script_binding=bind(Path(__file__).with_name('private_process_fixture.py')),
    ...
    def test_stubborn_root_and_descendant_cleanup(self):
        ...
        self.assertEqual(leaf['affinity'], [14]); self.assertEqual(child.accounting()['active'], 2)
        child.close(grace_seconds=.1); self.assert_closed(child, forced=True, exit_code=125)
        self.assertIsNone(exact_process(leaf)); self.assertEqual(child.receipt['final_job']['total'], 2)
    ...
    def test_root_exit_does_not_hide_live_descendant(self):
        ...
        self.assertTrue(child.root_exited()); self.assertEqual(child.accounting()['active'], 1)
    ...
    def test_last_job_handle_closes_on_owner_death(self):
        ...
        self.assertEqual(child.receipt['final_job']['total'], 2)

    def test_invalid_input_refused_before_spawn(self):
        kwargs = dict(executable_binding=bind(sys.executable), script_binding=bind(Path(__file__).with_name('private_process_fixture.py')))
