"""Join stopped V3 native/transport/GUI evidence. README_APPLICATION_CELL_REVIEW_V3.md."""
from pathlib import Path

from common import fingerprint, verify
from review_application_transport_v3 import record, review_cell as review_transport
from review_application_observations_v2 import review_observations
...
def join_reviews(folder, payload, transport, observations):
    ...
    require(transport['status'] == 'PASS_V3_APPLICATION_TRANSPORT_NATIVE_AND_DELIVERY_JOINS_ONLY'
        and observations['status'] == 'PASS_V2_DELIVERY_APPLICATION_OBSERVATION_JOINS_ONLY', 'Independent V3 checks required')
    ...
    _, delivery_envelope=checked('SOURCE_DELIVERY_ENVELOPE.json')
    delivery=transport['source_delivery_review']
    require(fingerprint(delivery_envelope['review'])==fingerprint(delivery)
        and delivery['status']=='PASS_APPLICATION_DELIVERY_SOURCE_CLOCK_JOIN_ONLY', 'Delivery envelope review changed')
    ...
    require(source['source_epoch_monotonic_sec'] == clock['source_epoch_monotonic_sec']
        == observations['phase_intervals']['source_origin_monotonic_sec']
        == delivery['source_origin_perf_counter'], 'Native/consumer/viewport/delivery origins differ')
    require(delivery['source_samples']==payload['job']['frames']==closure['source']['sent']
        ==delivery['summary']['successful_samples'] and delivery['summary']['complete_source'] is True,
        'Delivery/engine planned source census differs')
    last_append=delivery['source_origin_perf_counter']+delivery['summary']['last_append_return_after_origin_seconds']
    require(last_append<=closure['observed_monotonic_sec'],'Delivery append completed after recorded engine closure')
    ...
    return dict(native_terminal_and_source_start_joined=True, common_evidence_bindings=len(records),
        ...
        delivery_records=delivery['summary']['records'],delivery_samples=delivery['source_samples'],
        last_delivery_append_monotonic_sec=last_append,delivery_deadlines_accepted=False,
...
def review_cell(folder, *, checkpoint=None, **expected):
    """Caller must first reconstruct the qualified V3 production plan/population."""
    ...
    if checkpoint: checkpoint()
    return dict(status='PASS_V3_APPLICATION_CELL_EVIDENCE_JOINS_ONLY', transport=transport,
