"""Guarded whole-panel naming development checks; README_NAME_PANEL.md."""
import argparse
...
from review_name_panel import OWN,code_bindings
from review_scoring_bank import guard,require,shared_allowance
from scoring_bank import writer_lock
import test_name_panel as regression
...
def run(output):
    ...
    with writer_lock(LOCAL/'n4/metric-scoring.owner.lock'):
        ...
        try:
            ...
            q=load(HERE/'OBSERVED_NAME_SCORING_CHECK_V1.json');refq=load(HERE/'NAMING_REFERENCE_CHECK_V1.json')
            old=load(HERE/'APPLICATION_CELL_REVIEW_CHECK_V2.json')
            for b in (q['synthetic_observed_review'],q['synthetic_score'],old['synthetic_join_review']):verify(b)
            ...
            freeze(output/'ADMISSION.json',dict(owner=identity(process),code=code,source_snapshots=snapshots,inventory=inventory,
                D1_snapshot=active,synthetic_observed=q['synthetic_observed_review'],synthetic_score=q['synthetic_score'],
                synthetic_cell=old['synthetic_join_review'],payload=payloads[0],reference_inputs=refq['reference_inputs'],
                production_plan_created=False,new_application_or_model_started=False))
            checkpoint=lambda:guard(output,LOCAL,started,720);references=load_qualified_context(checkpoint=checkpoint)
            regression.CONTEXT.update(output=output,synthetic_observed=q['synthetic_observed_review'],synthetic_score=q['synthetic_score'],
            ...
            stream=io.StringIO();tests=unittest.TextTestRunner(stream=stream,verbosity=2).run(
                unittest.defaultTestLoader.loadTestsFromTestCase(regression.NamePanelTests))
            (output/'tests.txt').write_text(stream.getvalue(),encoding='utf-8')
            require(tests.wasSuccessful() and tests.testsRun==18 and not tests.skipped,'Name-panel development checks failed')
            for b in code+references['evidence']+[q['synthetic_observed_review'],q['synthetic_score'],old['synthetic_join_review'],payloads[0]]:verify(b)
            after=active_d1();require(after['result']['child']==active['result']['child'] and after['run_id']==active['run_id'],'D1 owner changed')
            checkpoint()
            freeze(output/'RESULT.json',dict(status='PASS_NAME_PANEL_DEVELOPMENT_CHECKS_ONLY',utc=datetime.now(timezone.utc).isoformat(),
            ...
            print('PASS: 18 naming-panel population, compaction, weighted-count and scope checks; no actual panel launched',flush=True)
        except BaseException as exc:
            freeze(output/'FAILED.json',dict(status='FAILED_NAME_PANEL_PROBE_PRESERVED',error_type=type(exc).__name__,owner=identity(process),
