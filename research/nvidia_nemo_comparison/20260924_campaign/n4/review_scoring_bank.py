"""Review sealed modeled scores without rerunning metrics. README_SCORING_REVIEW.md."""
import argparse
...
from scoring_bank import admit, code_bindings as scoring_code, prediction, verify_bindings, writer_lock
...
def code_bindings():
    return [bind(HERE / n) for n in ('review_scoring_bank.py', 'test_scoring_review.py', 'probe_scoring_review.py',
            'README_SCORING_REVIEW.md', 'asr_full_bank.py')] + scoring_code()
...
def run(args):
    ...
    with writer_lock(local/'n4/metric-scoring.owner.lock'):
        ...
        qualification = load(HERE/'SCORING_REVIEW_IMPLEMENTATION_V1.json')
        code = code_bindings()
        require(qualification['status'] == 'PASS_SCORING_REVIEW_DEVELOPMENT_ONLY' and qualification['code'] == code, 'Reviewer implementation is not qualified')
        ...
        freeze(args.output/'ADMISSION.json', dict(owner=identity(process), result=result_binding, scoring_admission=result['admission'],
            code=code, qualification=bind(HERE/'SCORING_REVIEW_IMPLEMENTATION_V1.json'), max_seconds=args.max_seconds,
        ...
        try:
            ...
            require(admission['code'] == scoring_code() and all(b in known['code'] for b in admission['code']), 'Scorer implementation differs')
