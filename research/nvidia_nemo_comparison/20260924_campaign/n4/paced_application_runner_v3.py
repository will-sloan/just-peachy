"""Fixed supervised coordinator/child wiring. README_PACED_APPLICATION_RUNNER_V3.md."""
import argparse
import ctypes as C
...
from paced_panel_plan_v3 import admit_plan, execution_payload, APPLICATION_POLICY
...
RUN_SCHEMA = 'n4-paced-application-run-v3'
QUALIFICATIONS = {
...
def code_bindings():
    names = ('paced_application_runner_v3.py', 'test_paced_application_runner_v3.py', 'probe_paced_application_runner_v3.py',
        'README_PACED_APPLICATION_RUNNER_V3.md')
...
def prepare(plan_path, output):
    ...
    with writer_lock(LOCAL/'n4/metric-scoring.owner.lock'):
        ...
        code = code_bindings(); q = load(HERE/'PACED_RUNNER_CHECK_V3.json')
        require(q['status'] == 'PASS_PACED_RUNNER_V3_DEVELOPMENT_ONLY' and q['code'] == code, 'Fixed runner is not qualified')
        ...
        freeze(output/'ADMISSION.json', dict(schema=RUN_SCHEMA, status='PREPARED_PACED_APPLICATION_RUN_ONLY', owner=identity(process),
            plan=plan_binding, code=code, executable=executable, qualification=bind(HERE/'PACED_RUNNER_CHECK_V3.json'), inventory=inventory,
...
def run(admission_path):
    ...
    qualification = load(HERE/'PACED_RUNNER_CHECK_V3.json')
    require(admission['qualification'] == bind(HERE/'PACED_RUNNER_CHECK_V3.json') and
        qualification['status'] == 'PASS_PACED_RUNNER_V3_DEVELOPMENT_ONLY' and qualification['code'] == admission['code'] and
