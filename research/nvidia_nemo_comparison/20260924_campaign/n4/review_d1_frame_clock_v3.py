"""Manifest-bound D1 frame geometry. See README_D1_FRAME_CLOCK_V3.md."""
import argparse
    ...
    for row in rows:
        ...
        if kind == 'n2_diarization_binding':
            ...
            for slot, start, end, run_start in timeline.exclusive_windows(last_query):
                expected_queries.append((slot, round(start*16000), round(end*16000), run_start, p['track_ids'][slot], end))
                last_query[slot] = end
            ...
            slot, first, last, _, track, native_end = expected_queries[0]
            ...
            close(row['source_time_sec'], native_end)
            elapsed = p['actual_finished_elapsed_sec']-p['actual_started_elapsed_sec']
            ...
            slot, first, last, _, track, native_end = expected_queries.popleft()
            close(row['source_time_sec'], native_end)
            serial += 1
