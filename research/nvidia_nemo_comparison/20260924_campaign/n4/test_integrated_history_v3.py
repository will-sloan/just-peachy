"""Full-census join and actual boundary regressions. README_INTEGRATED_HISTORY_V3.md."""
...
from integrated_bank_plan_v3 import build_join,exact_jobs,review_sources,read_component_bank,MAIN_MODE
from integrated_bank_v3 import predict_cell,guard,run,GIB
...
class TestArtifactBounds(unittest.TestCase):
    def test_exact_roundtrip_hash_size_and_expansion_bounds(self):
        from method_artifact_v3 import save_private, read_replay
        with tempfile.TemporaryDirectory() as tmp,patch('method_artifact_v3.LIMIT',128):
...
class TestIntegratedBank(unittest.TestCase):
    ...
    def test_resource_guard_stops_before_allocating_or_crossing_cutoff(self):
        ...
        with tempfile.TemporaryDirectory() as tmp,patch('integrated_bank_v3.shutil.disk_usage',return_value=SimpleNamespace(free=200*GIB)):
        ...
        with patch('integrated_bank_v3.shutil.disk_usage',return_value=SimpleNamespace(free=40*GIB)):
    ...
    def test_failed_prediction_preserved_and_remaining_denominators_retained(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('integrated_bank_v3.verify_plan',return_value=plan),patch('integrated_bank_v3.guard'),\
                    patch('asr_full_bank.payload_inventory',return_value={'errors':[],'total_logical_bytes':0}),\
                    patch('integrated_bank_v3.predict_cell',side_effect=predictor):
    ...
    def test_pre_cell_resource_stop_does_not_invent_a_failed_prediction(self):
        with tempfile.TemporaryDirectory() as tmp:
            plan,args=self.lifecycle_fixture(tmp)
            with patch('integrated_bank_v3.verify_plan',return_value=plan),\
                    patch('integrated_bank_v3.guard',side_effect=[None,ValueError('fixture budget stop')]),\
                    patch('asr_full_bank.payload_inventory',return_value={'errors':[],'total_logical_bytes':0}),\
                    patch('integrated_bank_v3.predict_cell') as predictor:
