"""Review stopped application transport evidence. README_CONTINUITY_REVIEW.md."""
import json
import math
...
def review_cell(folder, *, payload, plan_sha256, coordinator, code, executable, coordinator_argv, state):
    """Internal join; caller must reconstruct the qualified continuity plan.
    ...
    scripts = [b for b in code if Path(b['path']).name == 'continuity_application_runner.py']
    ...
    return dict(status='PASS_CONTINUITY_TRANSPORT_NATIVE_AND_DELIVERY_JOINS_ONLY', cell_id=payload['cell_id'], collected=collected_binding,
        ...
        limitations=['Caller must independently reconstruct the qualified continuity plan and terminal candidate census.',
