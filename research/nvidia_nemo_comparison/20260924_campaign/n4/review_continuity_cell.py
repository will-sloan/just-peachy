"""Join stopped continuity native/transport/GUI evidence. README_CONTINUITY_REVIEW.md."""
from pathlib import Path

from common import fingerprint, verify
from review_continuity_transport import record, review_cell as review_transport
...
def join_reviews(folder, payload, transport, observations):
    ...
    require(transport['status'] == 'PASS_CONTINUITY_TRANSPORT_NATIVE_AND_DELIVERY_JOINS_ONLY'
        and observations['status'] == 'PASS_V2_DELIVERY_APPLICATION_OBSERVATION_JOINS_ONLY', 'Independent continuity checks required')
...
def review_cell(folder, *, checkpoint=None, **expected):
    """Caller must reconstruct the qualified continuity plan and candidate census."""
    ...
    if checkpoint: checkpoint()
    return dict(status='PASS_CONTINUITY_CELL_EVIDENCE_JOINS_ONLY', transport=transport,
