"""Review stopped application transport evidence. README_APPLICATION_TRANSPORT_REVIEW_V2.md."""
import json
import math
...
def review_cell(folder, *, payload, plan_sha256, coordinator, code, executable, coordinator_argv, state):
    ...
    scripts = [b for b in code if Path(b['path']).name == 'paced_application_runner_v2.py']
    ...
    return dict(status='PASS_APPLICATION_TRANSPORT_AND_NATIVE_ENVELOPE_JOINS_ONLY', cell_id=payload['cell_id'], collected=collected_binding,
