# D1 manifest-bound native frame-clock review
...
`review_d1_frame_clock_v2.py` is a derivative of the original event scanner.
...
`review_d1_full_bank_v2.py` retains the original full-bank admission, source,
...
`local/n4/d1-full-bank-review-execution-v1` and choose a fresh output directory.

The tests use the actual frozen D1 application loop with stub models, including
a 55-second float32 interval fixture reproducing the original failure. They
...
& $jpPython -B -c "from metric_process import pin; pin(); import unittest; unittest.main(module=None)" test_d1_frame_clock_v2 test_review_d1_full_bank_v2 -v
& $jpPython -B "$jpCode\review_d1_full_bank_v2.py" --run "$jpLocal\d1-full-bank-v1" --output "$jpLocal\d1-full-bank-review-v2"
```
...
"%JP_PY%" -B -c "from metric_process import pin; pin(); import unittest; unittest.main(module=None)" test_d1_frame_clock_v2 test_review_d1_full_bank_v2 -v
"%JP_PY%" -B "%JP_CODE%\review_d1_full_bank_v2.py" --run "%JP_LOCAL%\d1-full-bank-v1" --output "%JP_LOCAL%\d1-full-bank-review-v2"
```

Wait for the exact reviewer owner to exit before downstream admission. Supply
the passed v2 REVIEW.json as `--d1-review` to the unchanged integrated-bank
