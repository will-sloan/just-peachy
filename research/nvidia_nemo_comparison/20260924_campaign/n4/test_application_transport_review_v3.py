...
import review_application_transport_v3 as review
...
class TransportReviewTests(unittest.TestCase):
    ...
    def fixture(self):
        ...
        for name in ('fixture.exe','paced_application_runner_v3.py'): (base/name).write_text('INERT TEST DATA',encoding='utf-8')
        exe = bind(base/'fixture.exe'); script = bind(base/'paced_application_runner_v3.py'); payload = fixture_input()
        ...
        # the exact V3 filename may be selected as this child's executable script.
    ...
    def test_join_preserves_all_acceptance_and_history_limits(self):
        ...
        self.assertEqual(result['status'],'PASS_V3_APPLICATION_TRANSPORT_NATIVE_AND_DELIVERY_JOINS_ONLY')
