...
import test_paced_panel_plan_v2 as regression
...
def run(output):
    ...
    with writer_lock(LOCAL/'n4/metric-scoring.owner.lock'):
        ...
        freeze(output/'ADMISSION.json',dict(owner=identity(process),code=code,
            source_snapshots=[bind(p) for p in sorted((output/'source').iterdir())],inventory=inventory,D1_snapshot=active,
            application_qualification=app_binding,application_context=app['application_context'],preparation=prep_binding,
            actual_shortlist_selected=False,production_plan_created=False))
        checks=[]
        try:
            ...
            require(tests.wasSuccessful() and tests.testsRun==6 and not tests.skipped,'V2 panel planner tests failed')
            ...
            guard(output,LOCAL,started,720)
            freeze(output/'RESULT.json',dict(status='PASS_JOURNAL_DERIVATIVE_PANEL_PLANNING_CHECKS_ONLY',utc=datetime.now(timezone.utc).isoformat(),
            ...
            print('PASS: six V2 planner tests and 1,240 fixture payloads across all 16 routes; no production plan',flush=True)
        except BaseException as exc:
            freeze(output/'FAILED.json',dict(status='FAILED_V2_PANEL_PLANNER_PROBE_PRESERVED',error_type=type(exc).__name__,
                admission=bind(output/'ADMISSION.json'),checks=checks))
            raise
