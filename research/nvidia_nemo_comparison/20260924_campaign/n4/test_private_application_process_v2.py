...
from private_application_process_v2 import PrivateApplicationProcess
...
class PrivateProcessTests(unittest.TestCase):
    def setUp(self):
        if OUTPUT is None: self.skipTest('Use probe_private_application_process_v2.py with a fresh private output')
    ...
    def launch(self, mode):
        self.child = PrivateApplicationProcess(self.root/'lifetime', executable_binding=bind(sys.executable),
            script_binding=bind(Path(__file__).with_name('private_process_fixture_v2.py')),
    ...
    def test_invalid_input_refused_before_spawn(self):
        kwargs = dict(executable_binding=bind(sys.executable), script_binding=bind(Path(__file__).with_name('private_process_fixture_v2.py')))
