...
V2 preserves the first probe and its source snapshot. V1 cleaned up every owned
job but failed three assertions that assumed only Python processes were counted.
V2 records the Windows job's actual members and requires any additional live
fixture members to be the system `conhost.exe`, on CPU14. Every recorded fixture
member must be gone after cleanup. Root-exit and parent-death tests still require
the complete job to become empty; console helpers are not excluded from cleanup.

Purpose: `private_application_process_v2.py` starts a bound Python child suspended
...
The implementation queries active job processes until zero and verifies root
exit before releasing handles. Kill-on-close on an exceptional cleanup path is
...
`probe_private_application_process_v2.py` is the runnable entry point. It pins the
...
& $jpPython -B "$jpCode\probe_private_application_process_v2.py" --output 'G:\Just_Peachy_N1\20260924_campaign\local\n4\private-process-probe-v2'
```
...
"%JP_PY%" -B "%JP_CODE%\probe_private_application_process_v2.py" --output "G:\Just_Peachy_N1\20260924_campaign\local\n4\private-process-probe-v2"
```
