"""Review all stopped V3 panel cells. README_APPLICATION_PANEL_REVIEW_V3.md."""
import argparse
...
from paced_panel_plan_v3 import admit_plan, execution_payload
from paced_application_runner_v3 import RUN_SCHEMA, code_bindings as runner_code, qualified_interpreter
...
QUALIFICATION = 'APPLICATION_PANEL_REVIEW_CHECK_V3.json'


def code_bindings():
    entries = [bind(HERE/name) for name in ('review_application_panel_v3.py','test_application_panel_review_v3.py',
        'probe_application_panel_review_v3.py','README_APPLICATION_PANEL_REVIEW_V3.md')]
    for name, status in (
        ...
        ('PACED_PANEL_PLAN_CHECK_V3.json','PASS_DELIVERY_PANEL_PLANNER_DEVELOPMENT_ONLY')):
...
def validate_population(plan, terminal, collected, progress, cell_names, progress_names):
    ...
    return dict(planned_cells=required, collected_cells=required, additional_or_missing_cells=0,
        population_origin='Reconstructed qualified V3 plan; no available-evidence denominator substitution')
...
def stopped_run(folder):
    ...
    require(a['schema'] == RUN_SCHEMA and a['status'] == 'PREPARED_PACED_APPLICATION_RUN_ONLY'
        and a['actual_application_started'] is False, 'Prepared V3 run admission required')
    ...
    rq = bind(HERE/'PACED_RUNNER_CHECK_V3.json'); q = load(rq['path'])
    code = runner_code(); executable = qualified_interpreter()
    require(a['qualification'] == rq and q['status'] == 'PASS_PACED_RUNNER_V3_DEVELOPMENT_ONLY'
    ...
    argv = [executable['path'],'-B',str(HERE/'paced_application_runner_v3.py'),'run','--admission',str(folder/'ADMISSION.json')]
...
def compact_cell(checked, row, payload):
    require(checked['status'] == 'PASS_V3_APPLICATION_CELL_EVIDENCE_JOINS_ONLY'
    ...
    return dict(status='PASS_V3_CELL_EVIDENCE_COVERAGE_ONLY',cell_id=row['cell_id'],composition=row['composition'],
...
def run(run_root, output):
    ...
    with writer_lock(LOCAL/'n4/metric-scoring.owner.lock'):
        ...
        try:
            ...
            require(qualification['status'] == 'PASS_V3_PANEL_CENSUS_REVIEW_DEVELOPMENT_ONLY'
            ...
            checkpoint()
            freeze(output/'REVIEW.json',dict(status='PASS_COMPLETE_V3_PANEL_EVIDENCE_COVERAGE_ONLY',utc=datetime.now(timezone.utc).isoformat(),
            ...
            print('Complete planned V3 evidence population checked; semantic/timing/resource acceptance remains separate',flush=True)
        except BaseException as exc:
            freeze(output/'FAILED.json',dict(status='FAILED_V3_PANEL_REVIEW_PRESERVED',owner=identity(process),
