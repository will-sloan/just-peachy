...
import test_application_transport_review_v2 as regression
...
def code_bindings():
    names = ('review_application_transport_v2.py','test_application_transport_review_v2.py','probe_application_transport_review_v2.py',
        'README_APPLICATION_TRANSPORT_REVIEW_V2.md','common.py','metric_process.py','paced_child_admission.py','paced_slot.py',
    ...
    for name in ('PACED_RUNNER_CHECK_V2.json','PRIVATE_PROCESS_CHECK_V3.json','NATIVE_JOURNAL_REVIEW_CHECK_V1.json'):
...
def run(output):
    ...
    with writer_lock(LOCAL/'n4/metric-scoring.owner.lock'):
        ...
        try:
            ...
            require(tests.wasSuccessful() and tests.testsRun==15 and not tests.skipped,'Transport review checks failed')
            for b in code:verify(b)
            ...
            freeze(output/'RESULT.json',dict(status='PASS_APPLICATION_TRANSPORT_V2_REVIEW_CHECKS_ONLY',utc=datetime.now(timezone.utc).isoformat(),
            ...
            print('PASS: 15 V2 transport/envelope-review tests; seven saved native lifetimes checked; no new application or model',flush=True)
        except BaseException as exc:
            freeze(output/'FAILED.json',dict(status='FAILED_TRANSPORT_V2_REVIEW_PROBE_PRESERVED',error_type=type(exc).__name__,admission=bind(output/'ADMISSION.json')))
            raise
