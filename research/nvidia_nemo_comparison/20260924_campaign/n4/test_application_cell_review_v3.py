"""Synthetic joined V3 cell fixtures; never execute fixture apps or audio."""
...
import review_application_cell_v3 as review
...
from delivery_review_fixtures import add_delivery, rebind_delivery
from paced_panel_plan_v3 import APPLICATION_POLICY
from source_delivery import RECORD,summarize
...
def rebind_closure(folder):
    ...
    rebind_delivery(folder,load(folder/'transport/INPUT.json'))
...
class CellReviewTests(unittest.TestCase):
    ...
    def fixture(self, *, invisible=False):
        ...
        for name in ('fixture.exe', 'paced_application_runner_v3.py'):
            (base/name).write_text('INERT JOIN FIXTURE; NEVER EXECUTED', encoding='utf-8')
        exe = bind(base/'fixture.exe'); script = bind(base/'paced_application_runner_v3.py'); code = [script]
        ...
        prepared.update(application_variant=APPLICATION_POLICY['variant'],delivery_policy=deepcopy(APPLICATION_POLICY['delivery']))
        rewrite(app/'PREPARED.json',prepared)
        result = load(app/'RESULT.json'); result.update(closure=validate_complete(observed,load(app/'ARCHIVE_INTEGRITY.json')),
            ...
            status=APPLICATION_POLICY['success_status'])
        rewrite(app/'RESULT.json', result)
        ...
        freeze(folder/'COLLECTED.json', dict(status='COLLECTED_SOURCE_DELIVERY_PACED_CELL_REQUIRES_REVIEW',cell_id=payload['cell_id'],input=permit['input'],
            ...
            application_policy=deepcopy(APPLICATION_POLICY),source_delivery_envelope=bind(folder/'SOURCE_DELIVERY_ENVELOPE.json')))
    ...
    def test_all_real_readers_compose_with_synthetic_facts(self):
        ...
        self.assertEqual(result['status'],'PASS_V3_APPLICATION_CELL_EVIDENCE_JOINS_ONLY')
        ...
        self.assertEqual(result['joins']['delivery_samples'],args['payload']['job']['frames'])
        self.assertGreater(result['joins']['delivery_records'],0);self.assertFalse(result['joins']['delivery_deadlines_accepted'])
    ...
    def test_delivery_reader_result_cannot_change_between_composed_joins(self):
        folder,args=self.fixture();transport=review.review_transport(folder,**args)
        checked=review.review_observations(folder/'application',payload=args['payload'],application_owner=transport['application'])
        transport['source_delivery_review']['source_samples']+=1
        with self.assertRaisesRegex(ValueError,'Delivery envelope review changed'):
            review.join_reviews(folder,args['payload'],transport,checked)

    def test_complete_delivery_after_engine_closure_is_refused(self):
        folder,args=self.fixture();app=folder/'application';path=app/'delivery/TRACE.bin'
        rows=list(RECORD.iter_unpack(path.read_bytes()));last=list(rows[-1]);last[4]=155.;rows[-1]=tuple(last)
        path.write_bytes(b''.join(RECORD.pack(*r) for r in rows))
        value=load(app/'delivery/OBSERVATION.json');frames=args['payload']['job']['frames']
        value['summary']=summarize(path.read_bytes(),origin=value['source_origin_perf_counter'],frames=frames,sent=frames,committed=frames)
        rewrite(app/'delivery/OBSERVATION.json',value)
        capture=load(app/'delivery/CAPTURE.json');capture.update(trace=bind(path),observation=bind(app/'delivery/OBSERVATION.json'))
        rewrite(app/'delivery/CAPTURE.json',capture);rebind_delivery(folder,args['payload'])
        with self.assertRaisesRegex(ValueError,'append completed after recorded engine closure'):review.review_cell(folder,**args)

    def test_observation_version_rejects_old_cell_status_and_policy(self):
        for kind in ('status','prepared_policy','result_policy'):
            folder,args=self.fixture();app=folder/'application'
            if kind=='prepared_policy':
                row=load(app/'PREPARED.json');row['delivery_policy']={};rewrite(app/'PREPARED.json',row)
            else:
                row=load(app/'RESULT.json')
                if kind=='status':row['status']='CELL_CLOSED_REQUIRES_REVIEW'
                else:row['application_variant']='old'
                rewrite(app/'RESULT.json',row)
            with self.subTest(kind=kind),self.assertRaises(ValueError):
                review.review_observations(app,payload=args['payload'],application_owner=observations.OWNER)
