"""Guarded derivative-context and invisible application prestart qualification."""
import argparse
...
def code_bindings():
    names=('application_source_context.py','test_application_source_context.py','test_journal_application_prestart.py',
        'probe_journal_application_prestart.py','README_APPLICATION_SOURCE_CONTEXT.md','paced_application_runner.py',
...
def child(admission_path,nonce):
    ...
    stream=io.StringIO();tests=unittest.TextTestRunner(stream=stream,verbosity=2).run(
        unittest.defaultTestLoader.loadTestsFromTestCase(JournalApplicationTests))
    (root/'gui_tests.txt').write_text(stream.getvalue(),encoding='utf-8')
    for b in admission['code']:verify(b)
    freeze(root/'CHILD_RESULT.json',dict(status='PASS_JOURNAL_APPLICATION_PRESTART_ONLY' if tests.wasSuccessful() else 'FAILED_PRESERVED',
...
def run(output):
    ...
    with writer_lock(LOCAL/'n4/metric-scoring.owner.lock'):
        ...
        try:
            ...
            require(result['status']=='PASS_JOURNAL_APPLICATION_PRESTART_ONLY' and result['tests_run']==9 and result['skipped']==0
            ...
            freeze(output/'RESULT.json',dict(status='PASS_JOURNAL_DERIVATIVE_CONTEXT_AND_PRESTART_ONLY',utc=datetime.now(timezone.utc).isoformat(),
            ...
            print('PASS: four lineage checks and nine private GUI checks; 16 backends prepared and closed; no inference',flush=True)
        except BaseException as exc:
            if owned is not None and not owned.closed:owned.close(grace_seconds=2)
            freeze(output/'FAILED.json',dict(status='FAILED_JOURNAL_DERIVATIVE_PRESTART_PRESERVED',error_type=type(exc).__name__,
