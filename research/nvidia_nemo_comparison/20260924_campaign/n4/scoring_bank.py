"""Score terminal method banks; never import predictor code. README_SCORING_BANK.md."""
import argparse
...
def admit(run,review_path=None):
    ...
    # Require the qualified frozen bank implementation, not a new unreviewed fork.
    implemented=load(HERE/'INTEGRATED_BANK_IMPLEMENTATION_V1.json')
    known={b['path']:b for b in bindings(implemented)}
    for b in c['code']:
        if Path(b['path']).name in ('integrated_bank.py','integrated_bank_plan.py') and known.get(b['path'])!=b:
            raise ValueError('Method implementation has not been qualified')
...
def code_bindings():
    return [bind(HERE/n) for n in ('metric_process.py','scoring_bank.py','scoring_report.py','test_scoring_bank.py',
        'probe_metric_process.py','README_SCORING_BANK.md','integrated_scoring_adapter.py','metrics.py','paired.py','common.py',
        'probe_integrated_scoring.py')]
...
def _run(args,p):
    started=time.monotonic();bundle=admit(args.run,args.review);plan=bundle['plan'];terminal=bundle['terminal']
    ...
    freeze(args.output/'ADMISSION.json',dict(owner=identity(p),terminal=bundle['terminal_binding'],review=bundle['review'],
        plan=terminal['plan'],code=code,environment=environment,environment_files_verified=environment_files,
