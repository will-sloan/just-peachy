"""Bounded saved-vector and source-clock qualification. README_PACED_ADAPTERS_V2.md."""
import argparse
...
def run(args):
    ...
    code = [bind(here/n) for n in ('paced_adapters_v2.py', 'test_paced_adapters_v2.py', 'probe_paced_adapters_v2.py',
        'README_PACED_ADAPTERS_V2.md', 'mode_galleries.py', 'controller_projection.py', 'common.py',
    ...
    freeze(args.output/'ADMISSION.json', dict(owner=identity(process), source=public['source_receipt'], code=code,
        preparation=preparation, runtimes=runtimes, expected_tests=7, private_bytes_at_start=size,
        scope='No model/source execution or GUI. Existing research vectors and synthetic source events only.'))
    sys.path[:0] = [str(root), str(root/'vendor')]
    import test_paced_adapters_v2 as tests
