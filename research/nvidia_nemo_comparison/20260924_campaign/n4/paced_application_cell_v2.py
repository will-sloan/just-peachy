"""Explicit source-delivery application variant; README_APPLICATION_DELIVERY.md."""
...
from application_delivery import SourceLaunchCapture, POLICY, review_files
...
class ApplicationCell:
    ...
    def __init__(self, output, job, contract):
        ...
        self.clock=self.viewport=None;self.delivery=None;self.engine=self.consumer=None;self.errors=[];self.started=False;self.closed=False
    ...
    def prepare(self, *, source, models_root, runtimes, gallery_preparation):
        ...
        self.delivery=SourceLaunchCapture(self.c,self.job,self.contract,source)
        ...
        receipt=dict(status='PREPARED_NO_SOURCE_OR_MODELS_STARTED',desktop=self.desktop,backend_id=backend,
            ...
            no_auto_start=True,delivery_policy=POLICY,application_variant='source-delivery-v2',integrated_N4_cells=0)
    ...
    def check(self):
        ...
        if self.delivery is not None:self.delivery.check()
    ...
    def close(self):
        ...
        errors=[];closure=None;delivery_receipt=None;delivery_join=None
        ...
        # Delivery extraction stays inside the observed process lifetime. Its
        # collection is additional to, never a replacement for, engine/archive closure.
        if self.delivery is not None:
            try:
                delivery_receipt=self.delivery.finish(self.output/'delivery',engine=self.engine)
                captured=load(delivery_receipt['path'])
                if self.started:
                    if captured['status']!='COLLECTED_APPLICATION_DELIVERY_REQUIRES_REVIEW':
                        raise RuntimeError('Source delivery capture failed')
                    delivery_join=review_files(self.output,self.job,self.contract,self.delivery.source_files)
                elif captured['status']!='PREPARED_WITHOUT_SOURCE_DELIVERY':
                    raise RuntimeError('Prestart delivery capture unexpectedly ran')
            except Exception as exc:errors.append('Source delivery: '+repr(exc))
        elif self.started:errors.append('Source delivery capture absent')
        resources=self.resources.close()
        if resources['status']!='OBSERVED_HOST_RESOURCES':errors.append('Resources: '+str(resources['error']))
        self.closed=True
        self.result=dict(status='CELL_WITH_SOURCE_DELIVERY_CLOSED_REQUIRES_REVIEW' if self.started and closure is not None and delivery_join is not None and not errors else
            ...
            application_variant='source-delivery-v2',delivery_policy=POLICY,
            delivery_capture=delivery_receipt,delivery_join=delivery_join,
