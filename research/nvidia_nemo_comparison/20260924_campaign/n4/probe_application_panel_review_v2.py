"""Development-only V2 full-population review checks. See its README."""
import argparse
...
from review_application_panel_v2 import code_bindings
import test_application_panel_review_v2 as regression
...
def run(output):
    ...
    with writer_lock(LOCAL/'n4/metric-scoring.owner.lock'):
        ...
        q=load(HERE/'APPLICATION_CELL_REVIEW_CHECK_V2.json');verify(q['synthetic_join_review'])
        (output/'source').mkdir();snapshots=[]
        for b in code[:4]:
            path=output/'source'/Path(b['path']).name;path.write_bytes(Path(b['path']).read_bytes())
            require(bind(path)['sha256']==b['sha256'],'Attempt source snapshot changed');snapshots.append(bind(path))
        ...
        try:
            ...
            require(tests.wasSuccessful() and tests.testsRun==8 and not tests.skipped,'V2 panel census checks failed')
            for b in code:verify(b)
            ...
            freeze(output/'RESULT.json',dict(status='PASS_V2_PANEL_CENSUS_REVIEW_CHECKS_ONLY',utc=datetime.now(timezone.utc).isoformat(),
            ...
            print('PASS: eight exact-population and scope checks; no production plan/panel or application run',flush=True)
        except BaseException as exc:
            freeze(output/'FAILED.json',dict(status='FAILED_V2_PANEL_CENSUS_PROBE_PRESERVED',error_type=type(exc).__name__,admission=bind(output/'ADMISSION.json')))
            raise
