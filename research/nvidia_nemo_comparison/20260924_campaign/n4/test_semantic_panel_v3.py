...
import review_semantic_panel_v3 as panel
import test_application_panel_review_v3 as prior

CONTEXT={}


class SemanticPanelTests(prior.PanelReviewTests):
    @classmethod
    def setUpClass(cls):
    ...
    def test_old_semantic_receipt_and_modified_reader_chain_are_refused(self):
        for change in ('label','timing','content_digest'):
            checked=deepcopy(self.checked)
            if change=='label':checked['status']='PASS_V2_APPLICATION_RECORDED_GUI_LABELS_ONLY'
            elif change=='timing':checked['observed']['status']='PASS_V2_APPLICATION_RECORDED_ROW_TIMING_ONLY'
            else:checked['observed']['content_review_sha256']='0'*64
            from common import fingerprint
            score=deepcopy(self.score);score['observed_review_sha256']=fingerprint(checked)
            with self.subTest(change=change),self.assertRaises(ValueError):
                panel.compact(checked,score,self.references,self.row,self.payload,{})

    def test_compact_semantics_keeps_raw_delivery_without_latency_promotion(self):
        source=self.example['source_delivery'];cell=self.checked['observed']['content']['cell']
        from common import fingerprint,verify
        summary=cell['transport']['source_delivery_review']['summary']
        self.assertEqual(source['summary_sha256'],fingerprint(summary))
        self.assertTrue(source['raw_summary_preserved_in_bound_envelope'])
        self.assertEqual(self.registry[source['envelope']['path']],source['envelope'])
        verify(source['envelope']);self.assertEqual(load(source['envelope']['path'])['review']['summary'],summary)
        self.assertEqual(source['source_samples'],self.payload['job']['frames'])
        self.assertFalse(source['scheduling_or_append_cost_subtracted'])
        self.assertFalse(source['deadline_or_continuity_accepted'])
        self.assertFalse(self.example['source_to_widget_latency_qualified'])
