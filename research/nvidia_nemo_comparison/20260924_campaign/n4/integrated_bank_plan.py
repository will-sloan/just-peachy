"""Exact reviewed-component join for N4. See README_INTEGRATED_BANK.md."""
...
def code_bindings():
    ...
    return [bind(HERE/n) for n in names]+[bind(HERE.parent/'supervision/supervisor.py')]
...
def prepare(args):
    ...
    context=dict(source_receipt=source,gallery_preparation=gallery,catalog=galleries['catalog'],manifest=manifest,panel=panel,
        reviews=reviews,runtimes=runtimes,qualification=[bind(HERE/'APPLICATION_PUBLICATION_CHECK_V1.json'),bind(HERE/'EMPTY_CONTROLLER_CHECK_V1.json')],
        code=code_bindings())
