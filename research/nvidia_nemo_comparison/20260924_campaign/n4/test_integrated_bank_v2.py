"""Full-census join and actual boundary regressions. README_INTEGRATED_BANK_CLOCK_V2.md."""
...
from integrated_bank_plan_v2 import build_join,exact_jobs,review_sources,read_component_bank,MAIN_MODE
from integrated_bank_v2 import predict_cell,guard,run,GIB
from mode_galleries import backend_contract
from method_artifact_v2 import read_replay
...
class TestArtifactBounds(unittest.TestCase):
    def test_exact_roundtrip_hash_size_and_expansion_bounds(self):
        from method_artifact_v2 import save_private, read_replay
        with tempfile.TemporaryDirectory() as tmp,patch('method_artifact_v2.LIMIT',128):
            path=Path(tmp)/'good.json.gz';value={'text':'x'*70}
            binding=save_private(path,value);self.assertEqual(read_replay(binding),value)
            for key,val in [('expanded_bytes',129),('expanded_bytes',True),('expanded_sha256','0'*64),('expanded_bytes',1)]:
                bad=deepcopy(binding);bad[key]=val
                with self.assertRaises(ValueError):read_replay(bad)
            target=Path(tmp)/'oversize.json.gz'
            with self.assertRaises(ValueError):save_private(target,{'text':'y'*150})
            self.assertFalse(target.exists())
            with self.assertRaises(FileExistsError):save_private(path,value)

    def test_corrupted_gzip_rejected_even_with_new_compressed_binding(self):
        from method_artifact_v2 import save_private,read_replay
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/'data.gz';binding=save_private(path,{'hello':'saved'})
            data=bytearray(path.read_bytes());data[-8]^=1;path.write_bytes(data)
            binding['compressed']=bind(path)
            with self.assertRaises((ValueError,OSError,EOFError)):read_replay(binding)


class TestLongNativeBoundary(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        TestIntegratedBank.setUpClass.__func__(cls)

    def test_both_clock_failure_clips_with_both_encoders(self):
        from contextlib import ExitStack
        local=Path('G:/Just_Peachy_N1/20260924_campaign/local/n4')
        for job_id in ['N2_S45_03_11_O0','N2_S45_06_01_O0']:
            for encoder in ['E0','E1']:
                with self.subTest(job=job_id,encoder=encoder),ExitStack() as stack:
                    root=Path(stack.enter_context(tempfile.TemporaryDirectory())) if BOUNDARY_OUTPUT is None else BOUNDARY_OUTPUT/(job_id+'_'+encoder)
                    root.mkdir(parents=True,exist_ok=True)
                    inputs=[bind(local/'asr-full-bank-v1/A3'/job_id/'RESULT.json'),
                            bind(local/'d1-full-bank-v1'/encoder/job_id/'RESULT.json')]
                    cells=[load(b['path']) for b in inputs]
                    job=cells[0]['job'];self.assertEqual(cells[1]['job'],job)
                    backend=next(r['key'] for r in self.catalog['backends'] if r['implemented']
                        and tuple(backend_contract(self.catalog,r['key'],MAIN_MODE)[k]
                                  for k in ['variant','diarization','encoder'])==('A3','D1',encoder))
                    parents=[dict(result=b,cache_key=c['cache_key'],events=c['events']) for b,c in zip(inputs,cells)]
                    row=dict(cell_id='clock-boundary-'+job_id+'_'+encoder,job_id=job_id,
                             contract=backend_contract(self.catalog,backend,MAIN_MODE),parents=parents,
                             cache_key=fingerprint(parents),D0_E1_association_qualification='FROZEN_NOMINAL')
                    result=predict_cell(row,job,self.context,root)
                    self.assertEqual(result['status'],'COMPLETE_MODELED_APPLICATION_METHODS_ONLY')
                    self.assertTrue(result['controller_closed'])
                    ...
                    publication=read_replay(result['outputs']['publication'])
                    self.assertGreater(publication['worker_counts']['completed'],0)
                    freeze(root/'BOUNDARY_RESULT.json',dict(result=result,inputs=inputs,
                        actual_original_480_audio_job=True,development_only=True))
...
class TestIntegratedBank(unittest.TestCase):
    ...
    def test_resource_guard_stops_before_allocating_or_crossing_cutoff(self):
        ...
        with tempfile.TemporaryDirectory() as tmp,patch('integrated_bank_v2.shutil.disk_usage',return_value=SimpleNamespace(free=200*GIB)):
        ...
        with patch('integrated_bank_v2.shutil.disk_usage',return_value=SimpleNamespace(free=40*GIB)):
    ...
    def test_failed_prediction_preserved_and_remaining_denominators_retained(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('integrated_bank_v2.verify_plan',return_value=plan),patch('integrated_bank_v2.guard'),\
                    patch('asr_full_bank.payload_inventory',return_value={'errors':[],'total_logical_bytes':0}),\
                    patch('integrated_bank_v2.predict_cell',side_effect=predictor):
    ...
    def test_pre_cell_resource_stop_does_not_invent_a_failed_prediction(self):
        with tempfile.TemporaryDirectory() as tmp:
            plan,args=self.lifecycle_fixture(tmp)
            with patch('integrated_bank_v2.verify_plan',return_value=plan),\
                    patch('integrated_bank_v2.guard',side_effect=[None,ValueError('fixture budget stop')]),\
                    patch('asr_full_bank.payload_inventory',return_value={'errors':[],'total_logical_bytes':0}),\
                    patch('integrated_bank_v2.predict_cell') as predictor:
