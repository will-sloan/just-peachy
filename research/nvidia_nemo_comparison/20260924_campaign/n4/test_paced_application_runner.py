...
import paced_application_runner as runner
...
class RunnerTests(unittest.TestCase):
    ...
    def fake_collection(self, root, *, fail_resume=False, failed_cleanup=False):
        ...
        class Child:
            ...
            def resume(self,register):
                ...
                app=folder/'application';freeze(app/'RESULT.json',dict(status='CELL_CLOSED_REQUIRES_REVIEW',source_start_requested=True,errors=[]))
                permit=load(self.output/'PERMIT.json')
                freeze(self.output/'CHILD_RESULT.json',dict(status='COLLECTED_APPLICATION_CELL_REQUIRES_REVIEW',
                    owner=dict(pid=123,create_time=1.),input=permit['input'],cell_result=bind(app/'RESULT.json')))
    ...
    def test_collection_releases_after_verified_cleanup(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(events,['acquire','spawn','register','lease','resume','close','release'])
    ...
    def worker_fixture(self, root, *, prepare_fails=False):
        ...
        class Cell:
            ...
            def close(self):
                events.append('close');result={'status':'CELL_CLOSED_REQUIRES_REVIEW'};freeze(output/'RESULT.json',result);return result
        stack=ExitStack()
        ...
        stack.enter_context(patch.dict(sys.modules,{'paced_application_cell':SimpleNamespace(ApplicationCell=Cell)}))
    ...
    def test_fixed_worker_orders_prime_prepare_run_close(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(load(transport/'CHILD_RESULT.json')['status'],'COLLECTED_APPLICATION_CELL_REQUIRES_REVIEW')
    ...
    def test_missing_production_plan_is_not_admitted(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch.object(runner,'read_review') as review,self.assertRaises(FileNotFoundError):runner.admit_plan(path)
