"""Owned Windows private-desktop process lifetime. README_PRIVATE_APPLICATION_PROCESS_V3.md."""
import ctypes as C
...
class PrivateApplicationProcess:
    ...
    def members(self):
        """Bounded job membership observation, including native console helpers."""
        require(self.job is not None and not self.closed, 'Owned open job required')
        class PROCESS_LIST(C.Structure):
            _fields_ = [('assigned', W.DWORD), ('listed', W.DWORD), ('pids', C.c_size_t*16)]
        listing = PROCESS_LIST()
        checked(self.kernel.QueryInformationJobObject(self.job, 3, C.byref(listing), C.sizeof(listing), None))
        require(listing.assigned == listing.listed and listing.listed <= 16, 'Job census incomplete')
        rows = []
        for pid in listing.pids[:listing.listed]:
            try:
                process = psutil.Process(pid)
                rows.append(dict(**identity(process), executable=process.exe(), affinity=process.cpu_affinity(),
                    parent_pid=process.ppid(), argv_sha256=fingerprint(process.cmdline())))
            except psutil.NoSuchProcess: rows.append(dict(pid=pid, exited_during_observation=True))
        self._event('job_members_observed', members=rows)
        return rows
    ...
    def close(self, *, grace_seconds=2, force_seconds=10):
        ...
        forced = False; empty = False; error = None; final_job = None; exit_code = None; observed_members_exited = False
        try:
            if self.assigned: self.members()
            ...
            observed = {(r['pid'], r['create_time']): r for event in self.events for r in event.get('members', []) if 'create_time' in r}
            if self.owner: observed[(self.owner['pid'], self.owner['create_time'])] = self.owner
            until = time.monotonic()+force_seconds
            while any(exact_process(owner) is not None for owner in observed.values()):
                require(time.monotonic() < until, 'An exact observed owned member remains after job/root exit')
                time.sleep(.025)
            observed_members_exited = True
            self._event('all_observed_member_identities_exited', owners=[dict(pid=p, create_time=c) for p, c in observed])
            ...
            okay = empty and observed_members_exited and error is None and not cleanup_errors and after == self.before
            self.receipt = dict(status='OWNED_PROCESS_LIFETIME_CLOSED' if okay else 'FAILED_LIFETIME_PRESERVED',
                ...
                job_empty_verified=empty, observed_members_exited=observed_members_exited, final_job=final_job, root_exit_code=exit_code,
