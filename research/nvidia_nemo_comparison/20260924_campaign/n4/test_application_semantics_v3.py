"""V3 actual-reader composition with fabricated application/clock facts only."""
...
class SemanticTests(unittest.TestCase):
    ...
    def fixture(self, *, baseline=False, wrong_caption=False):
        ...
        # This fixture may switch the synthetic backend after construction.
        # Bind its fabricated capture to that same contract before review.
        capture_path=app/'delivery/CAPTURE.json';capture=load(capture_path)
        capture['contract_fingerprint']=fingerprint(payload['contract']);rewrite(capture_path,capture)
    ...
    def test_actual_readers_join_complete_synthetic_n2_cell(self):
        folder,args = self.fixture(); result = content.review_content(folder,**args)
    ...
    def test_actual_readers_join_baseline_sorted_roster(self):
        folder,args = self.fixture(baseline=True); result = content.review_content(folder,**args)
    ...
    def test_snapshot_roster_cannot_change_display_spelling(self):
        ...
        with self.assertRaisesRegex(ValueError,'Observed display roster'): content.review_content(folder,**args)
    ...
    def test_enabled_assistance_fails_before_widget_interpretation(self):
        ...
        with patch.object(content,'review_widget') as later, self.assertRaisesRegex(ValueError,'assistance/reference'):
            content.review_content(folder,**args)
    ...
    def test_manual_correction_is_not_primary_caption_evidence(self):
        ...
        with self.assertRaisesRegex(ValueError,'manual or optional'): content.review_content(folder,**args)

    def test_valid_transport_does_not_certify_wrong_widget_text(self):
        folder,args=self.fixture(wrong_caption=True)
        with self.assertRaisesRegex(ValueError,'compatible preceding'): content.review_content(folder,**args)
    ...
    def test_valid_envelope_does_not_certify_corrupt_native_partition(self):
        ...
        with self.assertRaisesRegex(ValueError,'fragment changed'): content.review_content(folder,**args)
    ...
    def test_full_semantic_chain_and_reference_join_stay_development_only(self):
        folder,args=self.fixture();observed=content.review_labels(folder,**args)
        references=CONTEXT['references'];score=content.score_cell(references,observed,args['payload'])
        self.assertEqual(observed['status'],'PASS_V3_APPLICATION_RECORDED_GUI_LABELS_ONLY')
        self.assertEqual(score['native_lexical_spans'],4)
        self.assertEqual(set(score['scenarios']),{'observed','all_unknown','constant_name'})
        self.assertTrue(observed['observed']['content']['cell']['transport']['delivery_trace_independently_parsed'])
        for k in ('naming_accuracy_qualified','acquisition_latency_qualified','continuous_wrong_name_exposure_measured','N4_accepted'):
            self.assertIs(score[k],False)
        self.assertFalse(observed['evaluator_truth_loaded'])
        freeze(self.root/'SYNTHETIC_SEMANTIC_REVIEW.json',observed)
        freeze(self.root/'SYNTHETIC_NAME_SCORE.json',score)

    def test_v2_heading_receipt_is_not_promoted_to_v3(self):
        folder,args=self.fixture();observed=content.review_labels(folder,**args)
        observed['status']='PASS_V2_APPLICATION_RECORDED_GUI_LABELS_ONLY'
        with self.assertRaisesRegex(ValueError,'Independent observed-heading'):
            content.score_cell(CONTEXT['references'],observed,args['payload'])

    def test_old_reference_context_is_not_silently_reused(self):
        folder,args=self.fixture();observed=content.review_labels(folder,**args)
        references=deepcopy(CONTEXT['references']);references['inputs']['application_context']=CONTEXT['old_reference_context']
        with self.assertRaisesRegex(ValueError,'Observed job/gallery/context'):
            content.score_cell(references,observed,args['payload'])

    def test_delivery_trace_mutation_stops_before_semantic_projection(self):
        folder,args=self.fixture();path=folder/'application/delivery/TRACE.bin'
        with path.open('ab') as stream:stream.write(b'changed')
        with patch.object(content,'review_widget') as later,self.assertRaises(ValueError):
            content.review_labels(folder,**args)
        later.assert_not_called()

    def test_signed_timing_and_missing_final_observation_are_preserved(self):
        folder,args=self.fixture();value=content.review_timing(folder,**args)
        self.assertEqual(value['timing']['counts']['lexical_word']['without_final_row_visible_observation'],1)
        self.assertFalse(value['source_to_widget_latency_qualified'])
        from review_application_timing import sampled_state
        entry=dict(kind='lexical_word',source_window_seconds=[18.,25.],first_publication=dict(sequence=1,monotonic_sec=110.),first_final_publication=dict(sequence=2,monotonic_sec=111.))
        state=dict(observed_monotonic_sec=120.,publication_monotonic_range=[112.,116.],compatible_publications=2,
            ambiguous_native_publication=True,exact_consumed_event_attribution=False,caption_visible=True,retired=False,
            row_id='fixture',row_ref={'fixture':True},candidate_bindings_sha256='0'*64)
        result=sampled_state(state,dict(row_id='fixture',caption_visible=True,final=True),entry,100.,'first_visible')
        self.assertEqual(result['source_window_relative_interval_seconds'],[-5.,2.])
        self.assertIsNone(result['continuous_exposure_seconds'])

    def test_v3_reference_population_and_rosters_are_complete(self):
        context=CONTEXT['references'];p=context['population']
        self.assertEqual((p['cells'],p['scenes'],p['turns'],p['identities']),(480,240,1554,43))
        self.assertTrue(context['NEVER_PASS_TO_RUNTIME']);self.assertFalse(context['audio_loaded'])
        for gallery in context['galleries'].values():
            self.assertEqual((gallery['available_size'],gallery['intended_size'],gallery['unavailable_count']),(24,34,10))

    def test_gallery_migration_rejects_changed_bytes_at_new_path(self):
        inputs=CONTEXT['references']['inputs'];current=load(inputs['application_context']['path'])
        old=load(CONTEXT['old_reference_context']['path'])
        old_inputs=dict(inputs,gallery_preparation=old['gallery_preparation'])
        changed=deepcopy(current);document=load(current['gallery_preparation']['path'])
        document['model_inference']='foreign';path=self.root/'FOREIGN_GALLERY.json';freeze(path,document)
        changed['gallery_preparation']=bind(path)
        with self.assertRaisesRegex(ValueError,'changed gallery content'):
            content.migrate_reference_inputs(old_inputs,old,changed,inputs['application_context'])

    def test_gallery_migration_requires_prior_context_binding(self):
        inputs=CONTEXT['references']['inputs'];current=load(inputs['application_context']['path'])
        old=load(CONTEXT['old_reference_context']['path'])
        self.assertNotEqual(old['gallery_preparation']['path'],current['gallery_preparation']['path'])
        with self.assertRaisesRegex(ValueError,'Prior reference gallery context'):
            content.migrate_reference_inputs(inputs,old,current,inputs['application_context'])
