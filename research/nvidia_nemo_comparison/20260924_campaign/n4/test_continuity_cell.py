...
import review_continuity_cell as review
...
class CellReviewTests(unittest.TestCase):
    ...
    def fixture(self, *, invisible=False):
        ...
        for name in ('fixture.exe', 'continuity_application_runner.py'):
            (base/name).write_text('INERT JOIN FIXTURE; NEVER EXECUTED', encoding='utf-8')
        exe = bind(base/'fixture.exe'); script = bind(base/'continuity_application_runner.py'); code = [script]
    ...
    def test_all_real_readers_compose_with_synthetic_facts(self):
        ...
        self.assertEqual(result['status'],'PASS_CONTINUITY_CELL_EVIDENCE_JOINS_ONLY')
