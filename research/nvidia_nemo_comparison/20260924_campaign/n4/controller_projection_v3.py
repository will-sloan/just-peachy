"""Empty-safe derivative of Controller projection. README_INTEGRATED_HISTORY_V3.md."""
...
def validate_display_history(replay):
    ...
    if len(rows)>100000:raise ValueError('Bounded display history required')
    if not rows:return validate_empty_publication(replay)
...
def validate_empty_publication(replay):
    """Admit zero captions only from a complete explicit publication result."""
    if replay.get('publication_method_qualification')!='ACTUAL_METHODS_WITH_MODELED_MODULE_CLOCKS':
        raise ValueError('Empty display needs complete publication-method evidence')
    presentation=replay['presentation'];worker=replay['worker_counts'];policy=replay['policy_snapshot']
    if (presentation['rows'] or any(presentation[k]!=0 for k in ('raw_observations','final_utterances','formatting_revisions'))
            or worker['thread_alive'] or worker['error'] or not worker['closed'] or worker['accepted']!=worker['completed']
            or not policy['closed'] or policy['pending_events']):
        raise ValueError('Empty captions cannot hide text or incomplete policy work')
    publications=replay['publication_events'];session=None;previous=-1.;closed=[]
    if not publications or len(publications)>100000:raise ValueError('Empty result lacks bounded publication closure')
    for sequence,event in enumerate(publications,1):
        kind,p=event['event_type'],event['payload'];at=p['publication_monotonic_sec']
        if kind.startswith('transcript_') or kind in ('research_asr_observation','s6d_text_ready','s6d_display','s6d_punctuation_revision'):
            raise ValueError('Empty display discarded a raw or caption event')
        if (type(at) not in (int,float) or not math.isfinite(at) or at<previous
                or p['publication_sequence']!=sequence or not p.get('session_id')):
            raise ValueError('Empty publication chronology or identity differs')
        session=session or p['session_id'];previous=at
        if p['session_id']!=session:raise ValueError('Foreign empty publication session')
        if kind=='research_scheduler_watermark' and p['lane_closed']:
            if p['lower_bound_sec'] is not None or p['lane'] not in ('asr','speaker') or p['lane'] in closed:
                raise ValueError('Empty publication has invalid lane closure')
            closed.append(p['lane'])
    if set(closed)!={'asr','speaker'}:raise ValueError('Empty publication needs both lane closures')
    return session
...
def project_mode_result(replay,*,tap,preparation,n2_runtime,n3_runtime,data_root,record_history=True):
    ...
    try:
        ...
        def observe(event,index):
            ...
            if record_history:
                ...
                if history_bytes>64*1024**2:raise ValueError('Controller projection history exceeds 64-MiB bound')
    ...
    result['empty_caption_session']=not replay['display_events']
    return result
