"""Bounded private viewport evidence with span references. README_VIEWPORT_LEDGER_V2.md."""
...
class ViewportLedger:
    ...
    def close(self):
        ...
        with (self.directory/'SUMMARY.json').open('xb') as stream:
            if stream.write(data) != len(data): raise OSError('Incomplete compact summary write')
        return result
...
def expand_summary(path, summary):
    ...
    for item in spans.values():
        for kind in ('first_visible','first_final_visible','latest'):
            v = item[kind]
            if v is not None:
                # First/final/latest and multiple spans can deliberately share
                # one immutable compact state. Never mutate that shared value.
                item[kind] = dict(observed_monotonic_sec=v['observed_monotonic_sec'],
                    source_elapsed_sec=v['source_elapsed_sec'], row=read_row(path, v['row_ref']))
