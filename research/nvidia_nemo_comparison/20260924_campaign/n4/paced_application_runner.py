"""Fixed supervised coordinator/child wiring. README_PACED_APPLICATION_RUNNER.md."""
import argparse
import ctypes as C
...
from paced_child_admission import ChildAdmission, SCHEMA, write_lease
from paced_panel_plan import (build_plan, code_bindings as planner_code, execution_payload,
...
QUALIFICATIONS = {
    'PACED_PANEL_PLAN_CHECK_V1.json': 'PASS_PACED_PANEL_PLAN_DEVELOPMENT_ONLY',
...
def code_bindings():
    names = ('paced_application_runner.py', 'test_paced_application_runner.py', 'probe_paced_application_runner.py',
        'README_PACED_APPLICATION_RUNNER.md')
...
def admit_plan(path):
    """Reconstruct the full prepared panel from its reviewed banks and bindings."""
    plan_binding = bind(path); plan = load(path); folder = Path(path).parent
    result = load(folder/'RESULT.json'); verify(result['admission']); admission = load(result['admission']['path'])
    require(result['status'] == 'PREPARED_PANELS_AND_REPEATS_ONLY' and result['plan'] == plan_binding,
        'Complete prepared panel receipt required')
    require(exact_process(admission['owner']) is None and admission['code'] == planner_code(), 'Planner active or code changed')
    for b in admission['code']+[admission['preparation'], admission['selection']]: verify(b)
    mb, main = read_review(Path(plan['reviews']['main']['path']))
    xb, modes = read_review(Path(plan['reviews']['modes-panel']['path']))
    validate_review_pair(main, modes); reviews = {'main': mb, 'modes-panel': xb}
    require(admission['reviews'] == reviews == plan['reviews'], 'Panel review join differs')
    selection = load(admission['selection']['path']); validate_selection(selection, reviews)
    require(selection == plan['selection'], 'Panel selection changed')
    preparation = load(admission['preparation']['path'])
    ...
    ctx = main['context']; app_binding = bind(HERE/'PACED_APPLICATION_CELL_CHECK_V1.json'); app = load(app_binding['path'])
    require(ctx['manifest'] == manifest and ctx['panel'] == panel and app['source_receipt'] == ctx['source_receipt']
        and app['gallery_preparation'] == ctx['gallery_preparation'], 'Prepared panel/source/gallery differs')
    ...
    require({Path(b['path']).name: b for b in app_admission['runtimes']} == ctx['runtimes']
        and app_admission['catalog'] == ctx['catalog'], 'Qualified application runtime differs')
    assets = {}; models_root = None
    for kind in ('ASR', 'D1'):
        r = load(ctx['reviews'][kind]['path']); verify(r['admission']); component = load(r['admission']['path'])
        require(component['component_contract']['source_receipt'] == ctx['source_receipt'], 'Reviewed component source differs')
        if models_root is None: models_root = component['models_root']
        require(models_root == component['models_root'], 'Model root differs')
        for b in component['component_contract']['assets']:
            require(b['path'] not in assets or assets[b['path']] == b, 'Conflicting model binding'); assets[b['path']] = b
    context = dict(source_receipt=ctx['source_receipt'], catalog=ctx['catalog'], runtimes=ctx['runtimes'],
        ...
        planner_qualification=bind(HERE/'PACED_PANEL_PLAN_CHECK_V1.json'), code=planner_code())
    rebuilt = build_plan(selection, reviews, load(manifest['path'])['jobs'], load(panel['path'])['jobs'],
        load(regression['path'])['jobs'], load(ctx['catalog']['path']), context)
    require(plan == rebuilt and result['required'] == plan['required'], 'Reconstructed panel differs')
    for b in assets.values(): verify(b)
    verify(plan_binding)
    return plan_binding, plan
...
def collect_one(plan, index, folder, state, code):
    ...
    try:
        ...
        freeze(folder/'COLLECTED.json', dict(status='COLLECTED_SOURCE_PACED_CELL_REQUIRES_REVIEW', cell_id=payload['cell_id'],
            ...
            monitoring=monitoring, slot_admission=acquired, integrated_N4_cells=0, N4_accepted=False))
...
def prepare(plan_path, output):
    ...
    with writer_lock(LOCAL/'n4/metric-scoring.owner.lock'):
        ...
        code = code_bindings(); q = load(HERE/'PACED_RUNNER_CHECK_V1.json')
        require(q['status'] == 'PASS_PACED_RUNNER_DEVELOPMENT_ONLY' and q['code'] == code, 'Fixed runner is not qualified')
        executable = qualified_interpreter(); plan_binding, plan = admit_plan(plan_path)
        freeze(output/'ADMISSION.json', dict(status='PREPARED_PACED_APPLICATION_RUN_ONLY', owner=identity(process),
            plan=plan_binding, code=code, executable=executable, qualification=bind(HERE/'PACED_RUNNER_CHECK_V1.json'), inventory=inventory,
...
def run(admission_path):
    ...
    require(admission['status'] == 'PREPARED_PACED_APPLICATION_RUN_ONLY' and admission['code'] == code_bindings(), 'Run admission/code differs')
    qualification = load(HERE/'PACED_RUNNER_CHECK_V1.json')
    require(admission['qualification'] == bind(HERE/'PACED_RUNNER_CHECK_V1.json') and
        qualification['status'] == 'PASS_PACED_RUNNER_DEVELOPMENT_ONLY' and qualification['code'] == admission['code'] and
    ...
    with writer_lock(output/'run.owner.lock'):
        ...
        try:
            ...
            error = type(exc).__name__+': '+str(exc)[:2000]
        freeze(output/'RESULT.json', dict(status='COLLECTED_PACED_APPLICATION_PANEL_REQUIRES_REVIEW' if error is None else 'FAILED_PACED_APPLICATION_RUN_PRESERVED',
