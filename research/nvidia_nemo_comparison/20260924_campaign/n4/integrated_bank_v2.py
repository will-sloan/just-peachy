"""Bounded model-free N4 application-method bank. README_INTEGRATED_BANK_CLOCK_V2.md."""
import argparse
...
from integrated_bank_plan_v2 import SCHEMA,EXPECTED,build_join,code_bindings,exact_jobs,read_component_bank
from component_commands import asr_commands,d0_commands
from application_publication_v2 import replay_d0_publication,replay_d1_publication
from controller_projection_v2 import project_mode_result,forbid_inference
from probe_component_s7 import read_events
from method_artifact_v2 import read_replay,save_private
...
def run(args):
    ...
    allocation=args.allocation_gib*GIB;guard(root,allocation,policy,0)
    ...
    with lock(local/'n4/integrated-method-bank.owner.lock'):
        ...
        if inventory['errors'] or inventory['total_logical_bytes']+allocation+GIB>min(50,policy['resource_policy']['new_payload_allowance_gib'])*GIB:
            raise ValueError('Shared payload allowance insufficient, including 1-GiB contingency')
        ...
        try:
            for i,row in enumerate(plan['rows']):
                ...
                result=predict_cell(row,jobs[row['job_id']],context,cell)
                ...
                if (i+1)%128==0:
                    ...
                    if (fresh['errors'] or fresh['total_logical_bytes']+max(0,allocation-used)+GIB
...
def review(args):
    ...
    for i,(row,b) in enumerate(zip(plan['rows'],result['cells'])):
        ...
        verify(cell['closure'])
        from controller_projection_v2 import validate_display_history
...
if __name__=='__main__':
    ...
    p.add_argument('--allocation-gib',type=int,choices=range(1,9),default=8)
