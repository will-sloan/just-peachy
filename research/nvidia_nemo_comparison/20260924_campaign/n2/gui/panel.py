"""Actual source-paced Controller/Tk panel on a private desktop. See README.md."""
...
IO_HELPER=Path(__file__).resolve().parents[1]/'io_utils.py'
_io_spec=importlib.util.spec_from_file_location('_n2_gui_atomic_io',IO_HELPER)
_io=importlib.util.module_from_spec(_io_spec);_io_spec.loader.exec_module(_io)

MODULE = 'research.nvidia_nemo_comparison.20260924_campaign.n2.gui.panel'
...
def audio_jobs(regression,screen,tap='O0'):
    """Accept only the established audio firewall, never scene truth or labels."""
    rows=[]
    for manifest in (regression,screen):
        for row in manifest['jobs']:
            if set(row)!=AUDIO_KEYS or row['sample_rate_hz']!=16000 or row['gain']!=1 or row['reset_between_scenes'] is not True:
                raise ValueError('Audio-only manifest firewall failed')
            if row['tap'] not in ('O0','O1') or not isinstance(row['frames'],int) or row['frames']<=0:
                raise ValueError('Invalid audio-only job')
            rows.append(row)
    jobs=[]
    for short,scene,mode in SCENES:
        matches=[r for r in rows if r['job_id'].endswith(scene+'_'+tap)]
        # A scene appearing in both admitted panels must bind the identical audio.
        unique={(r['audio_sha256'],r['frames'],str(Path(r['audio_path']).resolve())) for r in matches}
        if len(unique)!=1:raise ValueError('Missing or conflicting admitted scene: '+scene)
        jobs.append(dict(cell_id='D1_E1_'+short,combination='D1_E1',mode=mode,example=short,audio=deepcopy(matches[0])))
    jobs.append(dict(cell_id='D1_E0_boundary',combination='D1_E0',mode='selected_closed',example='boundary',audio=deepcopy(jobs[0]['audio'])))
    return jobs
...
def aggregate_cell_reports(admission,runs,*,require_all=True):
    ...
    for run in runs:
        ...
        for field in ('source','source_bindings','inputs','common_ui_sha256','common_layout_sha256','runner','cpu','io_helper','io_version'):
...
def run_isolated_cells(admission,timeout_seconds):
    ...
    for job in admission['jobs']:
        ...
        environment=dict(os.environ,N2_GUI_ADMISSION=str(folder/'ADMISSION.json'),CUDA_VISIBLE_DEVICES='')
        ...
        try:
            ...
            command=[sys.executable,'-B','-m','prototype.tests.run_private_desktop','--receipt-dir',str(folder),
                     '--timeout-seconds',str(remaining),MODULE]
            with (folder/'LAUNCH_OUTPUT.log').open('xb') as log:
                process=subprocess.run(command,cwd=Path(__file__).resolve().parents[5],env=environment,
...
class ActualPanelTests(unittest.TestCase):
    def test_actual_saved_audio_panel(self):
        if not os.environ.get('N2_GUI_ADMISSION'):self.skipTest('Use panel.py with explicit source/config/audio/gallery inputs')
        import ctypes
        ...
        admission=load(os.environ['N2_GUI_ADMISSION']);check_binding(admission)
        ...
        import psutil
        process=psutil.Process();process.nice(psutil.BELOW_NORMAL_PRIORITY_CLASS);process.cpu_affinity([admission['cpu']])
        ...
        report=dict(schema='n2-actual-source-paced-gui-panel-v1',scope=SCOPE,desktop=desktop,status='RUNNING',cells=[],
                    admission=bound(os.environ['N2_GUI_ADMISSION']),cpu_affinity=process.cpu_affinity(),priority=int(process.nice()),
        ...
        for job in admission['jobs']:
            ...
            try:
                ...
                backend_key='nemotron_titanet' if job['combination']=='D1_E1' else 'nemotron_hybrid'
                ...
                while c.state in ('RUNNING','STARTING','STOPPING'):
                    ...
                    if elapsed>job['audio']['frames']/16000*8+180:raise TimeoutError('GUI cell timeout')
...
def main():
    ...
    for name in ('source','common-source','models-root','runtime-config','regression-manifest','screen-manifest','e0-gallery','e1-gallery','output'):
        parser.add_argument('--'+name,type=Path,required=True)
    parser.add_argument('--cpu',type=int,default=6);parser.add_argument('--tap',choices=['O0','O1'],default='O0')
    ...
    inputs={name:bound(getattr(args,name)) for name in ('runtime_config','regression_manifest','screen_manifest','e0_gallery','e1_gallery')}
    ...
    admission=dict(schema='n2-actual-gui-admission-v1',scope=SCOPE,source=str(source),common_source=str(common),output=str(output),
        ...
        full_panel=len(jobs)==6,job_count=len(jobs),cpu=args.cpu,threads=1,saved_audio_only=True,gain=1.,
