...
void servicer_task(void *args) {
    ...
    if(servicer->start_io == (int32_t)1)
    {
                            ...
                            size_t payload_len)
{
    ...
    *cmd_info = get_cmd_info(CONTROL_CMD_CLEAR_READ(cmd), res_info);
    ...
    if(((*cmd_info)->cmd_id != AEC_RESID_SPECIAL_CMD_AEC_FILTER_COEFFS)
        && ((*cmd_info)->cmd_id != PP_RESID_SPECIAL_CMD_PP_NLMODEL))
    {
...
#endif
        case AEC_SERVICER_RESID:
            aec_servicer_write_cmd(res_info, cmd, payload, payload_len);
            break;
        case AUDIO_SERVICER_RESID:
            break;
        case APPLICATION_SERVICER_RESID:
            return application_servicer_write_cmd(res_info, cmd, payload, payload_len);
        case USB_BUFFER_SERVICER_RESID:
            return usb_buffer_servicer_write_cmd(cmd, payload, payload_len);
    }
...
#endif
        case AEC_SERVICER_RESID:
            aec_servicer_read_cmd(res_info, cmd, payload, payload_len);
            break;
        case PLL_SERVICER_RESID:
            ret = pll_servicer_read_cmd(cmd, payload, payload_len);
            break;
        case APPLICATION_SERVICER_RESID:
            ret = application_servicer_read_cmd(res_info, cmd, payload, payload_len);
            break;
        case USB_BUFFER_SERVICER_RESID:
            ret = usb_buffer_servicer_read_cmd(cmd, payload, payload_len);
            break;
    }
    return ret;
}
