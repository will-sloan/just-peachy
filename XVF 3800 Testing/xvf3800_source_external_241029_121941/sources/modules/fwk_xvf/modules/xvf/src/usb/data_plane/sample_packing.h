...
static inline void unpack_buff_to_samples(uint8_t input[], const unsigned n_samples, const unsigned slot_size, int32_t output[], uint32_t vol_mul_h2d[n_samples]){
  switch(slot_size){
    case 4:
      for (int i = 0; i < n_samples; i++){
        unsigned base = i * 4;
        output[i] = ((uint32_t)input[base + 3] << 24) | ((uint32_t)input[base + 2] << 16) | ((uint32_t)input[base + 1] << 8) | (uint32_t)input[base + 0];
        output[i] = volume_scale(vol_mul_h2d[i % appconfUSB_CHANNELS_OUT], output[i]);
      }
    break;
    case 3:
      for (int i = 0; i < n_samples; i++){
...
static inline void pack_samples_to_buff(int32_t input[], const unsigned n_samples, const unsigned slot_size, uint8_t output[], uint32_t vol_mul_d2h[n_samples]){
  switch(slot_size){
    case 4:
      for (int i = 0; i < n_samples; i++){
        unsigned base = i * 4;
        int32_t sample = volume_scale(vol_mul_d2h[i % appconfUSB_CHANNELS_IN], input[i]);
        unsigned in_word = (unsigned)sample;
