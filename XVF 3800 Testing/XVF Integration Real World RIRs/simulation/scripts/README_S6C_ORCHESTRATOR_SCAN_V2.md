...
`s6c_orchestrator_scan_v2.py` is a separately bound orchestration layer around
...
& $s6cPython s6c_orchestrator_scan_v2.py test --name v2
& $s6cPython s6c_orchestrator_scan_v2.py prepare --jobs "$s6cReport\jobs\epoch2\recipes_v1.json" --workers 4 --name recipes_scan_v2
# Only after independent review, graceful old-pool closure and root admission:
& $s6cPython s6c_orchestrator_scan_v2.py run --admission "$s6cReport\orchestration\recipes_scan_v2\ADMISSION.json"
```
...
"C:\Users\amiri\Documents\GitHub\just-peachy\.edge-speech-env\python.exe" s6c_orchestrator_scan_v2.py test --name v2
"C:\Users\amiri\Documents\GitHub\just-peachy\.edge-speech-env\python.exe" s6c_orchestrator_scan_v2.py prepare --jobs "%S6C_REPORT%\jobs\epoch2\recipes_v1.json" --workers 4 --name recipes_scan_v2
REM Only after reviewed root launch admission:
"C:\Users\amiri\Documents\GitHub\just-peachy\.edge-speech-env\python.exe" s6c_orchestrator_scan_v2.py run --admission "%S6C_REPORT%\orchestration\recipes_scan_v2\ADMISSION.json"
```
...
V2 preserves the unexecuted V1 source, tests and admission. Before importing any frozen module it sets JP_S6C_SIM to the canonical current simulation root, rejects a conflicting preexisting value, and verifies frozen common/execution SIM, REPORT and PAYLOAD paths. The fresh-child fixture verifies inherited workspace paths as well as original worker/scanner resolution. No redirect to a snapshot-relative workspace is allowed.
