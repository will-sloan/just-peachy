...
def execution_sources_from_freeze(review,freeze_binding,expected_sha256,required_members):
    A.require(len(expected_sha256)==64 and freeze_binding['sha256']==expected_sha256 and review.get('owner_execution_source_freeze')==freeze_binding,'Root must bind the exact supplied V9 execution source freeze/hash')
    ...
    A.require(len(sources)==len(dependencies)==24 and all(b in sources for b in required_members),'Exact24 collision-free canonical execution dependencies including owner/bridge/policy required')
    return sources
...
def main():
    ...
    ap.add_argument('--output',type=Path,default=A.R/'runner/bank_queue_v4')
    ...
    A.require(review['status']=='ROOT_ACCEPTED_TELEMETRY_RECOVERY_V2_SOURCES_V4','Root V9 source review required')
    ...
    A.require(Path(owner['path']).name=='s6d_capture_owner_v9.py' and Path(bridge['path']).name=='s6d_capture_supervisor_bridge_v2.py','Exact V9/bridgeV2 epoch')
    ...
    helper_binding=A.bind(__file__);readme_binding=A.bind(Path(__file__).with_name('README_S6D_BANK_REMAINING_ADMIT_V4.md'))
    ...
    olddir=A.R/'runner/bank_queue_v3';oldref=A.bind(olddir/'ROOT_QUEUE_REVIEW.json')
    A.require(oldref['sha256']=='1e9db5d482ceff2d6e15a55eff68a81434ad0d823bbcb4f269e49085b28281ff','Exact interrupted bank admission')
    ...
    A.require(len(passes)==52 and sum(x['status']=='PASS' for x in passes)==51 and sum(x['status']=='FAIL' for x in passes)==1,'Exact52 history')
    failed=[x for x in passes if x['status']=='FAIL'][0]
    A.require(failed['attempt_id']=='P_MAIN6_S45_01_17','Expected preserved telemetry failure')
    ...
    spec=importlib.util.spec_from_file_location('reviewed_v9_closure',owner['path'])
    ...
    partial=A.bind(A.R/'partial_bank_review_v1/MAIN_FIRST16_TRANSPORT.json')
    A.require(partial['sha256']=='b52a327fe534061bf9415ced4c5d3a68ee7f5971760a216b8812081c576fa50d','Exact16 transport review')
    partial_data=A.read(partial['path'])
    A.require({x['attempt_id'] for x in partial_data['attempts']}==DONE,'Exact16 reviewed PASS credits')
    ...
    for group in old['groups']:
        ...
        auth.update(plan_sha256=pb['sha256'],source_bindings=sources,reviewed_recoveries=recoveries,
            ...
            partial_pass_transport_review=partial,scope='Retain16 verified PASS captures; fresh pre-QA_R3, failed17_R2, every other pending row. Original failures/charges preserved. No new waveform/gain/guard/profile/timing policy.')
    ...
    A.require(len(all_rows)==378 and len({x['attempt_id'] for x in all_rows})==378,'Exact378 unique future attempts')
    total=sum((Fraction(str(x['charged_playback_s'])) for x in passes),Fraction())+A.charge(all_rows)
    A.require(total==Fraction('19744.058') and len(passes)+len(all_rows)==430,'Exact430/19744.058 forecast')
    previous=None
    for job in queue['jobs']:
        oldid=job['job_id'];A.require(oldid.startswith('bank_v2_'),'Expected old job epoch')
        job['job_id']='bank_v3_'+oldid[len('bank_v2_'):]
        ...
        sources+=execution_sources+[sr,rb,ef,partial,helper_binding,readme_binding]
    ...
    receipt=dict(status='ROOT_ADMITTED_REMAINING_BANK_AFTER_TELEMETRY_RECOVERY_PENDING_LAUNCH',utc=dt.datetime.now(dt.timezone.utc).isoformat(),
        ...
        retained_pass_transport_review=partial,retained_pass_attempt_ids=sorted(DONE),current_charged_attempts=52,new_attempts_admitted=378,
        total_forecast_attempts=430,total_forecast_charged_seconds=float(total),original_failures_preserved=True,playback_started=False)
    A.save(out/'ROOT_QUEUE_REVIEW.json',receipt)
    print(json.dumps(dict(status=receipt['status'],receipt=A.bind(out/'ROOT_QUEUE_REVIEW.json'),attempts=378,stages=len(queue['jobs']))))
