"""Prepare conditional full-bank native data only; see README_S6D_NATIVE_REMAINING_PREPARE_V2.md."""
...
def prepare(output,payload):
    ...
    criteria=[('F03','short',lambda s:any(x.get('kind')=='utterance' and 0<(x['source_stop_sample']-x['source_start_sample'])/16000<=2 for x in s['segments'])),
        ...
        ('F06','long_return_100s',lambda s:s['duration_s']>=100),
    ...
    projection=dict(status='ARITHMETIC_FORECAST_NOT_MEASURED_FOUR_WORKER_PERFORMANCE',pilot_payload_authority=forecast_b,pilot_runtime_memory_authority=findings_b,
        ...
        new_source_seconds=total_sec,serial_wall_hours_source_length_scaled=[total_sec/44.6954375*77/3600,total_sec/44.6954375*83/3600],ideal_four_way_hours_source_length_scaled=[total_sec/44.6954375*77/14400,total_sec/44.6954375*83/14400],
        first16_serial_equivalent_minutes=[sum(j['audio_duration_sec'] for js in worker_jobs.values() for j in js[:4])/44.6954375*v/60 for v in (77,83)],first16_ideal_four_way_minutes=[sum(j['audio_duration_sec'] for js in worker_jobs.values() for j in js[:4])/44.6954375*v/240 for v in (77,83)],
    ...
    result=dict(schema='s6d-native-remaining-preparation.v1',status='CONDITIONAL_DATA_ONLY_PREPARED_NOT_ADMITTED',source=bind(__file__),source_readme=bind(Path(__file__).with_name('README_S6D_NATIVE_REMAINING_PREPARE_V2.md')),
        ...
        no_production_authorization_files_created=True,source_protocol_unchanged=True,metadata_payload_root=str(payload),native_session_protocol_state_directories_created=False,metadata_directories_created=True)
