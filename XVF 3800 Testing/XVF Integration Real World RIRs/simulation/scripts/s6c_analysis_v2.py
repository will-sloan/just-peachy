"""S6C separate-word/anonymous/route scoring. See README_S6C_ANALYSIS.md."""
...
SCHEMA="jp_s6c_core_analysis.v2"
CODES=("s6c_analysis_v2.py","s6c_analysis.py","s6b_analysis.py","s6a_text_metrics.py","s6a_cue_score.py","s6a_support_metrics.py","s6a_baseline_results.py","s4_h2_analysis.py","s5_statistics.py")
...
def registered_candidates():
 paths=[REPORT/"design"/name for name in ("REGISTERED_DESIGN_V1.json","C_ONLY_CALIBRATION_AMENDMENT_V1.json","ADMISSION_AND_FAMILY_RESCUE_V1.json")]
 bindings=[bind(p) for p in paths];docs=[verified(b) for b in bindings]
 for doc in docs[1:]:
  if doc["parent_design"]!=bindings[0]:raise ValueError("Amendment parent design is not exact original")
  verified(doc["parent_design"])
 rows=[r for doc in docs for r in doc["candidates"]]
 ids=[r["candidate_id"] for r in rows]
 if len(ids)!=len(set(ids)) or len(ids)!=184:raise ValueError("Exact 184 distinct registered labels required; settings aliases remain separate")
 if any(r.get("parent") is not None and r["parent"] not in ids for r in rows):raise ValueError("Unknown declared comparison parent")
...
def run(args):
 ...
 for item in selected:
  ...
  try:
   ...
   identity=dict(prediction=item["result"],support=sb,bank=bank_binding,codes=codes,packages=packages,metric_schema=SCHEMA,asr_tap=tap,identity_tap=itap)
 ...
 registry_path=REPORT/"design/REGISTERED_DESIGN_V1.json";registry,registry_bindings=registered_candidates()
...
def main():
 ...
 p.add_argument("--index",type=Path);p.add_argument("--output-subdir",default="core_analysis_v2")
