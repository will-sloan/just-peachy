...
RATE = 16000
POLICY = {
    'schema': 'jp_s6a_support_policy_v1',
...
from s6a_common import AllBankGuard
DevelopmentGuard = AllBankGuard
...
def freeze_scene_support(scene, capture_result, audio_metrics, noise_catalog, guard):
    ...
    for index, seg in enumerate(scene['segments']):
        ...
        if seg['kind'] == 'utterance':
            ...
            if source.get('split') not in {'development', 'reserve'}: raise PermissionError('Unknown noise partition')
    ...
    result = {'schema': 'jp_s6a_reference_support_v1', 'policy_sha256': fingerprint(POLICY), 'case_id': scene['case_id'],
...
def score_output(scene, support, output, events, pcm16, raw_pcm24, guard):
    ...
    result={'schema':'jp_s6a_output_support_metrics_v1','case_id':scene['case_id'],'output':output,'support_sha256':support['support_sha256'],
...
def shared_direction_metrics(scene, support, capture_result, callback_metadata, telemetry_rows, guard):
    ...
    return {'schema':'jp_s6a_shared_noise_direction_v1','case_id':scene['case_id'],'support_sha256':support['support_sha256'],
