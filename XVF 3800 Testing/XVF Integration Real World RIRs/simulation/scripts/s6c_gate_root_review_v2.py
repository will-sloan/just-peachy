"""Independent actual epoch4 gate admission review. See README_S6C_GATE_ROOT_REVIEW_V2.md."""
...
def main():
 ...
 for pair,mode in zip(pairs,modes):
  for tap in ('O0','O1'):
   ...
   assert aa['xvf']['mode']=='none' and bb['xvf']['mode']=='tracking_only'
   assert aa['tracker']['cues_enabled'] is False and bb['tracker']['cues_enabled'] is True
   for v in (aa,bb):
    ...
    v.pop('profile_id');v['tracker'].pop('cues_enabled');v['xvf'].pop('mode')
   assert aa==bb,(pair,tap,'not exact cue flag and routing companion')
   checks+=1
 ...
 out=REPORT/'independent_review/GATE192_ROOT_REVIEW_V2.json'
 value=dict(status='PASS_FOR_NATIVE_ADMISSION_NOT_EXECUTED',created_utc=__import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat(),checks=checks,
  jobs=mb,admission=ab,epoch=sb,panel=manifest['panel_proposal'],source=bind(__file__),readme=bind(Path(__file__).with_name('README_S6C_GATE_ROOT_REVIEW_V2.md')),
