...
import argparse,ast,hashlib,importlib,json,math,tempfile
from copy import deepcopy
from pathlib import Path
import numpy as np

HERE=Path(__file__).resolve().parent
EXPECTED='982e1c7b2bf38fdc7a92a023cff0e73e2cbccf2072233924e6eaa5e84657a712'
...
def run(output):
    ...
    with tempfile.TemporaryDirectory(prefix='s6c_hist_independent_') as td:
        ...
        negatives=[]
        def reject(name,ee,ss):
            try:a.research_prediction(ee,ss,job,api,classes)
            except ValueError:negatives.append(name)
            else:raise AssertionError('Changed semantic field admitted: '+name)
        for field,value in [('text','different words'),('source_start_sec',.01),('available_at_sec',.63),('first_display_label','Wrong speaker')]:
            altered=deepcopy(events);target=next(e for e in altered if e['event_type']=='transcript_partial');target['payload'][field]=value
            reject(field,altered,summary)
        altered=deepcopy(events);next(e for e in altered if e['event_type']=='speaker_decision')['payload']['decision']['anonymous_label']='Wrong speaker';reject('anonymous decision',altered,summary)
        altered=deepcopy(summary);altered['telemetry']['scheduler']['utterances'][0]['first_final_label']='Wrong speaker';reject('final retained label',events,altered)
        prior=HERE.parent/'staging/s6c/20260910T123540Z/historical_paced_analysis/pre_release_diagnostics_v1/s6c_historical_paced_analysis_v1.py'
        assert a.binding(prior)['sha256']=='7230d48b94ecdea4ab592f0c783b53d779a2f1b4b1cd3abadd06226ec5dc59f9'
        def ast_without_exclusions(path):
            tree=ast.parse(path.read_bytes());tree.body=[n for n in tree.body if not (isinstance(n,ast.Assign) and any(isinstance(t,ast.Name) and t.id=='RESEARCH_CLOCK_EXCLUSIONS' for t in n.targets))]
            return ast.dump(tree,include_attributes=False)
        assert ast_without_exclusions(source)==ast_without_exclusions(prior)
        assert a.RESEARCH_CLOCK_EXCLUSIONS=={'policy_compute_sec','modeled_available_at_sec','compute_finished_elapsed_sec','release_watermark_lower_bound_sec','release_after_all_lanes_closed'}
        result=dict(status='PASS_NARROW_REPAIR',source=a.binding(source),readme=a.binding(HERE/'README_S6C_HISTORICAL_PACED_ANALYSIS_V1.md'),review_source=a.binding(__file__),
            review_readme=a.binding(HERE/'README_TEST_S6C_HISTORICAL_PACED_COMPONENT_V2.md'),manifest=pb,code_sources=code.sources,metadata_sources=[x['source'] for x in reader.sources],
            ...
            exact_decisions=True,exact_final_utterances=True,negative_semantic_checks=negatives,other_executable_AST_unchanged=True,prior_source=a.binding(prior),model_calls=0,actual_native_cells_read=0,pcm_reads=0,
            finding='Only native release diagnostics are excluded additionally; synthetic original native/replay decisions and final utterances remain exact. No actual paced-data or accuracy claim.')
