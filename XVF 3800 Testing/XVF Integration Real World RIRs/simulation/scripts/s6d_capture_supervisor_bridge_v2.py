"""Same-process capture V5 bridge with explicit restoration policy; README_S6D_CAPTURE_V5.md."""
...
import uuid
import psutil
from s6d_restoration_policy_v1 import record_policy_valid,POLICY,NO_MUTATION_POLICY,SCHEMA
...
def restoration_proof(batch_dir,ledger,identity):
    ...
    no_mutation=value.get('schema_version')==SCHEMA and value.get('policy')==NO_MUTATION_POLICY and value.get('scope')=='No setters or playback occurred under this owner' and not any(r.get('batch')==Path(batch_dir).name for r in ledger.get('passes',[]))
    policy_valid=False
    if value.get('schema_version')==SCHEMA and value.get('policy')==POLICY:
        initial=value['initial_state'];binding(initial['path'],initial['sha256'])
        if Path(initial['path']).resolve()!=(Path(batch_dir)/'initial_state.json').resolve():raise ValueError('V5 policy initial-state path differs')
        policy_valid=record_policy_valid(load(initial['path']),value)
    checks=dict(owner_same_process=True,restoration_policy_valid=policy_valid or no_mutation,
    ...
    if no_mutation:
        checks['no_mutation_no_gain_set_claim']=True
    else:
        checks.update(exact_static_configuration_match=value.get('exact_static_configuration_match') is True,
            requested_gain_set_verified=value.get('requested_gain_set_verified') is True,
            packed_input_disabled=value.get('packed_input_disabled') is True and value.get('readback',{}).get('identity',{}).get('I2S_INPUT_PACKED')==[0])
    ...
    return dict(owner=binding(acquired),restoration=binding(restored),checks=checks,scope=value.get('scope'),
        restoration_policy=value.get('policy'),exact_full_snapshot_match=value.get('exact_full_snapshot_match'),
        autonomous_gain=value.get('autonomous_gain'),no_mutation_scope=no_mutation)
