"""S5 development-only text metrics. No models, audio reads, hardware, or tuning."""
...
def require_development(scene, development_ids):
    """Call before accessing task paths; metadata inventory alone is permitted."""
    cid = scene.get('case_id')
    if cid not in frozenset(development_ids) or scene.get('split') != 'development' or scene.get('task_scoring_allowed') is not True:
        raise PermissionError(f'S5 refuses task scoring outside the development allowlist: {cid}')
    return cid
...
def reference_layout(scene):
    ...
    for i, segment in enumerate(utterances):
        ...
        if segment.get('source_split', 'development') != 'development':
            raise PermissionError('Non-development source in development task scene')
...
def _rate(result, hypothesis_words, metric, scope):
    ...
    output = {'status': 'SCORED_DIAGNOSTIC', 'metric': metric, 'implementation': 'meeteval',
              ...
              'word_counts': counts, 'assignment': row.get('assignment'), 'scope': scope}
    for key in ('missed_speaker', 'falarm_speaker', 'scored_speaker'):
        if key in row: output[key] = int(row[key])
    return output


def score_scene(scene, native_metrics=None, events=None, *, development_ids, duration_s=None, include_multitalker=True):
    ...
    out = {'schema': SCHEMA, 'case_id': cid, 'population': layout['population'],
           ...
           'final_utterances': len(finals), 'reserve_task_scored': False,
...
def score_scene_files(scene, *, development_ids, metrics_path=None, events_path=None, **kwargs):
    """Guard before opening either task input. Caller binds input hashes/receipt identity."""
    require_development(scene, development_ids)
    ...
    return score_scene(scene, metrics, events, development_ids=development_ids, **kwargs)


def regression_s45(output):
    """Reproduce only previously inspected 24-scene raw counts, not new S5 comparisons."""
    bank = load_json(BANK)
    scenes = {s['case_id']: s for s in bank['scenes']}
    ids = bank['sentinel_scene_ids']
    assert len(ids) == 24 and len(set(ids)) == 24
    allowed = {s['case_id'] for s in scenes.values() if s['split'] == 'development'}
    rows, consumed, totals = [], [binding(BANK), binding(__file__)], {'O0': Counter(), 'O1': Counter()}
    for cid in ids:
        require_development(scenes[cid], allowed)
        for stream in ('O0', 'O1'):
            folder = S45 / 'h2' / cid / stream
            ...
            mb = binding(metric_path)
            assert mb['sha256'] == receipt['metrics_binding']['sha256']
            native = load_json(metric_path)
            got = score_scene(scenes[cid], native, development_ids=allowed, include_multitalker=False)
            fields = ('status', 'wer', 'cer', 'word_counts', 'character_counts', 'empty_reference_insertions', 'empty_reference_words_per_minute')
            assert all(got['text'].get(k) == native['text'].get(k) for k in fields), (cid, stream, got['text'], native['text'])
            rows.append({'case_id': cid, 'stream': stream, 'status': 'EXACT_HISTORICAL_RAW_COUNTS', 'population': got['population']})
            if got['text']['status'] == 'SCORED':
                totals[stream].update(got['text']['word_counts'])
                totals[stream].update({'char_' + k: v for k, v in got['text']['character_counts'].items()})
            consumed.extend((binding(folder / 'run_receipt.json'), mb))
    result = {'schema': 'jp_s5_s45_text_regression_v1', 'status': 'PASS_EXACT_48_HISTORICAL_TEXT_RESULTS',
              'scenes': 24, 'outputs': len(rows), 'reserve_task_reads': 0, 'new_multitalker_scores_calculated': 0,
              'rows': rows, 'primary_raw_counts': {k: dict(v) for k, v in totals.items()}, 'consumed_bindings': consumed}
...
def main():
    ...
    parser.add_argument('--regression-s45', action='store_true')
    parser.add_argument('--output', type=Path)
    args = parser.parse_args()
    if not args.regression_s45 or args.output is None:
        parser.error('Use --regression-s45 --output PATH; full S5 scoring is orchestrated by the frozen main scorer')
    result = regression_s45(args.output)
    print(json.dumps({k: result[k] for k in ('status', 'scenes', 'outputs', 'primary_raw_counts')}, indent=2))
