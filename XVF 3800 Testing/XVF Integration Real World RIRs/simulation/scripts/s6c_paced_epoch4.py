"""Canonical paired paced adapter; see README_S6C_PACED_EPOCH4.md."""
...
_helper=Path(__file__).with_name('s6c_long_native_epoch4.py')
if hashlib.sha256(_helper.read_bytes()).hexdigest()!='9c15795253b17722af6b56e8bc3eb524ce68665bfa2d5de2d95ef940f690ea9b':raise ValueError('Reviewed long admission utilities changed')
...
def source_bindings():
    return [L.bind(Path(__file__).with_name(n)) for n in ('s6c_paced_epoch4.py','README_S6C_PACED_EPOCH4.md','s6c_long_native_epoch4.py','README_S6C_LONG_NATIVE_EPOCH4.md','s6c_long_session.py','README_S6C_LONG_SESSION.md','s6c_native_prefix.py')]
...
def namespace_roots(namespace):
    if not re.fullmatch(r'[A-Za-z0-9_-]{1,60}',namespace):raise ValueError('Simple fresh paced namespace required')
...
def prepare(args):
    ...
    plan=dict(schema=SCHEMA,status='PREPARED_NO_MODELS_STARTED',created_utc=L.utc(),namespace=args.namespace,execution_manifest=eb,input_index=ib,panel=pb,panel_mode=args.panel_mode,candidates=candidates,
            ...
            with (folder/'WORKER_LOG.txt').open('x',encoding='utf-8') as log,(folder/'PROCESS_TREE_SAMPLES.jsonl').open('x',encoding='utf-8') as trajectory:
                ...
                while child.poll() is None:
                    ...
                    if elapsed-last_full>=20:
                        quiet(owned);resources=common.admit_work(full=True);L.check_headroom(resources);last_full=elapsed
...
def source_checks():
    ...
    return common.save(REPORT/'paced_candidates/SOURCE_CHECKS_V3.json',result,immutable=True)
