"""Pinned additive-epoch coordinator overlay. README_S6C_ORCHESTRATOR_SCAN_V5.md."""
...
import hashlib
_reviewed_v4=Path(__file__).with_name('s6c_orchestrator_scan_v4.py')
if hashlib.sha256(_reviewed_v4.read_bytes()).hexdigest()!='552f1a9ca6f8d89a932cb7d971e506f4e48b00191ddca4e0f612739e72596be3':
    raise ValueError('Reviewed V4 source changed')
...
REPORT=prior.REPORT
EDGE=prior.EDGE
SCHEMA='jp_s6c_coordinator_scan_overlay.v5'
TRUSTED_EPOCHS={
    ...
    'epoch6':'e7c6e91ed9ceaecc587789d96e24ca5ab0f458711a1e604adff71856f3fad934'}
...
def load_native(epoch='epoch6'):
    ...
    reference5_binding=binding(REPORT/'EPOCH5_EXECUTION_MANIFEST.json')
    if reference5_binding['sha256']!=TRUSTED_EPOCHS['epoch5']:raise ValueError('Pinned epoch5 reference differs')
    reference5=verified(reference5_binding);compatible_native(spec,reference5)
    for item in reference5['execution_files']:check_file(item)
...
def source_rows():
    names=('s6c_orchestrator_scan_v5.py','README_S6C_ORCHESTRATOR_SCAN_V5.md','s6c_orchestrator_scan_v4.py','README_S6C_ORCHESTRATOR_SCAN_V4.md','s6c_orchestrator_scan_v3.py','README_S6C_ORCHESTRATOR_SCAN_V3.md','s6c_orchestrator_scan_v2.py',
...
def run(args):
    ...
    if admission['schema']!=SCHEMA or admission['status']!='PREPARED_FOR_REVIEW':raise ValueError('Prepared V5 admission required')
...
def main():
    ...
    p.add_argument('--epoch',choices=tuple(TRUSTED_EPOCHS),default='epoch6');p.add_argument('--jobs',type=Path);p.add_argument('--workers',type=int,default=4)
    p.add_argument('--name',default='scan_v5');p.add_argument('--admission',type=Path);args=p.parse_args()
