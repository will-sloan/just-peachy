"""Pinned additive-epoch coordinator overlay. README_S6C_ORCHESTRATOR_SCAN_V3.md."""
...
REPORT=prior.REPORT
EDGE=prior.EDGE
SCHEMA='jp_s6c_coordinator_scan_overlay.v3'
TRUSTED_EPOCHS={
...
def source_rows():
    names=('s6c_orchestrator_scan_v3.py','README_S6C_ORCHESTRATOR_SCAN_V3.md','s6c_orchestrator_scan_v2.py',
...
def main():
    ...
    p.add_argument('--epoch',choices=tuple(TRUSTED_EPOCHS),default='epoch4');p.add_argument('--jobs',type=Path);p.add_argument('--workers',type=int,default=4)
    p.add_argument('--name',default='scan_v3');p.add_argument('--admission',type=Path);args=p.parse_args()
