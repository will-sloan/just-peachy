"""Single-owner finite S4.5 capture; README_S45.md and README_S45_HARDWARE_GATES.md."""
...
from s45_transport import prepare, payload_check, carrier_chunk
...
def capture(seconds,payload_counts):
    ...
    with sd.RawStream(samplerate=48000,channels=2,dtype='int24',device=(inp['index'],out['index']),blocksize=0,
        ...
        while not done.wait(.1):
            if (REPORT/'STOP_REQUEST.json').exists():errors.append('Coordinated watchdog stop requested');stream.abort();break
...
def code_bindings():
    paths=[SIM/'scripts'/n for n in ['s45_hardware.py','s45_transport.py','s45_common.py','s4_telemetry.py','s4_common.py','s4_restore.py','s3_hardware.py','s4_h2_run.py']]
    paths+=sorted((SIM/'scripts/s4_native').glob('*'))
    return [bind(p) for p in paths if p.is_file()]

def verified_restoration(path):
    from s4_h2_run import verified_restoration as exact_restoration
    return exact_restoration(path)

def manifest_path_for_cases(case_ids):
    assert case_ids and len(case_ids)==len(set(case_ids)), 'Require unique case IDs'
    if case_ids==['transport_regression']:
        return SIM/'reports/S3/20260908T214914Z/inputs_manifest.json'
    reference=[cid.startswith('S45_REF_') for cid in case_ids]
    assert all(reference) or not any(reference), 'Mixed canonical/reference batch is prohibited'
    assert 'transport_regression' not in case_ids, 'Tagged regression must run alone'
    return BANK/('REFERENCE_SCENE_MANIFEST.json' if all(reference) else 'SCENE_MANIFEST.json')

def verify_prior_restorations():
    """Missing receipt is unsafe for any batch known to have acquired hardware."""
    ledger=read(REPORT/'physical_ledger.json') if (REPORT/'physical_ledger.json').exists() else {'passes':[]}
    required={r['batch'] for r in ledger['passes']}
    for name in ['initial_state.json','owner_acquired.json']:
        required.update(p.parent.name for p in HARDWARE.glob('*/'+name))
    required.update(p.parent.name for p in HARDWARE.glob('*/restoration.json') if read(p).get('status')!='NOT_NEEDED')
    verified=[]
    for batch in sorted(required):
        path=HARDWARE/batch/'restoration.json'
        assert path.exists(), 'Missing restoration for acquired/charged batch: '+batch
        verified.append(verified_restoration(path))
        assert not any(r['batch']==batch and r.get('status')=='STARTED' for r in ledger['passes']), 'Unresolved physical pass after restoration: '+batch
    return verified

def check_global_attempts(case_ids,scenes,current_batch):
    """Call under the hardware lease, before mutation or playback."""
    ledger=read(REPORT/'physical_ledger.json') if (REPORT/'physical_ledger.json').exists() else {'passes':[]}
    receipts=[(p,read(p)) for p in HARDWARE.glob('*/*/case_result.json')]
    for cid in case_ids:
        if cid=='transport_regression':continue
        prior=[(p,r) for p,r in receipts if r['case_id']==cid]
        for p,r in prior:
            assert r['input_scene_sha256']==scenes[cid]['canonical_audio']['sha256'], 'Canonical/reference identity changed across attempts'
            assert r.get('status')!='PASS', 'Already accepted case cannot be physically repeated in a new batch: '+cid
        attempts={(r['batch'],cid) for _,r in prior}
        attempts.update((r['batch'],cid) for r in ledger['passes'] if r['case_id']==cid)
        assert (current_batch,cid) not in attempts, 'An existing attempt must be preserved; use a distinct batch'
        assert len(attempts)<LIMITS['attempts_per_scene'], 'Global per-scene attempt limit reached: '+cid

def close_started_pass(batch,case_id,error,meta=None):
    """Close only our unfinished ledger row; never reduce its charged duration."""
    path=REPORT/'physical_ledger.json'
    if not path.exists():return None
    ledger=read(path)
    rows=[r for r in ledger['passes'] if r['batch']==batch and r['case_id']==case_id and r.get('status')=='STARTED']
    assert len(rows)<=1, 'Duplicate active physical ledger rows'
    if not rows:return None
    row=rows[0];row.update(status='FAIL',ended_utc=now(),error=str(error),closure='Owned exception cleanup; full conservative playback charge retained')
    if meta is not None:row['captured_frames']=meta.get('captured_frames')
    save(path,ledger);return row

def check_physical_budget(ledger,seconds):
    assert 0<seconds<=LIMITS['active_playback_s'], 'Invalid finite playback duration'
    charged=sum(r['charged_playback_s'] for r in ledger['passes'])
    assert len(ledger['passes'])<LIMITS['physical_passes'] and charged+seconds<=LIMITS['active_playback_s'], 'Physical budget reached'
...
def run(batch,case_ids,recipe,final=False):
    ...
    manifest_path=manifest_path_for_cases(case_ids)
    manifest=read(manifest_path)
    if case_ids!=['transport_regression']:assert manifest['validation']['status']=='PASS'
    scenes={s['case_id']:s for s in manifest.get('scenes',[])}
    if case_ids==['transport_regression']:
    ...
    out=HARDWARE/batch;out.mkdir(parents=True,exist_ok=True)
    ...
    signature={'code_key':code_key,'recipe':recipe,'final_recipe_capture':final,'scene_manifest_sha256':bind(manifest_path)['sha256']}
    ...
    assert not (out/'initial_state.json').exists() and not (out/'owner_acquired.json').exists(), 'Closed/acquired batch is immutable; pending cases require a distinct batch'
    ...
    restoration={'status':'NOT_NEEDED'};active_ledger_case=None;meta=None
    with Progress('hardware_'+batch,len(pending)) as progress:
        try:
            ...
            # All global decisions occur while the single hardware lease is held.
            verify_prior_restorations();check_global_attempts(pending,scenes,batch)
            for prior in HARDWARE.glob('*/*/case_result.json'):
                r=read(prior)
            ...
            save(out/'owner_acquired.json',{'batch':batch,'acquired_utc':now(),'pid':os.getpid(),'code_key':code_key,'hardware_lease_acquired':True})
            ...
            for cid in pending:
                check_storage();scene=scenes[cid]
                if not launch_allowed(scene['duration_s']+90) or (REPORT/'STOP_REQUEST.json').exists():raise RuntimeError('Deadline or watchdog stop: preserve pending cases')
                folder=out/cid;folder.mkdir()
                ledger=read(REPORT/'physical_ledger.json') if (REPORT/'physical_ledger.json').exists() else {'passes':[]}
                check_physical_budget(ledger,scene['duration_s'])
                ...
                result={'case_id':cid,'batch':batch,'recipe':recipe,'status':'STARTING','duration_s':scene['duration_s'],
                    'code_key':code_key,'input_scene_sha256':scene['canonical_audio']['sha256'],'packed_input':fmt,'final_recipe_capture':final,'split':scene.get('split','diagnostic'),'reserve_task_scored':False}
            ...
            if active_ledger_case is not None:
                try:close_started_pass(batch,active_ledger_case,repr(e),meta)
                except BaseException as ledger_error:summary['ledger_cleanup_error']=repr(ledger_error)
            if result and result['status']=='STARTING':result.update(status='FAIL',error=repr(e));save(folder/'case_result.json',result)
        finally:
            progress.detail='Close handles and restore exact starting exposed state'
            try:progress.emit()
            except BaseException as progress_error:summary['cleanup_progress_error']=repr(progress_error)
            if telem is not None and telem_result is None:
                try:telem.stop('S4_final_cleanup');telem_result=telem.wait(20)
                except BaseException as telemetry_error:summary['telemetry_cleanup_error']=repr(telemetry_error)
            ...
            lease_released=not owned
            try:
                if owned:lock.seek(0);msvcrt.locking(lock.fileno(),msvcrt.LK_UNLCK,1)
                if lock:lock.close()
                lease_released=True
            except BaseException as lease_error:
                restoration.update(status='FAIL',lease_release_error=repr(lease_error))
                try:
                    if lock:lock.close()
                except BaseException:pass
            restoration['hardware_lease_released']=lease_released;save(out/'restoration.json',restoration)
