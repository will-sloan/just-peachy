"""Isolated epoch4 long-native admission; README_S6C_LONG_NATIVE_EPOCH4_FAST_V1.md."""
...
def write_observer_record(function,args,result,records,error):
    ...
    folder=REPORT/'observer_fast_v1/attempts';folder.mkdir(parents=True,exist_ok=True)
...
def output_roots(namespace):
    if not re.fullmatch(r'epoch4_[A-Za-z0-9_-]{1,64}',namespace) or not namespace.endswith('_fast_v1'):raise ValueError('New epoch4_ namespace required')
...
def worker_command(argv):
    ...
    if names & {'s6c_long_native_epoch4.py','s6c_paced_controls.py','s6c_paced_b36_v1.py','s6c_long_native_epoch4_fast_v1.py','s6c_paced_epoch4_fast_v1.py','s6c_paced_controls_fast_v1.py','s6c_paced_b36_fast_v1.py','s6c_long_b36_fast_v1.py','s6c_paced_cross_routes_fast_v1.py','s6c_paced_arrival_sentinel_fast_v1.py'}:return any(v in words for v in ('run','worker'))
...
def prepare(args):
    ...
    deps.extend([bind(__file__),bind(Path(__file__).with_name('README_S6C_LONG_NATIVE_EPOCH4_FAST_V1.md'))])
    plan=dict(schema='s6c-epoch4-long-native-admission.v1',status='PREPARED_NO_MODELS_STARTED',created_utc=utc(),namespace=args.namespace,observer_policy=observer_policy(),
...
def checks():
    ...
    r,p=output_roots('epoch4_test_fast_v1');assert r.parent==REPORT/'long_session' and p.parent==PAYLOAD/'long_session';count+=1
...
def source_checks():
    ...
    receipt=dict(schema='s6c-epoch4-long-fast-observer-source-checks.v1',status='PASS_MODEL_FREE_SOURCE_ADMISSION',created_utc=utc(),pure_checks=pure,
        ...
        protected_functions=protected_identities(driver),dependencies=deps+[bind(__file__),bind(Path(__file__).with_name('README_S6C_LONG_NATIVE_EPOCH4_FAST_V1.md'))],
    ...
    return common.save(REPORT/'long_native_epoch4/observer_fast_v1/SOURCE_CHECKS_V1.json',receipt,immutable=True)
