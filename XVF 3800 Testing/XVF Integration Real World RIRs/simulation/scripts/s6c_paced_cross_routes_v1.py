"""Exact registered cross-route paced option; README_S6C_PACED_CROSS_ROUTES_V1.md."""
...
PINS={'s6c_paced_epoch4.py':'b4b0dc48190654edbdcb6259cf2b8abc08d8675d49367863ff539f7eb54df8a3',
...
C=importlib.import_module('s6c_paced_epoch4');D=importlib.import_module('s6c_long_b36_v1')
for module in (C,D):
...
def source_bindings():
    own=[L.bind(Path(__file__)),L.bind(HERE/'README_S6C_PACED_CROSS_ROUTES_V1.md'),L.bind(HERE/'s6c_long_b36_v1.py'),L.bind(HERE/'README_S6C_LONG_B36_V1.md')]
...
def compile_adapter():
    tree=ast.parse((HERE/'s6c_paced_epoch4.py').read_bytes());nodes=[];edits=[];originals={}
    class Metadata(ast.NodeTransformer):
        ...
        def visit_Compare(self,node):
            if ast.unparse(node)=="'s6c_paced_epoch4.py' in names":
                ...
                return ast.copy_location(ast.parse("bool({'s6c_paced_epoch4.py','s6c_paced_cross_routes_v1.py'} & names)",mode='eval').body,node)
