"""Additive actual cadence-floor audit; README_S6C_CADENCE_FLOOR_AUDIT_V3.md."""
...
SCHEMA='jp_s6c_cadence_floor_native_audit.v3'
...
def sources():return [A.bind(p) for p in (Path(__file__),HERE/'README_S6C_CADENCE_FLOOR_AUDIT_V3.md',HERE/'s6c_native_cadence_audit.py')]
...
def payload_paths(rec):
    require(type(rec['pid']) is int and rec['pid']>0 and type(rec['creation_time']) in (int,float) and math.isfinite(rec['creation_time']) and rec['creation_time']>0,'Exact recorded native worker identity required')
    worker=PAYLOAD/'epoch5/workers'/(str(rec['pid'])+'_'+str(round(rec['creation_time']*1000)))
    session=Path(rec['session_dir']).resolve()
    require(session.parent==(worker/'sessions').resolve() and session.name.startswith('edge_paired_wav_'),'Native session outside exact recorded worker root')
    for key,name in [('events','events.jsonl'),('summary','session_summary.json'),('finalization','session_finalization_v3.json')]:
        require(Path(rec[key]['path']).resolve()==session/name,'Native payload path differs: '+key)
    return session

def prepare(args):
    quiet();out=REPORT/'cadence_floor_audit_v3';require(not out.exists(),'Preserve prior audit namespace');cb=A.bind(args.core);require(cb['sha256']==args.core_sha256,'Exact completed core receipt required')
...
def run(args):
    quiet();pb,plan=exact(Path(args.plan).resolve(),args.sha256);out=Path(pb['path']).parent
    require(out==REPORT/'cadence_floor_audit_v3' and plan['schema']==SCHEMA and plan['status']=='PREPARED_NO_EVENT_SCAN' and plan['sources']==sources(),'Exact prepared source/namespace differs')
...
def checks():
    ...
    session=PAYLOAD/'epoch5/workers/12_1234/sessions/edge_paired_wav_synthetic'
    paths=dict(pid=12,creation_time=1.234,session_dir=str(session),events=dict(path=str(session/'events.jsonl')),summary=dict(path=str(session/'session_summary.json')),finalization=dict(path=str(session/'session_finalization_v3.json')))
    require(payload_paths(paths)==session.resolve(),'Recorded worker session root');checked.append('Frozen worker-owned session layout admitted')
    for key,value in [('pid',13),('creation_time',1.235),('session_dir',str(PAYLOAD/'epoch5/cadence_floor/job/sessions/edge_paired_wav_synthetic'))]:
        changed=deepcopy(paths);changed[key]=value;bad('Foreign worker/session '+key,lambda changed=changed:payload_paths(changed))
    changed=deepcopy(paths);changed['summary']['path']=str(session/'other.json');bad('Wrong native basename',lambda:payload_paths(changed))
