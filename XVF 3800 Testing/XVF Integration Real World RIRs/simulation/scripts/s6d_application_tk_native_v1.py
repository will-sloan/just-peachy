"""Owned native/Tk multi-view harness; see README_S6D_APPLICATION_TK_NATIVE_V1.md."""
...
class GalleryScoreSpy:
    """Observe the real isolated gallery score once, without changing its output."""
...
def execute_gui(engine,job,manifest,manifest_bound,output,sourceproof,evidence,s6d,gui,event_factory,checkpoint,process):
    ...
    origin=time.perf_counter();root=tk.Tk();consumer=(output/'consumer_events.jsonl').open('x',encoding='utf-8',buffering=1)
    ...
    class Window(gui.EdgeSpeechWindow):
        ...
        def _record_s6d_gui(self,row):
            ...
            row={**row,'view_id':self.engine.view['name'],'source_started_monotonic_sec':fanout.source_origin,
                'actual_callback_monotonic_sec':time.perf_counter(),'widget_text_sha256':hashlib.sha256(self.transcript.get('1.0','end-1c').encode()).hexdigest(),
                'clock_scope':'owned actual Tk command/callback; no physical scanout','causal_input_count':self.engine.view['input_count']}
            self._s6d_render_handle.write(json.dumps(row,allow_nan=False)+'\n')
    for i,v in enumerate(views):
        surface=root if i==0 else tk.Toplevel(root);w=Window(surface,engine.config,engine=ViewFacade(engine,v,fanout));surface.title('S6D native view '+v['name']);windows.append(w)
    def checked():
        if checkpoint:checkpoint(engine=engine,telemetry=engine.telemetry(),job=job,output=output)
    def startup():
        try:checked();engine.start_file(Path(job['audio']['path']),realtime=True);checked()
    ...
    try:watcher.start();starter.start();root.after(0,pump);root.mainloop()
    finally:
        if not fanout.terminal:engine.stop()
        starter.join(3);observer_stop.set();watcher.join(3);consumer.close()
        for w in windows:
            if w._s6d_render_handle is not None:w._s6d_render_handle.close();w._s6d_render_handle=None
        root.destroy()
        if spy:spy.restore()
    if spy:write_new(output/'actual_gallery_queries.json',dict(gallery=job['gallery'],actual_score_calls=len(spy.rows),queries=spy.rows,selection_consumers_issued_queries=False))
