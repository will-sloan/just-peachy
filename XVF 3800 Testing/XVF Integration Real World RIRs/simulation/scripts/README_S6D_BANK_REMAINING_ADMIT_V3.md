...
PowerShell (root only; replace all final V8 authority placeholders with the independently accepted paths/hash):

```powershell
$sim='C:\Users\amiri\Documents\GitHub\just-peachy\XVF 3800 Testing\XVF Integration Real World RIRs\simulation'
$sourceReview='<ROOT_ACCEPTED_V8_SOURCE_REVIEW_PATH>'
$executionFreeze='<FINAL_V8_COMBINED_SOURCE_FREEZE_PATH>'
$executionSha='<EXACT_FINAL_V8_FREEZE_SHA256>'
$recovery='<ACTUAL_SUCCESSFUL_RECOVERY_V2_PATH>'
& 'C:\Users\amiri\Documents\GitHub\just-peachy\.edge-speech-env\python.exe' -B "$sim\scripts\s6d_bank_remaining_admit_v3.py" --source-review $sourceReview --execution-freeze $executionFreeze --execution-freeze-sha256 $executionSha --recovery $recovery
```
...
set "SOURCE_REVIEW=<ROOT_ACCEPTED_V8_SOURCE_REVIEW_PATH>"
set "EXECUTION_FREEZE=<FINAL_V8_COMBINED_SOURCE_FREEZE_PATH>"
set "EXECUTION_SHA=<EXACT_FINAL_V8_FREEZE_SHA256>"
set "RECOVERY=<ACTUAL_SUCCESSFUL_RECOVERY_V2_PATH>"
"C:\Users\amiri\Documents\GitHub\just-peachy\.edge-speech-env\python.exe" -B "%SIM%\scripts\s6d_bank_remaining_admit_v3.py" --source-review "%SOURCE_REVIEW%" --execution-freeze "%EXECUTION_FREEZE%" --execution-freeze-sha256 "%EXECUTION_SHA%" --recovery "%RECOVERY%"
```
...
The only executable delta from the independently checked V2 helper is the exact V8 owner, V3 root-review status, import namespace and this README binding. All 378 future IDs, 430-attempt/19,744.058-second accounting, canonical dependency isolation, Windows basename handling, source checks and physical40 GiB limits are unchanged. Recovery still uses schema edge-s6d-restoration-recovery.v2; the new helper implementation is V3. V1/V2 recovery failures and earlier source epochs remain evidence. Actual recovery and 20 owner check-plans plus independent literal queue review remain required before launch.
