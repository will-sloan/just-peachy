"""V11 adds recorded-QA recovery admission; README_S6D_CAPTURE_V11.md. No import-time I/O."""
...
def validate_plan(plan_path,authorization=None):
    ...
    if authorization is not None:
        ...
        required=[Path(__file__),SIM/'scripts/README_S6D_CAPTURE_V11.md',SIM/'scripts/README_S6D_CAPTURE_V5.md',SIM/'scripts/s6d_restoration_policy_v1.py',SIM/'scripts/s6d_capture_transport.py',SIM/'scripts/s6d_telemetry_v2.py',
            SIM/'scripts/README_S6D_TELEMETRY_V2.md',SIM/'scripts/s6d_closed_telemetry_restore_v4.py',SIM/'scripts/README_S6D_CLOSED_TELEMETRY_RESTORE_V4.md',
            SIM/'scripts/s6d_closed_telemetry_restore_v5.py',SIM/'scripts/README_S6D_CLOSED_TELEMETRY_RESTORE_V5.md',
            SIM/'scripts/s6d_closed_telemetry_restore_v6.py',SIM/'scripts/README_S6D_CLOSED_TELEMETRY_RESTORE_V6.md',
...
def recorder_port_preflight(batch_dir,provider):
    ...
    from s6d_closed_telemetry_restore_v4 import tcp_census,tcp_listener_decision
...
def prior_closure(report,reviewed_recoveries=()):
    ...
    for acquired in (report/'hardware_batches').glob('*/owner_acquired.json'):
        ...
        if not valid:
            ...
            if recovery.get('schema_version')=='edge-s6d-restoration-recovery.v2' and recovery.get('recovery_kind')=='closed_recorded_qa_telemetry_fresh_restore':
                from s6d_closed_telemetry_restore_v6 import verify_recovery_record
                ...
                if proof.get('status')!='VERIFIED_CLOSED_RECORDED_QA_TELEMETRY_FRESH_RESTORE' or proof.get('old_failure_preserved') is not True or proof.get('historical_python_terminal_receipt_present') is not True:
                    raise RuntimeError('Recorded-QA fresh restoration verifier did not prove separate closure')
                continue
            if recovery.get('schema_version')=='edge-s6d-restoration-recovery.v2' and recovery.get('recovery_kind')=='closed_recorded_telemetry_fresh_restore':
                from s6d_closed_telemetry_restore_v5 import verify_recovery_record
                proof=verify_recovery_record(recovery,owner_binding=record['owner'],restoration_binding=record['restoration'],initial_binding=binding(acquired.parent/'initial_state.json'))
                if proof.get('status')!='VERIFIED_CLOSED_RECORDED_TELEMETRY_FRESH_RESTORE' or proof.get('old_failure_preserved') is not True or proof.get('historical_python_terminal_receipt_present') is not True:
                    raise RuntimeError('Recorded-terminal fresh restoration verifier did not prove separate closure')
                continue
            if recovery.get('schema_version')=='edge-s6d-restoration-recovery.v2':
                ...
                from s6d_closed_telemetry_restore_v4 import verify_recovery_record
                proof=verify_recovery_record(recovery,owner_binding=record['owner'],restoration_binding=record['restoration'],initial_binding=binding(acquired.parent/'initial_state.json'))
