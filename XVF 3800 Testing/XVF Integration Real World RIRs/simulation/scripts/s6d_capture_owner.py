"""Explicit reviewed S6D hardware owner; README_S6D_CAPTURE.md. No import-time I/O."""
...
def validate_plan(plan_path,authorization=None):
    ...
    if authorization is not None:
        ...
        required=[Path(__file__),SIM/'scripts/s6d_capture_transport.py',SIM/'scripts/s6d_telemetry.py',
...
def capture_to_disk(attempt,packed_path,folder,stop_path,telemetry,sd,endpoints,storage_check=None,audio_state=None):
    ...
    try:
        if stop_path.exists() or not telemetry.healthy():raise RuntimeError('STOP or unhealthy telemetry before audio open')
        ...
        with stream_object as stream:
            ...
            while not done.wait(.05):
                ...
                if stop_path.exists() or writer_failed.is_set() or not telemetry.healthy():
...
def prior_closure(report):
    ...
    for acquired in (report/'hardware_batches').glob('*/owner_acquired.json'):
        ...
        receipt=read(path)
        if receipt.get('status')!='PASS' or receipt.get('exact_recorded_configuration_match') is not True or receipt.get('telemetry_process_closed') is not True:
            raise RuntimeError('Unproven prior restoration; explicit reviewed recovery required')
...
def execute(plan_path,authorization,batch,attempt_ids):
    ...
    from s6d_telemetry import S6DQueuedTelemetryLogger
    import soundfile as sf
    ...
    try:
        ...
        for attempt in selected:
            if (report/'STOP_REQUEST.json').exists():raise RuntimeError('Stop requested before next attempt')
            ...
            audio_state=dict(audio_open_attempted=False,audio_handles_closed=True)
            meta=capture_to_disk(attempt,packed_path,folder,report/'STOP_REQUEST.json',telem,sd,xvf_endpoints,lambda:storage(report,payload),audio_state)
            telem.stop('audio_closed');telem_result=telem.wait(20)
            if telem_result is None or not telem_result.get('control_owner_closed_proven'):raise RuntimeError('Telemetry cleanup unproven; competing control forbidden')
        ...
        telemetry_safe=telem is None or telem_result is not None and telem_result.get('control_owner_closed_proven') is True
        ...
        if mutated and initial and safe:
            try:
                ...
                restoration=dict(status='PASS' if same else 'FAIL',exact_recorded_configuration_match=same,readback=after,reset=reset_receipt,reapply=reapplied,
                    ...
                    telemetry_process_closed=True,scope='Exact exposed decimal getters/static/ancillary state and USB width; hidden adaptive history reset, not restored')
        ...
        elif owned:restoration=dict(status='PASS',exact_recorded_configuration_match=True,telemetry_process_closed=True,scope='No setters or playback occurred under this owner')
        try:
