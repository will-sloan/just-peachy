"""Bounded independent N00 admission/contrast audit; see README_S6C_N00_REVIEW.md."""
...
def run(args):
 ...
 source_bindings+=[receipt['index'],binding(REPORT/'design/REGISTERED_DESIGN_V1.json'),binding(__file__),binding(Path(__file__).with_name('README_S6C_N00_REVIEW.md'))]
 summary=[]
 for pid in sorted(index['profiles']):
  for tap in ('O0','O1'):
   rows=[v for v in profiles if v['profile_id']==pid and v['stream']==tap];comp=[v for v in rows if v['population'] in COMPLETE]
   c=Counter()
   for v in comp:
...
if __name__=='__main__':
 parser=argparse.ArgumentParser();parser.add_argument('--output-subdir',default='independent_review/n00_screen_v1');run(parser.parse_args())
