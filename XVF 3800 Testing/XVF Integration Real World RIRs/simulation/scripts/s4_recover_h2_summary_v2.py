"""Verified completion-summary recovery for remaining S4 jobs. See README_S4_H2_RECOVERY.md."""
...
ALLOWED_CASES = tuple(f'S4_{i:02}' for i in range(20, 25))
...
def validate_capture_identity(receipt, capture, gains):
    output = receipt['stream']
    if receipt['case_id'] != capture['case_id'] or receipt['raw_audio']['sha256'] != capture['output_audio'][output]['sha256']:
        raise ValueError('Original input does not join the authoritative accepted capture')
    if receipt['identity']['raw_audio_sha256'] != receipt['raw_audio']['sha256']:
        raise ValueError('Original job raw-audio identity mismatch')
    if receipt['identity']['gain_scalar'] != gains[output] or receipt['adapter']['gain_scalar'] != gains[output]:
        raise ValueError('Original adapter does not use the frozen output-specific gain')


def run(case_id, stream, execute=False):
    if case_id not in ALLOWED_CASES or stream not in ('O0', 'O1'):
        raise ValueError('Only remaining canonical S4_20..24 output jobs are in scope')
    recovery_path = REPORT / f'H2_SUMMARY_RECOVERY_{case_id}_{stream}.json'
    folder = REPORT / 'h2' / case_id / stream
    sessions = list((folder / 'empty_data/edge_speech_sessions').iterdir())
    if len(sessions) != 1 or not sessions[0].is_dir():
        raise ValueError('Require exactly one original session directory')
    session = sessions[0]
    receipt_path = folder / 'run_receipt.json'
    receipt = read(receipt_path)
    if receipt.get('case_id') != case_id or receipt.get('stream') != stream:
        raise ValueError('Requested case/output differs from the original failed invocation')
    ...
    selected = accepted_cases()[case_id]
    policy = read(REPORT / 'OUTPUT_LEVEL_POLICY.json')
    bind(REPORT / 'OUTPUT_LEVEL_POLICY.json', contract['output_policy_sha256'])
    bind(REPORT / 'ACCEPTED_FINAL_CAPTURES.json', contract['accepted_capture_selection_sha256'])
    validate_capture_identity(receipt, selected['case_result'], policy['fixed_host_gain'])
    if Path(receipt['raw_audio']['path']).resolve() != (selected['folder'] / (stream + '.wav')).resolve():
        raise ValueError('Receipt points at an excluded or different capture path')
    adapter = folder / f'input_{stream}_fixed_gain.wav'
    ...
    source_paths = {'events': events_path, 'stdout': stdout_path, 'labelled_transcript': labelled_path,
                    ...
                    'accepted_capture_selection': REPORT / 'ACCEPTED_FINAL_CAPTURES.json',
                    'accepted_case_result': selected['folder'] / 'case_result.json',
                    'output_level_policy': REPORT / 'OUTPUT_LEVEL_POLICY.json',
...
class RecoveryTests(unittest.TestCase):
    ...
    def test_output_specific_frozen_gain_and_accepted_capture(self):
        capture = {'case_id': 'S4_20', 'output_audio': {'O0': {'sha256': 'a'}, 'O1': {'sha256': 'b'}}}
        gains = {'O0': 1.4125375446227544, 'O1': 1.0}
        for output in ('O0', 'O1'):
            receipt = {'case_id': 'S4_20', 'stream': output, 'raw_audio': {'sha256': capture['output_audio'][output]['sha256']},
                       'identity': {'raw_audio_sha256': capture['output_audio'][output]['sha256'], 'gain_scalar': gains[output]},
                       'adapter': {'gain_scalar': gains[output]}}
            validate_capture_identity(receipt, capture, gains)
            bad = copy.deepcopy(receipt); bad['adapter']['gain_scalar'] = .5
            with self.assertRaises(ValueError): validate_capture_identity(bad, capture, gains)
            bad = copy.deepcopy(receipt); bad['raw_audio']['sha256'] = 'excluded'
            with self.assertRaises(ValueError): validate_capture_identity(bad, capture, gains)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--case', choices=ALLOWED_CASES)
    parser.add_argument('--stream', choices=('O0', 'O1'))
    ...
    if args.case is None or args.stream is None:
        parser.error('--case and --stream are required except for --self-test')
    run(args.case, args.stream, execute=args.execute)
