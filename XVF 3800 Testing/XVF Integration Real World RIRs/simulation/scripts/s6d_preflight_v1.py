"""Read-only S6D device/host preflight; see README_S6D_PREFLIGHT.md."""
...
def run(report):
 ...
 try:
  result["resources"]={"memory_available_bytes":psutil.virtual_memory().available,"disks":{d:{"free_bytes":psutil.disk_usage(d+":\\").free,"total_bytes":psutil.disk_usage(d+":\\").total} for d in ["C","G"]}}
  ...
  result["sources"]=[binding(__file__),binding(Path(__file__).with_name("README_S6D_PREFLIGHT.md")),binding(ROOT/"measurement_app/core.py"),binding(HOST)]
