"""Serial original paced runners; README_S6C_PACED_DISPATCH_V4.md."""
from __future__ import annotations
import argparse,importlib.util,hashlib,json,math,os,re,subprocess,sys,time,traceback
...
B36_NAME='s6c_paced_b36_fast_v2.py'
B36_SHA='130255266206ddb95c917ad5825328782dbeedafa9656814024c1ed1d32e0892'
WRAPPERS[B36_NAME]=(B36_SHA,'s6c-historical-paced-b36-fast.v2','paced_controls',None)

def b36_module():
 require(binding(HERE/B36_NAME)['sha256']==B36_SHA,'Exact independently reviewed B36 V2 source')
 spec=importlib.util.spec_from_file_location('_s6c_dispatch_b36_v2',HERE/B36_NAME);module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module);return module

class B36Reader:
 def read(self,path,expected=None):return read(Path(path).resolve(),expected)

def b36_completion(item,plan,owner,quiet_binding,inspect):
 helper=b36_module();reader=B36Reader();actual,pb=helper.admit_plan(reader,item['manifest'])
 require(actual==plan and pb==item['manifest'] and item['cells']==40,'Exact unchanged B36 forty-job plan')
 proof=helper.admit_complete_batch(reader,pb,inspect=inspect)
 require(proof['status']=='B36_NATIVE_BATCH_COMPLETE_WITH_EXPLICIT_C_ARCHIVE' and proof['manifest']==pb and proof['cells']==40,'Explicit new B36 archival proof')
 same_owner(proof['owner'],owner)
 launch,lb=read(proof['launch']['path'],proof['launch'])
 require(launch['quiet_admission']==quiet_binding and launch['coordinator']==item['helper'] and proof['owner']['argv']==owner['argv'],'Exact dispatcher B36 owner/quiet/helper chain')
 refs=[proof[k] for k in ('launch','completion','lease_release','archived_lease','observer_exit')]
 refs += [row['completion'] for row in proof['completed_cells']]
 return dict(status='ORIGINAL_BATCH_COMPLETE_AND_OWNERS_CLOSED',manifest=pb,cells=40,invocation=str(Path(lb['path']).parent),bindings=refs,owners=proof['current_child_observations'],explicit_b36_v2_admission=proof,scope='Explicit V2 coordinator C-side archive; native metadata transition proof only. Scientific analysis and inventory remain separate.')
...
HEARTBEAT_REPLACE_RETRY_SEC=2.0
HEARTBEAT_REPLACE_RETRY_INTERVAL_SEC=0.05
HEARTBEAT_REPLACE_MAX_ATTEMPTS=41

def replace_heartbeat(temporary,target):
 """Retry only transient Windows sharing/access errors on the same heartbeat tmp."""
 require(target.name=='HEARTBEAT.json' and temporary==target.with_name(target.name+'.tmp'),'Exact heartbeat replacement paths required')
 deadline=time.monotonic()+HEARTBEAT_REPLACE_RETRY_SEC
 attempts=0
 while True:
  attempts+=1
  try:
   ...
   if getattr(exc,'winerror',None) not in (5,32,33):raise
   remaining=deadline-time.monotonic()
   if remaining<=0 or attempts>=HEARTBEAT_REPLACE_MAX_ATTEMPTS:raise
   time.sleep(min(HEARTBEAT_REPLACE_RETRY_INTERVAL_SEC,remaining))
...
def save(path,value,replace=False):
 ...
 if replace:
  t=p.with_name(p.name+'.tmp');require(not t.exists(),'Preserved heartbeat temporary file');t.write_bytes(raw)
  if p.name=='HEARTBEAT.json':replace_heartbeat(t,p)
  else:os.replace(t,p)
 else:
  with p.open('xb') as f:f.write(raw);f.flush();os.fsync(f.fileno())
 return binding(p,raw)
HEARTBEAT_SNAPSHOT_LIMIT=20000
HEARTBEAT_SNAPSHOT_MAX_BYTES=16384
HEARTBEAT_SNAPSHOT_BLOCK_SIZE=1000

def save_heartbeat(root,value,sequence):
 """Write a bounded immutable observation; never replace a reader-held file."""
 require(type(sequence) is int and 1<=sequence<=HEARTBEAT_SNAPSHOT_LIMIT,'Bounded positive heartbeat sequence required')
 raw=(json.dumps(value,indent=2,allow_nan=False)+'\n').encode()
 require(len(raw)<=HEARTBEAT_SNAPSHOT_MAX_BYTES,'Heartbeat snapshot exceeds byte cap')
 p=Path(root)/'heartbeats'/f'{(sequence-1)//HEARTBEAT_SNAPSHOT_BLOCK_SIZE:03d}'/f'HEARTBEAT_{sequence:05d}.json'
...
def admit(queue_binding,authority_binding):
 ...
 for item in q['items']:
  ...
  require(re.fullmatch('[A-Za-z0-9_-]{1,64}',namespace) and namespace.endswith('_fast_v2' if kind or name==B36_NAME else '_fast_v1'),'Exact fresh wrapper namespace')
...
def completion(item,plan,owner,quiet_binding,inspect=state):
 ...
 if Path(item['helper']['path']).name==B36_NAME:return b36_completion(item,plan,owner,quiet_binding,inspect)
...
def run(args):
 ...
 save(out/'ADMISSION.json',dict(status='DISPATCHER_STARTED',owner=owner,queue=qb,authority=ab,source=binding(__file__),readme=binding(HERE/'README_S6C_PACED_DISPATCH_V4.md'),created_utc=utc()))
 ...
 try:
  for item,plan in zip(q['items'],plans):
   ...
   while child.poll() is None:
    ...
    save_heartbeat(out,dict(status='RUNNING',owner=owner,active_item=active,child=child_owner,child_observation=observed,completed_batches=len(completed),requested_batches=len(q['items']),completed_cells=sum(x['cells'] for x in completed),elapsed_sec=time.monotonic()-started,created_utc=utc()),heartbeat_sequence)
