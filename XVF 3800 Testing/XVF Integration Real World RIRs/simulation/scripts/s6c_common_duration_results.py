"""Fixed-competitor E-duration results. README_S6C_COMMON_DURATION_RESULTS.md."""
...
def table(receipt,name):
 ...
 if core.bind(b['path'])!=b:raise ValueError('Changed completed table')
...
def run(args):
 ...
 if nr['index']!=cr['index'] or nr['requested']!=672 or cr['requested']!=672:raise ValueError('Exact same six-profile56-case/two-tap panel required')
 index=core.verified(nr['index']);cases=set(index['case_ids'])
 if len(cases)!=56 or {core.route_key(r) for r in index['profile_routes']}!={(p,t,t) for p in IDS for t in ('O0','O1')}:raise ValueError('Exact common-duration panel routes required')
 ...
 if any(len(rows)!=178 for rows in groups.values()):raise ValueError('All178 source occurrences per panel route must remain')
 summary=[];strata=[];within=[];deltas=[];delay_rows=[];retained_summary=[]
 for (pid,tap),rows in groups.items():
  side,tier=IDS[pid];base=dict(profile_id=pid,stream=tap,roster=side,tier=tier,gallery_available_people=14,gallery_available_corpus_people='9 CMU ARCTIC;5 HiFiTTS',scored_scenes=56)
 ...
 result=dict(status='COMPLETE_SOURCE_BOUND_DESCRIPTIVE_COMPARISON',core=cb,names=nb,index=nr['index'],source_tables=[tb,rb],source_occurrences=len(turns),profiles=6,routes=12,scenes_per_route=56,source_occurrences_per_route=178,
  tests=fixtures(),sources=[core.bind(Path(__file__)),core.bind(Path(__file__).with_name('README_S6C_COMMON_DURATION_RESULTS.md')),*nr['codes']],
  artifacts=[core.bind(out/n) for n in tables],neural_calls=0,rescored_predictions=0,
  scope='Fixed14-person roster and competitors per side at each5/15/30 tier. Known/withheld populations and exact source support are matched; source-to-XVF domain mismatch persists. Nested enrollment duration changes speech content/native centroid too, not a duration-only physical intervention. Common Voice remains withheld here: no CV enrollment-duration inference.56-scene exploratory panel, not all240.',
...
if __name__=='__main__':
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--names-subdir',default='common_duration_panel_names_v3');p.add_argument('--core-subdir',default='common_duration_panel_core_v3');p.add_argument('--output-subdir',default='common_duration_results_v1');p.add_argument('--test',action='store_true');a=p.parse_args()
