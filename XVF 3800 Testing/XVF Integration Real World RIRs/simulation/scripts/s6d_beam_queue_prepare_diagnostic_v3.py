"""Held diagnostic-only80GiB dual-journal queue; see README_S6D_BEAM_DIAGNOSTIC_V3.md."""
import argparse
from copy import deepcopy
import hashlib
import json
from pathlib import Path
import s6d_beam_native_run_diagnostic_v3 as N
...
def prepare(manifest_path,manifest_sha,output):
    ...
    payload=payload_sources(m);runner=payload['runner']
    ...
    q['payload_policy'].update(max_new_payload_bytes=80*2**30,cap_exception=payload['exception'])
    ...
    for job in m['jobs']:
        ...
        sources=[runner,mb,m['runner_helper'],*m['source_files'],*m['support'].values(),m['assets'],job['profile_binding'],job['gallery'],job['admission'],job['s6d_settings'],*payload['guards'],N.bind(__file__),N.bind(Path(__file__).with_name('README_S6D_BEAM_DIAGNOSTIC_V3.md'))]
    ...
    proposal['payload_cap_exception']=payload['exception']
    N.save(out/'APPROVAL_PROPOSAL.json',proposal)
    N.save(out/'QUEUE_RECEIPT.json',dict(status='PROPOSED_NOT_APPROVED',source=N.bind(__file__),manifest=mb,queue=qb,approval_proposal=N.bind(out/'APPROVAL_PROPOSAL.json'),runner=runner,payload_cap_exception=payload['exception'],payload_source_acceptance=payload['acceptance'],job_count=len(q['jobs']),state_dir=str(P.G/'runner'/out.name/'supervisor_state'),models_started=0,processes_started=0,hardware_calls=0,limits='Root must independently approve exact diagnostic source/queue, actual capture/control reuse,80GiB exception graph, current floors and globally at most4 workers with no controlled native/C/core/Tk/HOST overlap before validate/launch'))
    print(json.dumps(N.bind(out/'QUEUE_RECEIPT.json')))

def payload_sources(manifest):
    """Exact accepted future-offline80 sources; source acceptance does not approve jobs."""
    def checked(ref):N.need(N.bind(ref['path'])==ref,'Changed payload authority');return ref
    acceptance=N.bind(P.R/'runner/source_epoch_payload_v5/ROOT_SOURCE_ACCEPTANCE.json');a=N.verified(acceptance)
    N.need(acceptance['sha256']=='585c3db4729d7b75ce42cc2cf7b0fc619d89afd096730c3215538ac1b48d4c34' and a['status']=='ROOT_ACCEPTED_FUTURE_OFFLINE_PAYLOAD_V5_AND_WRAPPER_V3_SOURCE_ONLY','Exact root future-offline source acceptance required')
    rf=checked(a['runner_source_freeze']);r=N.verified(rf);wf=checked(a['wrapper_source_freeze']);w=N.verified(wf)
    for b in r['sources']:checked(b)
    runner=checked(w['runner']);wrapper=checked(w['wrapper']);exception=checked(a['exception']);e=N.verified(exception)
    N.need(rf['sha256']=='ad06ac96a795c298f7602e314846fe7ef4505c0688f7b0902364d2e9245a4ad6' and wf['sha256']=='f1a4cf4505187311f3025075ff86cf3eeb0a872ea25db4707a2fab111e7435d7','Accepted payload source freezes differ')
    N.need(runner['sha256']=='57fb87aaba12495f724adbcc78a2609f82a897dd5d3877e6a2cabd54a68a7978' and wrapper['sha256']==N.PINS['protocol'],'Exact runnerV5/protocolV3 required')
    N.need(exception['sha256']=='079900e58493e18c3a995754ba7ac16ff680e4697ba8dc6521e0b6fe1eb3a54c','Exact root80GiB exception required')
    guards=[exception]+[checked(e[k]) for k in ('contract','forecast','prior_runner')]+[acceptance,rf,wf]
    N.need(manifest.get('payload_cap_exception')==exception and manifest.get('payload_source_acceptance')==acceptance,'Manifest must explicitly bind exception and source acceptance')
    N.need(manifest['support'].get('runner')==runner and manifest['support']['protocol']==wrapper and manifest['limits'].get('max_new_payload_gib')==80,'Manifest runner/protocol/payload graph differs')
    return dict(runner=runner,wrapper=wrapper,exception=exception,acceptance=acceptance,guards=guards)
