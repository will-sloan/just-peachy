"""Exact original/configured identity recovery; see README_S6D_CLOSED_TELEMETRY_RESTORE_V4.md."""
...
KIND='closed_native_telemetry_fresh_restore'
REVIEW_STATUS='ROOT_ACCEPTED_TELEMETRY_RECOVERY_V2_SOURCES_V4'
...
def scan_decision(snapshot,known,current):
    ...
    return dict(status='NO_MATCHING_TASK_CONTROL_PROCESSES',current=current,known_processes=known_states,matching_processes=[],unreadable_control_processes=[],processes_scanned=len(seen),excluded_only_current_pid=True,telemetry_pid_creation_persisted=False,scope='All inspectable command lines scanned for exact task-control markers; critical interpreter/control processes with unreadable command lines or creation times block. No process was stopped or killed.')

def required_sources():
    producer=Path('G:/Just_Peachy_S6D/20260913T195357Z/bank_captures_v1/beam_bank/S45_01_17/P_MAIN6/P_MAIN6_S45_01_17/telemetry/source')
    return [Path(__file__),Path(__file__).with_name('README_S6D_CLOSED_TELEMETRY_RESTORE_V4.md'),SIM/'scripts/s6d_restoration_policy_v1.py',*[SIM/'scripts'/n for n in ('s3_hardware.py','s4_restore.py','s4_common.py','s0_common.py')],ROOT/'measurement_app/core.py',ROOT/'measurement_app/__init__.py',*[ROOT/'tools/xvf321/binary/host_v3.0.0/win32'/n for n in ('xvf_host.exe','device_usb.dll','command_map.dll')],*[producer/n for n in ('s6d_telemetry.py','S6DQueuedTelemetry.cs','Run-S6D-Telemetry.ps1','s6d_capture_transport.py')]]
...
def inspect_review(path,expected_hash,execute=False):
    ...
    need(a.get('owner_thread_id')==ROOT_THREAD and a.get('run_id')=='20260913T195357Z' and a.get('original_batch')=='bank_v2_P_MAIN6_B1' and a.get('recovery_kind')==KIND,'Wrong recovery owner/batch/kind')
    need(a.get('historical_telemetry_pid_creation') is None,'Do not invent an unpersisted historical telemetry PID')
    ...
    need(folder==Path('G:/Just_Peachy_S6D/20260913T195357Z/bank_captures_v1/beam_bank/S45_01_17/P_MAIN6/P_MAIN6_S45_01_17/telemetry'),'Wrong failed telemetry attempt')
    expected_paths={'native_result':folder/'native/result.json','native_inspection':folder/'native/command_map_inspection.json','native_samples':folder/'native/samples.jsonl','native_transactions':folder/'native/transactions.tsv','stdout':folder/'stdout.bin','received':folder/'received_telemetry.jsonl','stderr':folder/'stderr.bin'}
    ...
    need(not (folder/'result.json').exists(),'Unexpected Python terminal receipt; separate review required')
    proof=native_closure(telemetry,a['expected_measurement_rows']);inspection=read(telemetry['native_inspection']['path'])
...
def failed_configuration_proof(originals,values):
    """Bind the recorded failed17 state to its admitted attempt and original ledger."""
    expected=Path('G:/Just_Peachy_S6D/20260913T195357Z/bank_captures_v1/beam_bank/S45_01_17/P_MAIN6/P_MAIN6_S45_01_17/configuration.json')
    ref=originals['failed_configuration'];need(Path(ref['path']).resolve()==expected,'Exact failed17 configuration path required')
    config=verify(ref);admission=values['admission'];plan=verify(admission['plan']);exact=policy_module().exact
    attempt_id='P_MAIN6_S45_01_17';rows=[x for x in plan['attempts'] if x.get('attempt_id')==attempt_id]
    need(len(rows)==1 and attempt_id in admission['attempt_ids'],'Unique original admitted failed attempt required');attempt=rows[0]
    need(attempt.get('case_id')=='S45_01_17' and attempt.get('profile')==config.get('profile')=='P_MAIN6','Failed configuration case/profile differs')
    ...
    need(len(charged)==1 and all(charged[0].get(k)==v for k,v in dict(batch='bank_v2_P_MAIN6_B1',case_id=attempt['case_id'],profile=attempt['profile'],status='FAIL',input_sha256=source['sha256']).items()),'Original charged failure/source differs')
    ...
    return dict(status='EXACT_FAILED_CONFIGURATION_BOUND',configuration=ref,plan=admission['plan'],attempt_id=attempt_id,case_id=attempt['case_id'],profile=attempt['profile'],source_audio=source)
...
def recover_prevalidated(context,services):
    ...
    result=dict(schema='s6d-closed-telemetry-fresh-restore-result.v1',status='STARTED',recovery_kind=KIND,fixture_only=services.fixture,started_utc=utc(),recovery_identity=services.identity,source_review=context['review_ref'],original_bindings=context['original'],original_ledger_snapshot=ledger_snapshot,native_telemetry_closure=context['native_proof'],known_processes=context['known'],process_scans=[],hardware_lock_acquired=False,hardware_lock_released=False,device_restore_started=False,device_restore_completed=False,audio_streams_opened=0,packed_playback_attempts=0,processes_terminated=0,historical_python_terminal_receipt_present=False,original_telemetry_process_closed=False)
...
def verify_recovery_record(receipt,*,owner_binding,restoration_binding,initial_binding):
    """File-only V6 admission API. No process census, ports, locks, getters or setters."""
    ...
    closure=native_closure(authority['telemetry_bindings'],authority['expected_measurement_rows']);need(receipt['native_telemetry_closure']==measurement['native_telemetry_closure']==closure,'Actual native cleanup/population proof differs')
    ...
    need(all(receipt.get(k) is True for k in ('current_telemetry_processes_absent','hardware_lease_released','audio_handles_closed','packed_input_disabled','no_playback','requires_explicit_new_queue_admission')) and all(receipt.get(k) is False for k in ('no_setters_or_reset','original_telemetry_process_closed','historical_python_terminal_receipt_present')) and receipt.get('historical_telemetry_pid_creation') is None and receipt.get('original_strict_snapshot_status_preserved')=='FAIL','New versus historical closure scope differs')
    ...
    return dict(status='VERIFIED_CLOSED_NATIVE_TELEMETRY_FRESH_RESTORE',recovery_kind=KIND,original_owner=owner_binding,original_restoration=restoration_binding,recovery_measurement=receipt['recovery_measurement'],historical_python_terminal_receipt_present=False,old_failure_preserved=True)
