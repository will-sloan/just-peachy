...
SCHEMA="jp_s6c_core_analysis.v1"
CODES=("s6c_analysis.py","s6b_analysis.py","s6a_text_metrics.py","s6a_cue_score.py","s6a_support_metrics.py","s6a_baseline_results.py","s4_h2_analysis.py","s5_statistics.py")
...
def validate_payload(value,item):
 ...
 if value.get("schema")=="jp_s6c_prediction.v1" and (not isinstance(value.get("identity"),dict) or digest(value["identity"])!=value.get("prediction_key")):raise ValueError("S6C prediction identity digest mismatch")
...
def run(args):
 ...
 registry_path=REPORT/"design/REGISTERED_DESIGN_V1.json";registry=read(registry_path)["candidates"]
 ...
 receipt=dict(schema=SCHEMA,status="COMPLETE_REQUESTED_INDEX" if success else "PARTIAL_RESUMABLE",
  index=index_binding,input_index=input_binding,bank=bank_binding,codes=codes,packages=packages,registry=bind(registry_path),
...
def main():
 ...
 p.add_argument("--index",type=Path);p.add_argument("--output-subdir",default="core_analysis_v1")
