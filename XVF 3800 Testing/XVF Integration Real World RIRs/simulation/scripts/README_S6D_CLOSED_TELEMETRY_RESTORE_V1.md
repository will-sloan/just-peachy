...
- `status`: `ROOT_ACCEPTED_TELEMETRY_RECOVERY_V2_SOURCES_V1` for execution; file-only inspection also accepts `PROPOSED_SOURCE_REVIEW_ONLY`.
...
Outputs are process-snapshot JSONs, `ORIGINAL_LEDGER.json`, command logs from unchanged Control, durable `RESTORE_STARTED.json`, `RESULT.json`, and only on verified success `RECOVERY.json`. The recovery has `schema_version=edge-s6d-restoration-recovery.v2` and the distinct recovery kind above. It binds the **fresh** reapply/readback/result, current native/process closure and released lock. `no_playback=true`, but `no_setters_or_reset=false`; original telemetry closure and historical Python terminal receipt remain false. V5 intentionally refuses this new recovery kind; separately reviewed V6 admission must call `verify_recovery_record`.
...
& $py -B "$sim\scripts\s6d_closed_telemetry_restore_v1.py" inspect-inputs --source-review '<exact root review or proposal.json>' --source-review-sha256 '<SHA256>'
```
...
"C:\Users\amiri\Documents\GitHub\just-peachy\.edge-speech-env\python.exe" -B "%SIM%\scripts\s6d_closed_telemetry_restore_v1.py" inspect-inputs --source-review "<exact review.json>" --source-review-sha256 "<SHA256>"
```
...
File-only V6 API: `verify_recovery_record(receipt, owner_binding=..., restoration_binding=..., initial_binding=...)` returns a proof or raises. It rechecks stored source/root authority, original immutable ledger/owner/state, native populations, saved scan decisions, fresh restore/policy and release flags. It does not run a process scan or touch hardware. Current process closure must also be enforced by the new owner before its new hardware work.
