"""Bound actual R0 feature and v3 cue-action diagnostics; README_S6C_CUE_DECISIONS.md."""
...
def freeze():
    ...
    if target.exists():
        ...
        return value
    authority=sealed()
    paths=[Path(__file__),Path(__file__).with_name('README_S6C_CUE_DECISIONS.md'),Path(E.__file__),
        E.REPORT/'EPOCH1_EXECUTION_MANIFEST.json',E.REPORT/'epoch1/CHALLENGE_N00_PREDICTION_INDEX.json',
        E.REPORT/'cue_audit_v1/PHYSICAL_CUE_AUDIT.json']
...
def reduced(profile):
    p=deepcopy(profile);p.pop('profile_id',None);p['tracker']['cues_enabled']=False
    return p
...
def action_counts(value):
    ...
    for d in value['decisions']:
        ...
        counts['existing_candidate_comparisons']+=len(contributions)
        counts['decisions_with_nonzero_spatial_contribution']+=any(abs(r.get('cue_score',0))>1e-12 for r in contributions.values())
...
def decisions():
    ...
    feature_index=E.read(OUT/'R0_FEATURE_CUE_AUDIT.json')
    ...
    for (pid,tap),p in profiles.items():
        ...
        parents=sorted(qid for (qid,qtap),q in profiles.items() if qtap==tap and q['cue_condition']=='CUES_OFF' and reduced(q['profile'])==reduced(p['profile']))
        if parents:matched.append(dict(parent=parents[0],equivalent_parent_config_ids=parents,candidate=pid,stream=tap,cue_condition=p['cue_condition']))
    ...
    value=dict(status='COMPLETE_ACTUAL_N00_CHALLENGE_ACTION_DIAGNOSTIC',schema='s6c-cue-action-audit.v1',created_utc=E.utc(),
        plan=E.bind(OUT/'CUE_DECISION_PLAN.json'),feature_audit=E.bind(OUT/'R0_FEATURE_CUE_AUDIT.json'),
...
def checks():
    ...
    return dict(status='PASS',checks=8,new_model_calls=0)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('action',choices=('freeze','features','decisions','checks'));a=p.parse_args()
