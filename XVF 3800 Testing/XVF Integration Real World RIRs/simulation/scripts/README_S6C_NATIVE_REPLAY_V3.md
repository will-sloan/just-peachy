...
& '.\.edge-speech-env\python.exe' "$sim\scripts\s6c_native_replay_v3.py" --epoch epoch1 --results "$sim\reports\S6C\20260910T123540Z\jobs\epoch1\smoke_v1_RESULTS.json" --label smoke_v3
```
...
V3 admits prior result bindings only from successful rows and explicitly excludes measured policy_total_sec and gallery loaded_elapsed_sec from semantic equality. Logical labels, support, event order and original source clocks remain strict. V1/V2 source and smoke outputs remain unchanged.
