"""Replay exact S6C native observations and verify logical event parity. See README_S6C_NATIVE_REPLAY_V3.md."""
...
EXECUTION_FIELDS={'identity_compute_sec','policy_compute_sec','tracker_compute_sec',
    ...
    'release_after_all_lanes_closed','compute_finished_elapsed_sec','modeled_available_at_sec','policy_total_sec','loaded_elapsed_sec'}
...
def run(epoch,results_paths,label):
    ...
    prior_bindings={(r['candidate_id'],r['case_id'],r['stream'],r['identity_tap']):r['result'] for r in prior['rows'] if r.get('result') and r.get('status') in ('COMPLETE','COMPLETE_REUSED')} if prior else {}
