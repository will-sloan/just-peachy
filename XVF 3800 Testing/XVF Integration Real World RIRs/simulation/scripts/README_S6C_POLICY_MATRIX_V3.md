...
& '.\.edge-speech-env\python.exe' "$sim\scripts\s6c_policy_matrix_v3.py" --epoch epoch1 --indices "$r\jobs\epoch1\recipes_v1_RESULTS.json" "$r\jobs\epoch1\split_v1_RESULTS.json" --label component_panel_v1 --candidates C065 C066 C067 C068 C069 C070 C071 C072 C073 C074 C075 C076 C077 C078 C079 C080 C081 C082 C083 C084 C085 C086
```
...
V3 also recomputes every target/source execution digest and rejects unequal manifests claiming the same digest. Epoch3 was frozen before this last metadata-admission repair; its unchanged APP/profiles are used with the separately frozen epoch3_policy_v3 orchestration bundle. Use JP_S6C_SIM and that bundle script for authoritative new rescue replays; all input receipts and the wrapper itself remain bound.

Authoritative rescue command, PowerShell from repository root:

```powershell
$sim = Join-Path (Get-Location) 'XVF 3800 Testing\XVF Integration Real World RIRs\simulation'
$env:JP_S6C_SIM = $sim
$r = "$sim\reports\S6C\20260910T123540Z"
& '.\.edge-speech-env\python.exe' "$sim\staging\s6c\20260910T123540Z\epoch3_policy_v3\s6c_policy_matrix_v3.py" --epoch epoch3 --source-epochs "$r\EPOCH2_EXECUTION_MANIFEST.json" --indices "$r\completed_slices\epoch2\N01_panel_v1_RESULTS.json" --label fresh_family_rescue_v1 --candidates C065 C079 C117 C118 C119 C120 C121 C122 C123 C124 C125 C126 C127 C128 C129 C130 C139 C140
```
