...
Outputs: fresh `reports/S6C/20260910T123540Z/cadence_floor_audit_v3/PLAN.json`,
...
& $analysisTask scripts/s6c_cadence_floor_audit_v3.py checks
# Only after exact native/parity/core closure; use the printed actual receipt hash.
& $analysisTask scripts/s6c_cadence_floor_audit_v3.py prepare --core reports/S6C/20260910T123540Z/cadence_floor_native_core_v3/ANALYSIS_RECEIPT.json --core-sha256 EXACT_COMPLETED_CORE_SHA256
& $analysisTask scripts/s6c_cadence_floor_audit_v3.py run --plan reports/S6C/20260910T123540Z/cadence_floor_audit_v3/PLAN.json --sha256 EXACT_PREPARED_PLAN_SHA256
```
...
"%S6C_CADENCE_PY%" scripts\s6c_cadence_floor_audit_v3.py checks
"%S6C_CADENCE_PY%" scripts\s6c_cadence_floor_audit_v3.py prepare --core reports\S6C\20260910T123540Z\cadence_floor_native_core_v3\ANALYSIS_RECEIPT.json --core-sha256 EXACT_COMPLETED_CORE_SHA256
"%S6C_CADENCE_PY%" scripts\s6c_cadence_floor_audit_v3.py run --plan reports\S6C\20260910T123540Z\cadence_floor_audit_v3\PLAN.json --sha256 EXACT_PREPARED_PLAN_SHA256
```
...
## Admission repair lineage

V2 remains preserved with its failed0-event-cell receipt. It assumed session payloads
were under each job receipt folder; the frozen native executor instead creates
`epoch5/workers/PID_CREATIONMS/sessions/SESSION`. V3 checks that exact recorded
worker identity root and requires event, summary and finalization basenames within
one declared session. It also resolves the supplied plan path before its namespace
comparison. Neither change alters the original incidence parser or aggregate AST.
No actual event file was read by the failed V2 invocation and no model was retried.
