"""Exact171+fresh5 closed-output scoring; README_S6D_NATIVE176_COMPOSITE_V2.md."""
...
def composite_declarations(spec, reader=bound_json):
    """Select actual immutable manifests; never rewrite original manifests or metric inputs."""
    cb=spec['composite_execution'];E.need(cb['sha256']==COMPOSITE_SHA,'Exact reviewed171+5 composite required')
    composite=reader(cb);validation=reader(spec['closed_input_validation']);admission=reader(spec['root_build_admission'])
    E.need(composite['schema']=='s6d-native176-composite-execution.v1' and composite['expected']==dict(original_declared=176,original_completed=171,recovery_required=5,recovery_FINISH_done=5,recovery_FINISH_total=5),'Exact composite population required')
    E.need(composite['stopped_original_attempt_credited'] is False and composite['old176_FINISH_claimed'] is False,'Historical failed/stopped state must remain uncredited')
    E.need(admission.get('schema')=='s6d-native176-scoring-build-admission.v2' and admission.get('status')=='ROOT_ACCEPTED_NATIVE176_COMPOSITE_CLOSED_INPUT_BUILDER_V2' and admission.get('owner_thread_id')=='01a0812d-3ff0-7ed0-a06c-4df61b62a459' and admission.get('allow_closed_input_construction') is True,'Explicit root composite input admission required')
    E.need(admission['composite']==cb and admission['builder']==spec['builder'] and E.binding(spec['builder']['path'])==spec['builder'],'Root composite/builder binding differs')
    E.need(validation['status']=='ALL176_COMPOSITE_CLOSED_INPUTS_VERIFIED' and validation['admission']==spec['root_build_admission'] and validation['composite']==cb and validation['builder']==spec['builder'],'Composite closure proof differs')
    E.need(validation['original_completed']==171 and validation['recovery_completed']==5 and validation['owner_exit']['status']=='ALL_RECORDED_INSTANCES_ABSENT','Both actual closure populations required')
    selection=spec['execution_selection'];E.need(selection==validation['execution_selection'] and len(selection)==176,'Root/builder-selected176 source mapping differs')
    original=reader(composite['original']['queue']);old_jobs={j['job_id']:j for j in original['jobs']}
    rows=composite['rows'];ids=[r['scientific_job_id'] for r in rows]
    E.need(len(rows)==176 and len(set(ids))==176 and ids==[j['job_id'] for j in original['jobs']],'Original176 order/membership differs')
    manifests=spec['execution_manifests'];E.need(len(manifests)==4 and len({b['path'] for b in manifests})==4,'Exact two original and two recovery manifests required')
    available={};bindings={b['path']:b for b in manifests}
    for mb in manifests:
        m=reader(mb);mids=[j['job_id'] for j in m['jobs']];E.need(len(set(mids))==len(mids),'Duplicate job within a source manifest')
        for job in m['jobs']:available.setdefault(job['job_id'],[]).append(mb)
    E.need(set(available)==set(ids),'Unmapped or omitted manifest job')
    expected=[];origins=[];selected={}
    for row in rows:
        job_id=row['scientific_job_id'];argv=old_jobs[job_id]['argv'];path=argv[argv.index('--manifest')+1];sha=argv[argv.index('--manifest-sha256')+1]
        old=bindings.get(path);E.need(old is not None and old['sha256']==sha,'Original literal queue manifest differs')
        if row['origin']=='original_completed':
            E.need(row['original_execution_job_id']==job_id and row['saved_checkpoint_record']['status']=='DECLARED_ARTIFACTS_VERIFIED','Original selected completion differs')
            mb=old;execution=job_id;allowed=[old]
        else:
            E.need(row['origin']=='recovery_required' and row['native_job_id']==job_id and row['execution_job_id']==job_id+'_recover1' and row['original_manifest']==old,'Recovery scientific/execution ID differs')
            mb=row['recovery_manifest'];execution=row['execution_job_id'];allowed=[old,mb]
        E.need(len(available[job_id])==len(allowed) and all(x in available[job_id] for x in allowed),'Foreign or duplicate alternative source declaration')
        expected.append(dict(scientific_job_id=job_id,execution_job_id=execution,origin=row['origin'],manifest=mb));origins.append(row['origin']);selected[job_id]=mb
    E.need(origins.count('original_completed')==171 and origins.count('recovery_required')==5 and selection==expected,'Exact original171/recovery5 selection required')
    E.need(len(spec['jobs'])==176 and spec['unavailable_jobs']==[] and len({x['job_id'] for x in spec['jobs']})==176 and {x['job_id'] for x in spec['jobs']}==set(ids),'All176 closed items required')
    E.need(all(item['manifest']==selected[item['job_id']] for item in spec['jobs']),'Item manifest differs from explicit selection')
    proofs=validation['completion_validations'];E.need(len(proofs)==176 and {x['job_id'] for x in proofs}==set(ids),'All176 completion proofs required')
    execution={x['scientific_job_id']:x['execution_job_id'] for x in expected}
    E.need(all(p['execution_job_id']==execution[p['job_id']] and p['identity']['job_id']==p['execution_job_id'] for p in proofs),'Selected actual execution proof differs')
    return selected
...
def run(spec_path,sha):
    ...
    declared=composite_declarations(spec)
