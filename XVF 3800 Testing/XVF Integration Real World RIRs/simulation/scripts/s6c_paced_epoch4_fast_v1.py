"""Canonical paired paced adapter; see README_S6C_PACED_EPOCH4_FAST_V1.md."""
...
_helper=Path(__file__).with_name('s6c_long_native_epoch4_fast_v1.py')
if hashlib.sha256(_helper.read_bytes()).hexdigest()!='0160a9a09c54183d2896dc6363e66ed67f1f0e049d7dcdddd4c7d22e6cb6b166':raise ValueError('Reviewed long admission utilities changed')
...
PANEL=REPORT/'design/confirmation_plan_v1/PACED_PANEL_PROPOSAL_V1.json'
PANEL_SHA='f303d7e80bc9dd8fa8b7ba7444216d1e6b29f75a0cce8762c40d92ce5201c6da'
SCHEMA='s6c-canonical-paired-paced.v1'
...
def source_bindings():
    return [L.bind(Path(__file__).with_name(n)) for n in ('s6c_paced_epoch4_fast_v1.py','README_S6C_PACED_EPOCH4_FAST_V1.md','s6c_long_native_epoch4_fast_v1.py','README_S6C_LONG_NATIVE_EPOCH4_FAST_V1.md','s6c_long_session.py','README_S6C_LONG_SESSION.md','s6c_native_prefix.py','s6c_historical_fast_observer_v1.py','README_S6C_HISTORICAL_FAST_OBSERVER_V1.md')]
...
def namespace_roots(namespace):
    if not re.fullmatch(r'[A-Za-z0-9_-]{1,60}',namespace) or not namespace.endswith('_fast_v1'):raise ValueError('Simple fresh paced namespace required')
    return REPORT/'paced_candidates'/namespace,PAYLOAD/'paced_candidates'/namespace


def panel():
    value,b=L.read_bound(PANEL)
    if b['sha256']!=PANEL_SHA or len(value['case_ids'])!=16 or len(set(value['case_ids']))!=16 or len(value['repeated_case_ids'])!=4 or len(set(value['repeated_case_ids']))!=4 or not set(value['repeated_case_ids'])<=set(value['case_ids']) or value['streams']!=['O0','O1']:raise ValueError('Exact reviewed 16+4/two-tap panel required')
    return value,b


def selected_panel(value,mode,candidates):
    if mode=='full16_plus4':return dict(case_ids=value['case_ids'],repeated_case_ids=value['repeated_case_ids'])
    if mode!='gate6_diagnostic' or candidates!=['C071','C082']:raise ValueError('Diagnostic selection requires exact original C071,C082 order')
    if value['family_native_gate_case_ids']!=GATE_CASES:raise ValueError('Reviewed six-case diagnostic list differs')
    return dict(case_ids=value['family_native_gate_case_ids'],repeated_case_ids=[])
...
def grid(selected,p):
    rows=[]
    for rep,cases in ((1,p['case_ids']),(2,p['repeated_case_ids'])):
        order=selected if rep==1 else list(reversed(selected))
        for case in cases:
            for tap in ('O0','O1'):
                for candidate in order:rows.append((candidate,case,tap,rep))
    return rows
...
def prepare(args):
    ...
    plan=dict(schema=SCHEMA,status='PREPARED_NO_MODELS_STARTED',created_utc=L.utc(),namespace=args.namespace,observer_policy=L.observer_policy(),execution_manifest=eb,input_index=ib,panel=pb,panel_mode=args.panel_mode,candidates=candidates,
        ...
        scope=('Original C071/C082 six-case diagnostic, both ASR routes, one repetition. Tests actual pacing effects on released tracker context/debt and full cost/quality; does not promise cue-event rescue or choose finalists.' if args.panel_mode=='gate6_diagnostic' else 'Canonical single-scene paced candidate panel, 16 cases plus four repeats, both ASR routes, fresh process/model/session/gallery state per cell. No finalist is implied before explicit selected IDs are supplied.'),
...
def quiet(excluded=()):
    ...
    for p in psutil.process_iter(['pid','cmdline','create_time']):
        ...
        if L.worker_command(argv) or ('s6c_paced_epoch4.py' in names and any(v in argv for v in ('run','worker'))):
...
def worker(args):
    ...
    if Path(args.owner_lease).resolve()!=REPORT/'PACED_QUIET_OWNER.json' or lease['manifest']!=pb or lease['kind']!='S6C_CANONICAL_PACED_EPOCH4' or not L.state(lease['pid'],lease['creation_time']):raise ValueError('Live parent-owned quiet lease required')
    ...
    try:
        ...
        guard()
        return common.save(root/'CELL_RESULT.json',dict(schema='s6c-canonical-paced-cell-result.v1',status='COMPLETE',created_utc=L.utc(),owner=owner,manifest=pb,job=job,source=sb,
...
def read_cell_chain(job,plan,pb,spec):
    ...
    if cell['schema']!='s6c-canonical-paced-cell-result.v1' or cell['status']!='COMPLETE' or cell['manifest']!=pb or cell['job']!=job or cell['source']!=sb:raise ValueError('Canonical cell receipt identity differs')
...
def run(args):
    ...
    with lock.open('x',encoding='utf-8') as f:json.dump(dict(**owner,manifest=pb,kind='S6C_CANONICAL_PACED_EPOCH4',launch=launch),f);f.flush();os.fsync(f.fileno())
...
def checks():
    count=0;p=dict(case_ids=[f'case{i}' for i in range(16)],repeated_case_ids=[f'case{i}' for i in range(4)])
    rows=grid(['C1','C2'],p);assert len(rows)==80 and len(set(rows))==80;count+=1
    assert rows[-1][0]=='C1' and rows[0][0]=='C1';count+=1
    pair={tap:dict(stream=tap,case_id='case',duration_sec=44.6954375,input_gain=1.,already_gained=True,historical_gain_applied_once=1.4125375446227544 if tap=='O0' else 1.,audio={},audio_pcm_sha256=tap,telemetry={}) for tap in ('O0','O1')}
    assert validate_source_rows(pair)==715127;count+=1
    src=source_record(pair,{})
    assert src['source_offset_samples']==src['inserted_gap_samples']==0 and src['duration_samples']==715127;count+=1
    for field,value in (('duration_sec',45.),('input_gain',1.4125375446227544),('already_gained',False),('historical_gain_applied_once',1.),('case_id','other'),('telemetry',{'different':True})):
        bad=deepcopy(pair);bad['O0'][field]=value
        try:validate_source_rows(bad)
        except ValueError:count+=1
        else:raise AssertionError('Changed canonical source admitted: '+field)
    for name in ('../old','x/y','', 'x'*61):
        try:namespace_roots(name)
        except ValueError:count+=1
        else:raise AssertionError('Unsafe namespace')
    spec=dict(profiles=[dict(candidate_id='C1',asr_tap=t,identity_tap=t,gallery_condition='NONE',cue_condition='CUES_OFF') for t in ('O0','O1')])
    assert len(candidate_routes(spec,['C1']))==2;count+=1
    for candidates in ([],['C1','C1'],['C2']):
        try:candidate_routes(spec,candidates)
        except ValueError:count+=1
        else:raise AssertionError('Unregistered candidate grid admitted')
    for condition in ('ALL_EXPECTED_SETUP','COMMON30_FIXED_ROSTER_A'):
        bad=deepcopy(spec);bad['profiles'][0]['gallery_condition']=condition
        try:candidate_routes(bad,['C1'])
        except ValueError:count+=1
        else:raise AssertionError('Unsupported gallery admitted')
    jobs=[dict(job_id=j,candidate_id='C1',asr_tap='O0',identity_tap='O0') for j in ('one','two')]
    empty=progress(jobs,[],[],0.);assert empty['heuristic_eta_range_sec'] is None and empty['completed_cell_source_seconds_per_wall_second'] is None and empty['mean_nested_model_startup_sec'] is None;count+=1
    measured=dict(job_id='one',candidate_id='C1',asr_tap='O0',identity_tap='O0',source_sec=40.,completed_cell_wall_sec=50.,model_startup_sec=2.,native_elapsed_sec=45.)
    running=progress(jobs,[measured],[measured],100.,'two',10.)
    assert running['completed_cell_source_seconds_per_wall_second']==.8 and running['invocation_source_seconds_per_elapsed_second']==.4 and running['heuristic_eta_range_sec']==[30.,60.];count+=1
    reused=progress(jobs,[measured],[],100.)
    assert reused['completed_source_sec']==40 and reused['new_source_sec_this_invocation']==0 and reused['completed_cell_source_seconds_per_wall_second'] is None and reused['heuristic_remaining_wall_sec']==50;count+=1
    assert progress(jobs,[measured],[measured],100.,'two',100.)['heuristic_remaining_wall_sec']==5.;count+=1
    try:progress(jobs,[measured,measured],[],100.)
    except ValueError:count+=1
    else:raise AssertionError('Duplicate progress completion accepted')
    assert phase_observations([],40.)['exact_eof_drain_sec'] is None;count+=1
    samples=[dict(elapsed_from_native_launch_sec=t,telemetry=dict(source_duration_sec=c),phase=phase) for t,c,phase in ((39.,38.,'periodic'),(41.,40.,'periodic'),(43.,40.,'terminal_after_finalization'))]
    observed=phase_observations(samples,40.)
    assert observed['full_cursor_to_terminal_interval_sec']==[2.,4.] and observed['exact_eof_drain_sec'] is None and observed['max_sample_gap_sec']==2.;count+=1
    try:phase_observations(list(reversed(samples)),40.)
    except ValueError:count+=1
    else:raise AssertionError('Reordered phase clock accepted')
    final=phase_observations([samples[-1]],40.);assert final['full_cursor_to_terminal_interval_sec']==[0.,None];count+=1
    gate=selected_panel(dict(**p,family_native_gate_case_ids=GATE_CASES),'gate6_diagnostic',['C071','C082'])
    assert len(grid(['C071','C082'],gate))==24 and gate['repeated_case_ids']==[];count+=1
    for mode,ids in (('gate6_diagnostic',['C071']),('gate6_diagnostic',['C082','C071']),('other',['C071','C082'])):
        try:selected_panel(dict(**p,family_native_gate_case_ids=GATE_CASES),mode,ids)
        except ValueError:count+=1
        else:raise AssertionError('Unsupported diagnostic mode/profile set admitted')
    count+=reuse_checks()
    return dict(status='PASS_MODEL_FREE',checks=count,model_calls=0,native_calls=0,panel_cells_per_candidate=40,scope='Pure grid/source/route guards; final candidate IDs and native execution are deferred.')


def reuse_checks():
    count=0;parent=L.STAGING/'paced_candidates/guard_fixtures';parent.mkdir(parents=True,exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='reuse_',dir=parent) as temp:
        root=Path(temp)
        def save(path,value):
            path.parent.mkdir(parents=True,exist_ok=True);path.write_text(json.dumps(value,allow_nan=False),encoding='utf-8');return L.bind(path)
        def reject(call):
            try:call()
            except (ValueError,KeyError):return 1
            raise AssertionError('Invalid completed chain admitted')
        pb=save(root/'MANIFEST.json',{'fixture':True});plan=dict(execution_manifest={});spec=dict(gallery_index=None)
        raw=b'\0'*8;sha=hashlib.sha256(raw).hexdigest();source=dict(duration_sec=4/16000,duration_samples=4,pcm_sha256={'O0':sha,'O1':sha})
        sb=save(root/'source.json',source);owner=dict(pid=12345,creation_time=10.,argv=['python','worker'])
        job=dict(job_id='cell',job_key='key',candidate_id='C071',case_id='case',asr_tap='O0',identity_tap='O0',repetition=1,source=sb,profile_row={'fixture':'exact'},gallery_row=None,report_root=str(root/'job'),payload_root=str(root/'payload'))
        folder=Path(job['report_root']);out=native_folder(job);session=Path(job['payload_root'])/out.relative_to(folder)/'sessions/session1';session.mkdir(parents=True)
        journals={}
        for name in ('audio_spool.pcm16','identity_audio_spool.pcm16'):
            (session/name).write_bytes(raw);journals[name]=L.bind(session/name)
        closure=save(session/'session_finalization_v3.json',dict(state='COMPLETED',live_lanes_at_finalization=[],resident_bundle_lease_retained=False,event_and_transcript_handles_closed=True))
        phases=[dict(elapsed_from_native_launch_sec=1.,telemetry=dict(source_duration_sec=source['duration_sec']),phase='terminal_after_finalization')]
        out.mkdir(parents=True);(out/'PROCESS_SAMPLES.jsonl').write_text(json.dumps(phases[0])+'\n',encoding='utf-8')
        native=dict(schema='s6c_continuous_paced_native.v1',status='COMPLETE',composition=sb,profile=job['profile_row'],owner=owner,source_duration_sec=source['duration_sec'],long_session_gallery_condition=None,gallery_index=None,resident_bundle_loads=1,resident_sessions_created=1,live_owned_lanes=[],hardware_invocations=0,final_telemetry=dict(asr_cursor_sec=source['duration_sec']),model_load_sec=.1,native_elapsed_sec=1.,total_observed_worker_sec=1.2,native_journals=journals,native_artifacts=[*journals.values(),closure],process_samples=L.bind(out/'PROCESS_SAMPLES.jsonl'),process_sample_count=1)
        nb=save(out/'RESULT.json',native)
        cell=dict(schema='s6c-canonical-paced-cell-result.v1',status='COMPLETE',manifest=pb,job=job,owner=owner,source=sb,source_kind='CANONICAL_SINGLE_SCENE_PAIR',actual_execution_epoch='epoch4',execution_manifest=plan['execution_manifest'],source_offset_samples=0,inserted_gap_samples=0,original_native_function_unchanged=True,native_result=nb)
        cb=save(folder/'CELL_RESULT.json',cell)
        admission=dict(status='STARTED',owner=owner,manifest=pb,job=job,source=sb,source_kind='CANONICAL_SINGLE_SCENE_PAIR',actual_execution_epoch='epoch4',execution_manifest=plan['execution_manifest']);save(folder/'CELL_ADMISSION.json',admission)
        launch=dict(job_key='key',manifest=pb,pid=owner['pid'],creation_time=owner['creation_time'],argv=owner['argv']);save(folder/'LAUNCH.json',launch)
        outcome=dict(status='NATIVE_RETURNED',owner=owner,manifest=pb,job_key='key',native_result=nb,error=None,protected_functions_restored=True);save(folder/'CELL_OUTCOME.json',outcome)
        (folder/'PROCESS_TREE_SAMPLES.jsonl').write_text(json.dumps(dict(process=dict(processes=[owner])))+'\n',encoding='utf-8')
        *_,artifacts=read_cell_chain(job,plan,pb,spec)
        old=dict(status='COMPLETE',job_key='key',job_id='cell',all_owned_processes_closed=True,source_kind='CANONICAL_SINGLE_SCENE_PAIR',cell_result=cb,native_result=nb,artifacts=artifacts,owned_processes=[owner],external_process_samples=1,measurement=completed_measurement(job,native,2.))
        assert validate_complete(old,job,plan,pb,spec,lambda p,c:False)==old['measurement'];count+=1
        for alive in (True,None):count+=reject(lambda:validate_complete(old,job,plan,pb,spec,lambda p,c:alive))
        for field,value in (('cell_result',{}),('native_result',{}),('artifacts',artifacts[:-1]),('job_id','foreign'),('external_process_samples',9),('owned_processes',[])):
            bad=deepcopy(old);bad[field]=value;count+=reject(lambda:validate_complete(bad,job,plan,pb,spec,lambda p,c:False))
        for field,value in (('case_id','foreign'),('source_sec',20.),('model_startup_sec',7.),('phase_observations',{}),('completed_cell_wall_sec',float('nan')),('completed_cell_wall_sec',True),('completed_cell_wall_sec',0.)):
            bad=deepcopy(old);bad['measurement'][field]=value;count+=reject(lambda:validate_complete(bad,job,plan,pb,spec,lambda p,c:False))
        # A byte-valid foreign cell, under the current caller's key, is rejected
        # by the actual chain reader before outer artifact self-consistency.
        for field,value in (('job',{**job,'case_id':'foreign'}),('manifest',{}),('source',{}),('owner',{**owner,'pid':54321}),('source_offset_samples',1),('original_native_function_unchanged',False)):
            bad=deepcopy(cell);bad[field]=value;save(folder/'CELL_RESULT.json',bad)
            count+=reject(lambda:validate_complete(old,job,plan,pb,spec,lambda p,c:False));save(folder/'CELL_RESULT.json',cell)
        for field,value in (('status','STARTED'),('composition',{}),('profile',{}),('source_duration_sec',1.),('gallery_index',{}),('long_session_gallery_condition',{}),('resident_sessions_created',2),('live_owned_lanes',['x'])):
            bad=deepcopy(native);bad[field]=value;count+=reject(lambda:validate_native_identity(bad,job,source,spec,owner))
        bad=deepcopy(native);bad['native_journals']['audio_spool.pcm16']['sha256']='wrong';count+=reject(lambda:validate_native_identity(bad,job,source,spec,owner))
        for name,original,field,value in (('CELL_ADMISSION.json',admission,'job',{}),('LAUNCH.json',launch,'pid',54321),('CELL_OUTCOME.json',outcome,'protected_functions_restored',False)):
            bad=deepcopy(original);bad[field]=value;save(folder/name,bad);count+=reject(lambda:read_cell_chain(job,plan,pb,spec));save(folder/name,original)
        assert validate_complete(old,job,plan,pb,spec,lambda p,c:False)==old['measurement'];count+=1
        called=[]
        def release(*args):called.append(True);return {'status':'RELEASED'}
        for alive in (True,None):
            result=release_after_closure(None,None,None,[dict(alive=alive)],release);assert not called and result['released'] is False;count+=1
        assert release_after_closure(None,None,None,[dict(alive=False)],release)['status']=='RELEASED' and called;count+=1
        def denied(p,c):raise psutil.AccessDenied(p)
        states=owned_states({12345:10.},denied);assert states[0]['alive'] is None and states[0]['inspection_error'];count+=1
    return count


@L.observer_entry
def source_checks():
    pure=checks();driver,common,spec,eb=load_native();p,pb=panel();index,ib=L.read_bound(spec['input_index']['path'],spec['input_index']);lookup={(r['case_id'],r['stream']):r for r in index['rows']};verified=[]
    for case in p['case_ids']:
        pair={tap:lookup[case,tap] for tap in ('O0','O1')};record=source_record(pair,ib)
        for row in pair.values():verify_pcm(row)
        L.verify(record['telemetry']);verified.append(record)
    prior,prior_binding=L.read_bound(REPORT/'paced_candidates/SOURCE_CHECKS_V2.json');history=[]
    for name in ('s6c_paced_epoch4.py','README_S6C_PACED_EPOCH4.md'):
        old=next(b for b in prior['sources'] if Path(b['path']).name==name);saved=L.bind(L.STAGING/'paced_candidates/pre_reuse_and_gate6_v2'/name)
        if any(old[k]!=saved[k] for k in ('bytes','sha256')):raise ValueError('Prior progress-version source snapshot differs')
        history.append(dict(original=old,snapshot=saved))
    result=dict(schema=SCHEMA,status='PASS_MODEL_FREE_SOURCE_ADMISSION',created_utc=L.utc(),checks=pure,execution_manifest=eb,panel=pb,input_index=ib,sources=source_bindings(),
        previous_source_checks=prior_binding,preserved_previous_sources=history,correction_scope='Re-admit exact original completion/native/artifact/measurement identity, retain lease for live or unverified owned children, and add the explicit original C071/C082 gate6 diagnostic selection. Native body, default 16+4 grid, model/profile/source mapping unchanged.',
        original_native_function=L.protected_identities(driver)['native'],verified_canonical_sources=verified,verified_views=len(verified)*2,
        actual_view_duration_sec=sorted({r['duration_sec'] for r in verified}),source_offset_samples=0,inserted_gap_samples=0,new_audio_files=0,model_calls=0,native_calls=0,
        candidate_selection='DEFERRED; no candidate manifest prepared',scope='Exact canonical full PCM/source-clock admission and grid fixtures only. This does not establish paced runtime completion or currently quiet resources.')
    return common.save(REPORT/'paced_candidates/observer_fast_v1/SOURCE_CHECKS_V1.json',result,immutable=True)


if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__,allow_abbrev=False);p.add_argument('action',choices=('checks','source-checks','prepare','run','worker'));p.add_argument('--namespace');p.add_argument('--candidates');p.add_argument('--panel-mode',choices=('full16_plus4','gate6_diagnostic'),default='full16_plus4');p.add_argument('--deadline-utc');p.add_argument('--manifest',type=Path);p.add_argument('--quiet-admission',type=Path);p.add_argument('--job-id');p.add_argument('--owner-lease',type=Path);a=p.parse_args()
