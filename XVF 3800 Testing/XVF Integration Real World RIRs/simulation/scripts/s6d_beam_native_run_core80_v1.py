"""Future serial core96 V5/80 wrapper; see README_S6D_BEAM_CORE80_V1.md."""
...
RATE=16000
PINS={'protocol':'bdb9896cc3cdfefe85ea1602b904373ceb24de0de52965b0a66b3c56ecd1fe12',
...
def core_scope(manifest):
    """Restrict the additive budget epoch to the complete, serial two-mode core panel."""
    jobs=manifest.get('jobs',[]);modes={'same_pass_auto_control','mono_asr_beam_identity'}
    need(manifest.get('stage')=='main_core' and len(jobs)==96 and len({j['job_id'] for j in jobs})==96,'Exact96 core-only jobs required; C12/diagnostics remain separate')
    cells={}
    for job in jobs:
        need(job.get('mode') in modes and job.get('capture_profile')=='P_MAIN6' and job.get('asr_raw_stream')=='auto_asr_raw','Original MAIN core mode/input required')
        cells.setdefault(job['case_id'],[]).append(job['mode'])
    need(len(cells)==48 and all(len(values)==2 and set(values)==modes for values in cells.values()),'Each of48 cases needs both original matched core modes')
    limits=manifest['limits']
    need(limits.get('serial_jobs') is True and limits.get('maximum_resident_stacks')==1 and limits.get('maximum_identity_focus_states')==2 and limits.get('cpu_affinity')==[12,13,14,15] and limits.get('cpu_threads_each')==1,'Original serial model allocation required')
    need(type(limits.get('max_new_payload_gib')) is int and limits['max_new_payload_gib']==80,'This source epoch requires exact80GiB future authority')
...
def validate_multistream(result,job,finalization,closure,journals,dispatch):
    errors=[]
    def check(v,m):
...
def run(manifest_path,manifest_sha,job_id):
    ...
    try:
        ...
        errors=validate_multistream(result,job,finalization,closure,journals,dispatch)
        audit=dict(status='PASS_FULL_MULTISTREAM_EVIDENCE' if not errors else 'REJECTED',errors=errors,job_id=job_id,manifest=mb,physical_case_result=admission['original_case_result'],journals=journals,dispatch=dispatch,source_proofs=job['stream_proofs'],consumer_events=eb)
