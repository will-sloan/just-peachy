...
Inputs: exact epoch4 native source graph, original panel, explicit candidate IDs, canonical paired PCM and telemetry without trim/gap/gain changes, fixed A/B or no gallery, fresh namespace ending `_fast_v2`, deadline and later quiet admission. Default mode is 40 cells per candidate. The existing `gate6_diagnostic` mode remains exactly C071,C082 with six cases/both taps/one repeat (24 cells). No candidate selection changes. The original 512 MiB pending-cell reserve, 8-hour coordinator bound and all existing per-cell timeouts remain.

Outputs: original families `REPORT/paced_candidates/<namespace>` and `G:/Just_Peachy_S6C/20260910T123540Z/paced_candidates/<namespace>`. MANIFEST and sources are preparation only. Later actual cells retain LAUNCH/CELL_ADMISSION/CELL_RESULT/CELL_OUTCOME/COMPLETE/native payloads, coordinator PACED_INDEX and invocation lease closure. Actual argv names `s6c_paced_epoch4_fast_v2.py worker`, with the admitted manifest and job. Scanner restoration receipts are separate as described above. `source-checks` writes `REPORT/paced_candidates/observer_fast_v2/SOURCE_CHECKS_V1.json`, verifying exact PCM and assets/source bindings without neural calls.

PowerShell:
```powershell
$sim = 'C:\Users\amiri\Documents\GitHub\just-peachy\XVF 3800 Testing\XVF Integration Real World RIRs\simulation'
$edge = 'C:\Users\amiri\Documents\GitHub\just-peachy\.edge-speech-env\python.exe'
& $edge -B "$sim\scripts\s6c_paced_epoch4_fast_v2.py" checks
& $edge -B "$sim\scripts\s6c_paced_epoch4_fast_v2.py" source-checks
& $edge -B "$sim\scripts\s6c_paced_epoch4_fast_v2.py" prepare --namespace c065_control_fast_v2 --candidates C065 --deadline-utc 2026-09-13T11:35:40Z
# Only after exact quiet admission, replace the last path with its actual authorized file:
& $edge -B "$sim\scripts\s6c_paced_epoch4_fast_v2.py" run --manifest "$sim\reports\S6C\20260910T123540Z\paced_candidates\c065_control_fast_v2\MANIFEST.json" --quiet-admission C:\path\to\AUTHORIZED_QUIET_ADMISSION.json
```
