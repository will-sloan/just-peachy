# Serial paced dispatcher V2

This prospective V2 preserves V1 source, all manifests, and the failed dispatcher attempt. It addresses the observed heartbeat `os.replace` WinError5 without asserting that a concurrent reader caused that particular failure. The original C067 coordinator can continue after dispatcher failure; this helper must not be launched to take it over or to resume that namespace. Root must resolve the existing invocation before any later separately authorized queue.

## Narrow heartbeat repair

Only replacement of `HEARTBEAT.json.tmp` by sibling `HEARTBEAT.json` retries `PermissionError` with Windows error 5 (access denied), 32 (sharing violation), or 33 (lock violation). Access denied may be permanent: the error class permits a bounded retry, never an inference of eventual success. The payload is serialized and written once; retries reuse exactly the same temporary path and bytes. No retry deletes or rewrites either file. Success remains atomic `os.replace`.

The limit is 2 seconds of monotonic elapsed time, 50ms between attempts, and at most 41 replace attempts. Exhaustion re-raises the latest error and preserves the temporary file; an existing temporary file is still refused. Other exceptions, other Windows error numbers, non-heartbeat replacements and immutable writes retain immediate failure behavior. OS calls and scheduler pauses cannot be preempted by this user-space budget. This can delay the next existing stop/deadline observation by the bounded retry interval; it does not extend any authority or native deadline.

The only pre-existing function changes are `save`'s heartbeat branch and `run`'s V2 README binding. All other dispatch, finite queue, wrapper, source, owner, closure, stop and native semantics remain exact V1. New execution requires a fresh namespace and root authority binding this V2 source; no authority is created by the test command.

`test_s6c_paced_dispatch_v2.py` shares this maintained README. Its inputs are the two source versions, this README and a fresh output directory. Outputs are one bound `SOURCE_CHECKS.json`. It checks original AST continuity, all three allowed error numbers, exact temporary bytes, permanent errors, elapsed/attempt bounds, retained temporary files, and immutable/non-heartbeat behavior. A real Windows private temporary file is opened for reading without delete sharing; replacement first fails and then succeeds after the test's owned handle is released. It never opens a runtime heartbeat or coordinator and never launches a subprocess/model. The actual Windows test is required and reports failure if unavailable. Commands below run these tests; use a new output directory each time.

`s6c_paced_dispatch_v2.py` launches a finite, explicitly approved queue of existing paced batch runners one at a time. It does not select candidates, alter their manifests, change resource/model/timing settings, perform scientific analysis or grant native execution permission. Source checks are implemented in `test_s6c_paced_dispatch_v2.py`.
...
```json
{
  ...
  "dispatcher": {"path": "ABSOLUTE_s6c_paced_dispatch_v2.py", "bytes": 123, "sha256": "EXACT_SHA"},
...
& $edgePy -B test_s6c_paced_dispatch_v2.py --output '..\reports\S6C\20260910T123540Z\serial_paced_dispatcher\source_checks_v2_NEW'
# Run only with root's exact reviewed queue and authority; replace all placeholders.
& $edgePy -B s6c_paced_dispatch_v2.py run --queue 'ABSOLUTE_QUEUE.json' EXACT_QUEUE_SHA --authority 'ABSOLUTE_ROOT_AUTHORITY.json' EXACT_AUTHORITY_SHA
```
...
"C:\Users\amiri\Documents\GitHub\just-peachy\.edge-speech-env\python.exe" -B test_s6c_paced_dispatch_v2.py --output "..\reports\S6C\20260910T123540Z\serial_paced_dispatcher\source_checks_v2_NEW"
"C:\Users\amiri\Documents\GitHub\just-peachy\.edge-speech-env\python.exe" -B s6c_paced_dispatch_v2.py run --queue "ABSOLUTE_QUEUE.json" EXACT_QUEUE_SHA --authority "ABSOLUTE_ROOT_AUTHORITY.json" EXACT_AUTHORITY_SHA
```

The fixture command produces only a source-bound `SOURCE_CHECKS.json` and private temporary heartbeat/immutable files, with an owned Windows sharing handle. Popen is never called. It starts no batch or model. Choose a fresh check output path every time.
