"""Recorded B3 recovery fixtures; README_S6D_CLOSED_TELEMETRY_RESTORE_V5.md."""
...
def main():
    ...
    m=load(a.source_root/'s6d_closed_telemetry_restore_v5.py','recovery5');rows=[]
    batch=m.R/'hardware_batches/bank_v3_P_MAIN6_B3';case=Path('G:/Just_Peachy_S6D/20260913T195357Z/bank_captures_v1/beam_bank/S45_04_19/P_MAIN6/P_MAIN6_S45_04_19');td=case/'telemetry'
    ...
    original.update(ledger=m.bind(m.R/'physical_ledger.json'),bridge_failure=m.bind(m.R/'physical_supervisor/bank_v3_P_MAIN6_B3/CAPTURE_BRIDGE_FAILURE.json'),supervisor_launch=m.bind(m.R/'runner/bank_queue_v4/ROOT_LAUNCH_V1.json'),failed_configuration=m.bind(case/'configuration.json'),failed_capture_metadata=m.bind(case/'capture_metadata.json'))
    ...
    proof=m.recorded_terminal_closure(refs,2390,bridge,summary)
    ...
    def context(label):
        review=dict(status='PROPOSED_SOURCE_REVIEW_ONLY',fixture_only=True,allow_fresh_exposed_restore=False,owner_thread_id=m.ROOT_THREAD,run_id='20260913T195357Z',original_batch='bank_v3_P_MAIN6_B3',recovery_kind=m.KIND,historical_telemetry_pid_creation=proof['historical_telemetry_pid_creation'],original_bindings=original,source_bindings=[m.bind(x) for x in m.required_sources()],telemetry_bindings=refs,expected_measurement_rows=2390,recovery_output_root=str(out/label))
    ...
    check('actual_recorded_terminal_native2390_and_parent_binding',lambda:equal(proof['historical_telemetry_pid_creation'],dict(pid=609852,creation_time=1789410880.3176687)))
    ...
    def changed_ref(key,mutate):
        ...
        reject(lambda:m.recorded_terminal_closure(changed,2390,bridge,summary))
    ...
    check('wrong_native_population_rejected',lambda:reject(lambda:m.recorded_terminal_closure(refs,2381,bridge,summary)))
    def lifecycle_change():
        events=m.json_lines(refs['lifecycle']['path']);events[5]['monotonic_ns']=events[2]['monotonic_ns']-1
        path=out/'bad_lifecycle.jsonl';path.write_text(''.join(json.dumps(x)+'\n' for x in events));changed=dict(refs,lifecycle=m.bind(path));reject(lambda:m.recorded_terminal_closure(changed,2390,bridge,summary))
    ...
    check('recorded_telemetry_alive_rejected_even_without_marker',lambda:reject(lambda:m.scan_decision(census(609852,1789410880.3176687),known,current)))
    check('reused_pid_is_not_historical_identity',lambda:m.scan_decision(census(609852,1789410881.0),known,current))
    ...
    def verifier(change=None):
        ...
        if change=='kind':rec['recovery_kind']='closed_native_telemetry_fresh_restore'
    ...
    check('file_only_verifier_recomputes_recorded_failure_full_restore',lambda:equal(verifier()['status'],'VERIFIED_CLOSED_RECORDED_TELEMETRY_FRESH_RESTORE'))
    for field in ('history','known','preidentity','kind'):check('verifier_rejects_'+field,lambda field=field:reject(lambda:verifier(field)))
    def owner_dispatch(mode):
        tree=ast.parse((a.source_root/'s6d_capture_owner_v10.py').read_bytes());node=next(x for x in tree.body if isinstance(x,ast.FunctionDef) and x.name=='prior_closure')
        ...
        def v5(receipt,**kwargs):
            calls.append('v5');equal(kwargs,dict(owner_binding=own,restoration_binding=failed,initial_binding=initialref))
            if mode=='invalid':raise ValueError('Rejected exact new recovery')
            return dict(status='VERIFIED_CLOSED_RECORDED_TELEMETRY_FRESH_RESTORE',old_failure_preserved=True,historical_python_terminal_receipt_present=True)
        ...
        modules={'s6d_closed_telemetry_restore_v5':types.SimpleNamespace(verify_recovery_record=v5),'s6d_closed_telemetry_restore_v4':types.SimpleNamespace(verify_recovery_record=v4)}
        with patch.dict(sys.modules,modules):
            if mode=='invalid':reject(lambda:scope['prior_closure'](local,[ref]))
            else:scope['prior_closure'](local,[ref])
        equal(calls,['v4'] if mode=='old_kind' else ['v5'])
    for mode in ('valid','invalid','old_kind'):check('owner_v10_'+mode+'_exact_dispatch',lambda mode=mode:owner_dispatch(mode))
    def retained():
        ...
        old=functions(a.source_root/'s6d_closed_telemetry_restore_v4.py');new=functions(a.source_root/'s6d_closed_telemetry_restore_v5.py')
        ...
        old=functions(a.source_root/'s6d_capture_owner_v9.py');new=functions(a.source_root/'s6d_capture_owner_v10.py')
        equal(set(old),set(new))
        for key in set(old)-{'validate_plan','prior_closure'}:equal(old[key],new[key])
        val=next(n for n in tree(a.source_root/'s6d_capture_owner_v10.py').body if isinstance(n,ast.FunctionDef) and n.name=='validate_plan')
