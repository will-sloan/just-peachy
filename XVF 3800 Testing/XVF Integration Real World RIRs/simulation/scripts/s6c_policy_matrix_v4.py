"""Shared v3 policy matrix over compatible genuine S6C neural evidence. README_S6C_POLICY_MATRIX_V4.md."""
...
def assert_compatible_epochs(target,source):
    import ast
    for epoch_spec in (target,source):
        if epoch_spec['execution_digest']!=digest({k:epoch_spec[k] for k in ('execution_files','assets','versions','state_policy')}):
            raise ValueError('Execution manifest digest does not describe its actual source/model graph')
        registry=verified(epoch_spec['effective_profile_registry'])
        if registry.get('status')!='VALIDATED_REAL_V3_API' or epoch_spec['profiles']!=registry['profiles']:
            raise ValueError('Execution embedded profiles differ from their admitted registry')
    for b in source['execution_files']+source['assets']:bind(b['path'],b['sha256'])
    def app_files(spec):
    ...
    if app_files(target)!=app_files(source) or not app_files(target):raise ValueError('Cross-epoch application bytes differ')
    for key in ('assets','versions','python','input_index','scene_manifest','state_policy'):
        if target[key]!=source[key]:raise ValueError('Cross-epoch neural/source authority differs: '+key)
    def extraction(spec):
        file=next(b for b in spec['execution_files'] if Path(b['path']).name=='s6c_execution.py')
        tree=ast.parse(Path(file['path']).read_text(encoding='utf-8'))
        return ast.dump(next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='extract'),include_attributes=False)
    if extraction(target)!=extraction(source):raise ValueError('Cross-epoch raw evidence extraction differs')
    # Strong boundary for additive epoch4 and original epoch2. The entire
    # native worker module and common utilities stay byte-identical, including
    # module-level imports/environment and any helper used by those functions.
    for name in ('s6c_execution.py','s6c_common.py'):
        left=next(b for b in target['execution_files'] if Path(b['path']).name==name)
        right=next(b for b in source['execution_files'] if Path(b['path']).name==name)
        if left['sha256']!=right['sha256'] or left['bytes']!=right['bytes']:
            raise ValueError('Cross-epoch complete native worker/common module differs: '+name)
...
def run(epoch,indices,label,panel='challenge',candidates=None,source_epochs=None):
    ...
    assert_compatible_epochs(spec,spec)
    ...
    source_specs={spec['execution_digest']:spec};source_epoch_bindings=[bind(spec_path)]
    for source_epoch_path in source_epochs or []:
        binding=bind(source_epoch_path);other=verified(binding)
        assert_compatible_epochs(spec,other)
        if other['execution_digest'] in source_specs and source_specs[other['execution_digest']]!=other:raise ValueError('Unequal source manifests share an execution digest')
        source_specs[other['execution_digest']]=other;source_epoch_bindings.append(binding)
    ...
    for source_index in sources:
        ...
        for row in rows:
            ...
            if receipt['status']!='COMPLETE' or identity['execution_digest'] not in source_specs:raise ValueError('Wrong/unadmitted native epoch/status')
    ...
    plan_binding=save(REPORT/epoch/(label+'_POLICY_PLAN.json'),dict(status='REGISTERED_EXACT_POLICY_ROWS',epoch=epoch,source_indices=sources,
        helper=bind(__file__),execution_manifest=bind(spec_path),compatible_source_epochs=source_epoch_bindings,panel=panel,candidate_ids=sorted({p['candidate_id'] for p in selected}),case_ids=case_ids,
    ...
    for cid,p,source,gal_binding,cue_binding,n in planned:
        ...
        if source['sha256'] not in evidence_cache:
            evidence,vectors,_=load_evidence(source['path'],verified(source)['identity']['execution_digest'],expected_receipt=source)
        ...
        identity=dict(schema='jp_s6c_policy_prediction.v1',execution_digest=spec['execution_digest'],profile=p['profile'],source=source,
            ...
            oracle_like=p['cue_condition']=='NOMINAL_GEOMETRY_DIAGNOSTIC',orchestrator=bind(__file__),compatible_source_epochs=source_epoch_bindings)
...
if __name__=='__main__':
    ...
    p.add_argument('--label',required=True);p.add_argument('--panel',choices=('challenge','all'),default='challenge');p.add_argument('--candidates',nargs='*');p.add_argument('--source-epochs',nargs='*')
    a=p.parse_args();print(json.dumps(run(a.epoch,a.indices,a.label,a.panel,a.candidates,a.source_epochs),indent=2))
