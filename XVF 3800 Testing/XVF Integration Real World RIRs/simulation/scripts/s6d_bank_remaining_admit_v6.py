...
SKIP_GROUPS=set()
RENAMES={'QA_P_MAIN6_B3_PRE_R2':'QA_P_MAIN6_B3_PRE_R3'}
...
def execution_sources_from_freeze(review,freeze_binding,expected_sha256,required_members):
    A.require(len(expected_sha256)==64 and freeze_binding['sha256']==expected_sha256 and review.get('owner_execution_source_freeze')==freeze_binding,'Root must bind the exact supplied V11 execution source freeze/hash')
    ...
    A.require(len(sources)==len(dependencies)==28 and all(b in sources for b in required_members),'Exact28 collision-free canonical execution dependencies including owner/bridge/policy required')
    return sources
...
def main():
    ...
    ap.add_argument('--output',type=Path,default=A.R/'runner/bank_queue_v6')
    ...
    A.require(review['status']=='ROOT_ACCEPTED_RECORDED_QA_TELEMETRY_RECOVERY_V6_SOURCES_V1','Root V11 source review required')
    ...
    A.require(Path(owner['path']).name=='s6d_capture_owner_v11.py' and Path(bridge['path']).name=='s6d_capture_supervisor_bridge_v2.py','Exact V11/bridgeV2 epoch')
    ...
    helper_binding=A.bind(__file__);readme_binding=A.bind(Path(__file__).with_name('README_S6D_BANK_REMAINING_ADMIT_V6.md'))
    ...
    olddir=A.R/'runner/bank_queue_v5';oldref=A.bind(olddir/'ROOT_QUEUE_REVIEW.json')
    A.require(oldref['sha256']=='2f79e4251ef1e6905daf9b14e8f09b340a73d5a51065194634b77d87c28e228b','Exact interrupted bank admission')
    ...
    A.require(len(passes)==121 and sum(x['status']=='PASS' for x in passes)==118 and sum(x['status']=='FAIL' for x in passes)==3,'Exact121 history')
    A.require({x['attempt_id'] for x in passes if x['status']=='FAIL'}=={'P_MAIN6_S45_01_17','P_MAIN6_S45_04_19','QA_P_MAIN6_B3_PRE_R2'},'All three original telemetry failures preserved')
    ...
    spec=importlib.util.spec_from_file_location('reviewed_v11_closure',owner['path'])
    ...
    A.require(cp['status']=='REPORT_BLOCKED' and cp['active']['job_id']=='bank_v4_P_MAIN6_B3_pre_QA','Exact interrupted B3 replacement QA state')
    expected_closed=set()
    A.require(set(cp['completed'])==expected_closed,'No completed stages in interrupted bankV5')
    ...
    for group in old['groups']:
        ...
        for original in plan['attempts']:
            ...
            if ident in RENAMES:
                row.update(attempt_id=RENAMES[ident],retry_of_attempt_id=ident,
                    retry_reason='Preserved QA_PRE_R2 telemetry-finalization FAIL; exact source repeated as fresh pre-QA')
        ...
        auth.update(plan_sha256=pb['sha256'],source_bindings=sources,reviewed_recoveries=recoveries,
            ...
            partial_pass_transport_review=partial,scope='Retain60 earlier canonical MAIN plus18 B3 transport-verified PASS captures; replace failed B3 pre-QA_R2 with fresh pre-QA_R3, retain unstarted04_19_R2 and every other pending row. All three failures/charges preserved. No new waveform/gain/guard/profile/timing policy.')
    ...
    A.require(total==Fraction('19818.0819375') and len(passes)+len(all_rows)==433,'Exact433/19818.0819375 forecast')
    previous=None
    for job in queue['jobs']:
        oldid=job['job_id'];A.require(oldid.startswith('bank_v4_'),'Expected old job epoch')
        job['job_id']='bank_v5_'+oldid[len('bank_v4_'):]
    ...
    receipt=dict(status='ROOT_ADMITTED_REMAINING_BANK_AFTER_TELEMETRY_RECOVERY_PENDING_LAUNCH',utc=dt.datetime.now(dt.timezone.utc).isoformat(),
        ...
        retained_pass_transport_review=partial,retained_pass_attempt_ids=sorted(DONE),current_charged_attempts=121,new_attempts_admitted=312,previous_closed_stages=previous_closed,previous_checkpoint=cp_ref,excluded_completed_groups=sorted(SKIP_GROUPS),
        total_forecast_attempts=433,total_forecast_charged_seconds=float(total),original_failures_preserved=True,playback_started=False)
