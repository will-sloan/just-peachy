"""Exploratory arrival sentinel; see README_S6C_PACED_ARRIVAL_SENTINEL_FAST_V1.md."""
...
HELD=Path(__file__).with_name('s6c_paced_epoch4_fast_v1.py')
if hashlib.sha256(HELD.read_bytes()).hexdigest()!='c13ed3849fb86aeb50341d823c2969b5084cd6e90aa8fee4c24785405e8d20c4':raise ValueError('Held canonical adapter changed')
...
def source_bindings():
    names=('s6c_paced_arrival_sentinel_fast_v1.py','README_S6C_PACED_ARRIVAL_SENTINEL_FAST_V1.md','s6c_paced_epoch4_fast_v1.py','README_S6C_PACED_EPOCH4_FAST_V1.md','s6c_long_native_epoch4_fast_v1.py','README_S6C_LONG_NATIVE_EPOCH4_FAST_V1.md','s6c_long_session.py','README_S6C_LONG_SESSION.md','s6c_native_prefix.py','s6c_historical_fast_observer_v1.py','README_S6C_HISTORICAL_FAST_OBSERVER_V1.md')
...
def namespace_roots(namespace):
    if not re.fullmatch(r'[A-Za-z0-9_-]{1,60}',namespace) or not namespace.endswith('_fast_v1'):raise ValueError('Simple fresh sentinel namespace required')
...
def quiet(excluded=()):
    ...
    for p in psutil.process_iter(['pid','cmdline','create_time']):
        ...
        if L.worker_command(argv) or (bool({'s6c_paced_epoch4.py','s6c_paced_arrival_sentinel_v1.py','s6c_paced_epoch4_fast_v1.py','s6c_paced_arrival_sentinel_fast_v1.py'}&names) and any(v in argv for v in ('run','worker'))):
...
def checks():
    import ast
    import s6c_paced_epoch4_fast_v1 as baseline
    if Path(baseline.__file__).resolve()!=Path(__file__).with_name('s6c_paced_epoch4_fast_v1.py').resolve():raise ValueError('Wrong held adapter import')
...
def source_checks():
    ...
    return common.save(REPORT/'paced_arrival_sentinel/observer_fast_v1/SOURCE_CHECKS_V1.json',result,immutable=True)
