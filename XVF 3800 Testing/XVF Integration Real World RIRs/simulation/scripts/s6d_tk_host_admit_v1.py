...
def context(group):
    ...
    return dict(group=group,queue=qb,approval_proposal=ab,q=q,a=a,manifests=manifests,runner=bound(RUNNER),allocation_source=bound(ALLOC),credits=bound(CREDITS),C12=bound(C12),physical_bank=bound(BANK),source=bound(__file__),readme=bound(Path(__file__).with_name('README_S6D_TK_HOST_ADMIT_V1.md')))
...
def plan(c):
    return dict(schema='s6d-tk-host-group-admission-plan.v1',status='PROPOSED_NOT_ADMITTED',group=c['group'],queue=c['queue'],approval_proposal=c['approval_proposal'],runner=c['runner'],allocation_source=c['allocation_source'],native176_68_acceptance=c['credits'],source=c['source'],readme=c['readme'],
        ...
        source_manifests=[x[0] for x in c['manifests'].values()],sequence=['C12 runtime closure','physical bank closure','tk8','Tk8 full-source/UI/owner closure','host2','HOST2 full-source/correctness/owner closure'],
        maximum_NN_stacks=1,physical_supervisor_overlap=False,affinity=[12,13,14,15],threads_each=1,payload_cap_bytes=40*2**30,
        no_80GiB_exception=True,root_proof_required='Exact root-reviewed completed C12 acceptance; HOST additionally exact root-reviewed Tk8 whole-source/UI/owner acceptance. No receipt status is inferred from a partial output.',
...
def review_gate(review,c):
    ...
    bank=verified(proofs['PhysicalBank']['binding'])
    need(isinstance(proofs['PhysicalBank'].get('expected_status'),str) and proofs['PhysicalBank']['expected_status'].startswith('ROOT_ACCEPTED_') and bank.get('status')==proofs['PhysicalBank']['expected_status'],'Actual root-selected bank closure required')
    need(bank.get('owner_thread_id')==THREAD and bank.get('run_id')=='20260913T195357Z' and bank.get('queue')==c['physical_bank'] and bank.get('all_owners_closed') is True,'Exact physical bank closure required')
    known=known+closed_instances(bank)
    if c['group']=='host2':
...
def prepare(directory):
    ...
    for group,p in plans.items():
        ...
        proofs=dict(C12=p['C12_acceptance'],PhysicalBank=dict(binding=None,expected_status=None))
