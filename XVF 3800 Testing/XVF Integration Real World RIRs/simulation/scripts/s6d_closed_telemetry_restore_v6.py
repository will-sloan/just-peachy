"""B3 pre-QA recorded-terminal fresh restore; see README_S6D_CLOSED_TELEMETRY_RESTORE_V6.md."""
...
KIND='closed_recorded_qa_telemetry_fresh_restore'
REVIEW_STATUS='ROOT_ACCEPTED_RECORDED_QA_TELEMETRY_RECOVERY_V6_SOURCES_V1'
...
def required_sources():
    producer=Path('G:/Just_Peachy_S6D/20260913T195357Z/bank_captures_v1/beam_bank/tagged_C_sentinel/P_INPUT_QA6/QA_P_MAIN6_B3_PRE_R2/telemetry/source')
    return [Path(__file__),Path(__file__).with_name('README_S6D_CLOSED_TELEMETRY_RESTORE_V6.md'),SIM/'scripts/s6d_restoration_policy_v1.py',*[SIM/'scripts'/n for n in ('s3_hardware.py','s4_restore.py','s4_common.py','s0_common.py')],ROOT/'measurement_app/core.py',ROOT/'measurement_app/__init__.py',*[ROOT/'tools/xvf321/binary/host_v3.0.0/win32'/n for n in ('xvf_host.exe','device_usb.dll','command_map.dll')],*[producer/n for n in ('s6d_telemetry_v2.py','S6DQueuedTelemetry.cs','Run-S6D-Telemetry.ps1','s6d_capture_transport.py')]]
...
def supervisor_launch_proof(originals,values):
    """Bind this failed QA to the exact bankV5 root and owned child launch."""
    root=R/'runner/bank_queue_v5';ref=originals['supervisor_launch']
    need(Path(ref['path']).resolve()==root/'ROOT_LAUNCH_V1.json' and ref['sha256']=='d29524d67cdf3cb40de289e56838c96c63a776e4d0a7fa6a0ebe33e6bf03ecfe','Exact actual QA supervisor launch required')
    launch=verify(ref);need(launch==values['supervisor_launch'],'Supervisor launch values differ from bound bytes')
    need(launch.get('status')=='SUPERVISOR_LAUNCHED' and type(launch.get('pid')) is int and launch['pid']==638164 and positive(launch.get('creation_time')) and launch['creation_time']==1789415331.064818,'Exact QA supervisor PID/creation required')
    qref=launch['queue'];aref=launch['approval']
    need(Path(qref['path']).resolve()==root/'QUEUE.json' and qref['sha256']=='04d024889deb327f36b2e16c455d276c4766ec89bab12741cdf95c733cacd82e' and Path(aref['path']).resolve()==root/'APPROVAL.json' and aref['sha256']=='afcc46a83ff2556853b6d5e807346087330fc4b8732c0a1595b6a64821608935','Exact launched QA queue/approval required')
    queue=verify(qref);approval=verify(aref)
    need(queue.get('run_id')=='20260913T195357Z' and queue.get('owner_thread_id')==queue.get('owner_session_id')==ROOT_THREAD and approval.get('queue_sha256')==qref['sha256'],'QA queue root identity differs')
    child_ref=originals['capture_launch']
    need(Path(child_ref['path']).resolve()==root/'state/LAUNCH_9e237467322d4b13985ce9d7f81b2ad3.json' and child_ref['sha256']=='2394fb4fc2cdfea595513b74ca810863be4eeb72c748aae546a8d31088e1c80b','Exact failed QA child launch required')
    child=verify(child_ref);bridge=values['bridge_failure'];admission=values['admission']
    need(child.get('queue')==qref and child.get('launch_state')=='LAUNCHED' and child.get('shell') is False,'QA child launch queue/state differs')
    need(all(child.get(k)==bridge.get(k) for k in ('run_id','job_id','child_run_id','pid','creation_time')) and child.get('job_id')=='bank_v4_P_MAIN6_B3_pre_QA' and child.get('child_run_id')=='9e237467322d4b13985ce9d7f81b2ad3' and type(child.get('pid')) is int and child['pid']==639888 and positive(child.get('creation_time')) and child['creation_time']==1789415369.4817412,'Failed QA bridge identity differs from owned child launch')
    jobs=[j for j in queue['jobs'] if j.get('job_id')==child['job_id']];need(len(jobs)==1,'Unique exact QA queue job required');job=jobs[0]
    digest=hashlib.sha256(json.dumps(job,sort_keys=True,separators=(',',':'),allow_nan=False).encode()).hexdigest()
    need(child.get('job_sha256')==digest and digest in approval['approved_job_sha256'] and child.get('argv')==job['argv'] and child.get('cwd')==job['cwd'],'QA child literal job differs from approved queue')
    argv=job['argv']
    for key in ('plan','authorization'):
        need(admission[key]==bridge[key] and argv[argv.index('--'+key)+1]==admission[key]['path'] and argv[argv.index('--'+key+'-sha256')+1]==admission[key]['sha256'],'Failed QA admitted plan/authorization differs from launched job')
    need(argv[argv.index('--batch')+1]==child['job_id'] and argv[argv.index('--attempt-ids')+1:]==['QA_P_MAIN6_B3_PRE_R2'] and admission['attempt_ids']==['QA_P_MAIN6_B3_PRE_R2'],'Exact sole failed QA attempt required')
    return dict(status='EXACT_BANK_V5_QA_SUPERVISOR_AND_CHILD_LAUNCH_BOUND',supervisor_launch=ref,supervisor_identity=dict(pid=launch['pid'],creation_time=launch['creation_time']),capture_launch=child_ref,child_identity=dict(pid=child['pid'],creation_time=child['creation_time']),queue=qref,approval=aref,job_sha256=digest)
...
def inspect_review(path,expected_hash,execute=False):
    ...
    need(a.get('owner_thread_id')==ROOT_THREAD and a.get('run_id')=='20260913T195357Z' and a.get('original_batch')=='bank_v4_P_MAIN6_B3_pre_QA' and a.get('recovery_kind')==KIND,'Wrong recovery owner/batch/kind')
    ...
    supervisor_launch_proof(original,values)
    ...
    need(folder==Path('G:/Just_Peachy_S6D/20260913T195357Z/bank_captures_v1/beam_bank/tagged_C_sentinel/P_INPUT_QA6/QA_P_MAIN6_B3_PRE_R2/telemetry'),'Wrong failed telemetry attempt')
...
def failed_configuration_proof(originals,values):
    """Bind the recorded failed QA_PRE_R2 state to its admitted attempt and original ledger."""
    expected=Path('G:/Just_Peachy_S6D/20260913T195357Z/bank_captures_v1/beam_bank/tagged_C_sentinel/P_INPUT_QA6/QA_P_MAIN6_B3_PRE_R2/configuration.json')
    ref=originals['failed_configuration'];need(Path(ref['path']).resolve()==expected,'Exact failed QA_PRE_R2 configuration path required')
    config=verify(ref);admission=values['admission'];plan=verify(admission['plan']);exact=policy_module().exact
    attempt_id='QA_P_MAIN6_B3_PRE_R2';rows=[x for x in plan['attempts'] if x.get('attempt_id')==attempt_id]
    need(len(rows)==1 and attempt_id in admission['attempt_ids'],'Unique original admitted failed attempt required');attempt=rows[0]
    need(attempt.get('case_id')=='tagged_C_sentinel' and attempt.get('profile')==config.get('profile')=='P_INPUT_QA6','Failed configuration case/profile differs')
    ...
    need(len(charged)==1 and all(charged[0].get(k)==v for k,v in dict(batch='bank_v4_P_MAIN6_B3_pre_QA',case_id=attempt['case_id'],profile=attempt['profile'],status='FAIL',input_sha256=source['sha256']).items()),'Original charged failure/source differs')
...
def verify_recovery_record(receipt,*,owner_binding,restoration_binding,initial_binding):
    """File-only V11 admission API. No process census, ports, locks, getters or setters."""
    ...
    supervisor_launch_proof(originals,{k:verify(originals[k]) for k in ('supervisor_launch','bridge_failure','admission')})
    ...
    return dict(status='VERIFIED_CLOSED_RECORDED_QA_TELEMETRY_FRESH_RESTORE',recovery_kind=KIND,original_owner=owner_binding,original_restoration=restoration_binding,recovery_measurement=receipt['recovery_measurement'],historical_python_terminal_receipt_present=True,old_failure_preserved=True)
