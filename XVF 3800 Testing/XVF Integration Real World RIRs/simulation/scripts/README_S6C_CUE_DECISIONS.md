...
`s6c_cue_decisions.py` reads the sealed, real S6B R0 embeddings and completed
...
rejections, existing candidate comparisons, nonzero cue-score contributions,
borderline existing-track selections, lifecycle/evidence/prototype lineage and
display events. It does not count parser support as activation.

Only profiles exactly equal after removing their display profile ID and toggling
`tracker.cues_enabled` are cue-matched. All eligible identical parent IDs are
...
Under `simulation/reports/S6C/20260910T123540Z/cue_decisions_v1`:

- `CUE_DECISION_PLAN.json`: frozen code and input authority.
- `features/<case>/<tap>.json`: all feature support, receipt and qualified-pair
  measurements; `R0_FEATURE_CUE_AUDIT.json` binds all480 outputs and summaries.
...
$jpAudit = Join-Path $jpRoot 'XVF 3800 Testing\XVF Integration Real World RIRs\simulation\scripts\s6c_cue_decisions.py'
& (Join-Path $jpRoot '.edge-speech-env\python.exe') $jpAudit checks
& (Join-Path $jpRoot '.edge-speech-env\python.exe') $jpAudit freeze
& (Join-Path $jpRoot '.edge-speech-env\python.exe') $jpAudit features
...
set "JP_AUDIT=%JP_ROOT%\XVF 3800 Testing\XVF Integration Real World RIRs\simulation\scripts\s6c_cue_decisions.py"
"%JP_ROOT%\.edge-speech-env\python.exe" "%JP_AUDIT%" checks
"%JP_ROOT%\.edge-speech-env\python.exe" "%JP_AUDIT%" freeze
"%JP_ROOT%\.edge-speech-env\python.exe" "%JP_AUDIT%" features
