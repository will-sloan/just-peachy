"""Isolated epoch4 long-native admission; README_S6C_LONG_NATIVE_EPOCH4.md."""
...
def protected_identities(driver):return {name:dict(identity=id(getattr(driver,name)),code_sha256=hashlib.sha256(marshal.dumps(getattr(driver,name).__code__)).hexdigest()) for name in PROTECTED}
def output_roots(namespace):
    if not re.fullmatch(r'epoch4_[A-Za-z0-9_-]{1,64}',namespace):raise ValueError('New epoch4_ namespace required')
...
def worker_command(argv):
    ...
    if names & {'s6c_long_native_epoch4.py','s6c_paced_controls.py','s6c_paced_b36_v1.py'}:return 'run' in words
...
def prepare(args):
    ...
    deps.extend([bind(__file__),bind(Path(__file__).with_name('README_S6C_LONG_NATIVE_EPOCH4.md'))])
    plan=dict(schema='s6c-epoch4-long-native-admission.v1',status='PREPARED_NO_MODELS_STARTED',created_utc=utc(),namespace=args.namespace,
...
def protected_override(driver,replacements):
    ...
    try:
        ...
        if protected_identities(driver)!=before:raise ValueError('Protected native helper identity changed')
        yield before
        if protected_identities(driver)!=before:raise ValueError('Protected native helper identity changed during execution')
    finally:
        for name,value in old.items():setattr(driver,name,value)
        if protected_identities(driver)!=before:raise ValueError('Protected helper was modified; cannot certify restoration')
...
def run(args):
    ...
    def guard(full=False,reserve=0):
        ...
        if full:
            last_full=now;record=dict(utc=utc(),elapsed_sec=now-started,resources=value);checks.append(record)
...
def checks():
    ...
    r,p=output_roots('epoch4_test');assert r.parent==REPORT/'long_session' and p.parent==PAYLOAD/'long_session';count+=1
...
def source_checks():
    ...
    receipt=dict(schema='s6c-epoch4-long-source-checks.v2',status='PASS_MODEL_FREE_SOURCE_ADMISSION',created_utc=utc(),pure_checks=pure,
        previous_source_checks=prior_binding,preserved_previous_sources=prior_sources,
        correction_scope='Exact-buffer authority reads/pre-native recheck and actual post-rename lease outcome. Original native helper and composition unchanged; previous source admission preserved.',
        ...
        protected_functions=protected_identities(driver),dependencies=deps+[bind(__file__),bind(Path(__file__).with_name('README_S6C_LONG_NATIVE_EPOCH4.md'))],
    ...
    return common.save(REPORT/'long_native_epoch4/SOURCE_CHECKS_V2.json',receipt,immutable=True)
