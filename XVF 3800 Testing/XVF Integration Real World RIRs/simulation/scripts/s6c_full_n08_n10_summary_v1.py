"""Exact full N08/N10 table summary; see README_S6C_FULL_N08_N10_SUMMARY_V1.md."""
...
SOURCE_ROWS=(
    ...
    ('C072','full_n08_n10_native_core_v3','bd19ed5f87b07b82b5c7fb9f095437603b20261f1d8944bb9767e8b7d99fbdae',960),
    ('C074','full_n08_n10_native_core_v3','bd19ed5f87b07b82b5c7fb9f095437603b20261f1d8944bb9767e8b7d99fbdae',960))
...
def extreme_rows(left,right,tap,metric):
    ...
    selected=[]
    for label,ordered in (('largest_observed_harm',sorted(rows,key=lambda x:(-x[0],x[1]))[:2]),
                          ('largest_observed_benefit',sorted(rows,key=lambda x:(x[0],x[1]))[:2])):
...
def main(args):
    out=REPORT/'full_n08_n10_results_v1'
    b.require(not out.exists(),'Fresh output namespace')
    scope,sb=b.obj(REPORT/'design/FULL_N08_N10_COMPARISON_SCOPE_V1.json')
    b.require(sb['sha256']=='3e101c1329446c358d1773f33bb1524d6f620d3e2e934a267f2c11ea23715b53','Held 18-pair/extreme scope')
    ...
    comparison,cb=b.obj(REPORT/'full_n08_n10_comparisons_v1/COMPARISON_RECEIPT.json')
    ...
    b.require({r['comparison_id'] for r in pairs}=={r['comparison_id'] for r in scope['comparisons']},'Exactly declared 18 comparisons')
    ...
    b.require(len(uncertainty['comparisons'])==18,'18 uncertainty records')
    ...
    for path in (Path(__file__),Path(__file__).with_name('README_S6C_FULL_N08_N10_SUMMARY_V1.md'),BASE):
        b.read(path)
    out.mkdir()
    result=dict(status='COMPLETE_EXACT_TABLE_SUMMARY',scope=sb,source_receipts=receipts,comparison_receipt=cb,
        ...
        limits=['Exact completed table aggregation only; native receipts/observations/words remain transitively bound.',
                ...
                'cp combines word and anonymous attribution errors; changed decoder words prevent isolated attribution conclusions.',
                ...
                'Two largest signed harms/benefits per metric/tap are descriptive extremes, not typical or selection criteria.',
