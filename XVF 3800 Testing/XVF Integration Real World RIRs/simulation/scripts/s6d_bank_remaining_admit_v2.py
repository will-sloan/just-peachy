...
def execution_sources_from_freeze(review,freeze_binding,expected_sha256,required_members):
    A.require(len(expected_sha256)==64 and freeze_binding['sha256']==expected_sha256 and review.get('owner_execution_source_freeze')==freeze_binding,'Root must bind the exact supplied V7 execution source freeze/hash')
...
def main():
    ...
    A.require(review['status']=='ROOT_ACCEPTED_TELEMETRY_RECOVERY_V2_SOURCES_V2','Root V7 source review required')
    ...
    A.require(Path(owner['path']).name=='s6d_capture_owner_v7.py' and Path(bridge['path']).name=='s6d_capture_supervisor_bridge_v2.py','Exact V7/bridgeV2 epoch')
    ...
    helper_binding=A.bind(__file__);readme_binding=A.bind(Path(__file__).with_name('README_S6D_BANK_REMAINING_ADMIT_V2.md'))
    ...
    spec=importlib.util.spec_from_file_location('reviewed_v7_closure',owner['path'])
