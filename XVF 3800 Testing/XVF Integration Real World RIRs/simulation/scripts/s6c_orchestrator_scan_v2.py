"""Coordinator-only exact scan overlay. See README_S6C_ORCHESTRATOR_SCAN_V2.md."""
...
SCHEMA='jp_s6c_coordinator_scan_overlay.v2'
...
def bind_environment():
    ...
    if existing is not None and Path(existing).resolve()!=SIM:
        raise ValueError('Conflicting JP_S6C_SIM; refusing a redirected workspace')
...
def load_native():
    ...
    if common.SIM.resolve()!=SIM or common.REPORT.resolve()!=REPORT or common.PAYLOAD.resolve()!=Path('G:/Just_Peachy_S6C/20260910T123540Z').resolve():
        raise ValueError('Frozen native common paths do not match the explicitly admitted workspace')
    if execution.SIM!=common.SIM or execution.REPORT!=common.REPORT or execution.PAYLOAD!=common.PAYLOAD:
        raise ValueError('Frozen execution/common workspace paths differ')
...
def source_rows():
    return [binding(Path(__file__).with_name(name)) for name in ('s6c_orchestrator_scan_v2.py','README_S6C_ORCHESTRATOR_SCAN_V2.md','s6c_admission_fast_v1.py','README_S6C_ADMISSION_FAST_V1.md')]
...
def fixtures(args):
    bind_environment();original_environment=os.environ['JP_S6C_SIM']
    os.environ['JP_S6C_SIM']=str(SIM/'incorrect_fixture_root')
    try:bind_environment()
    except ValueError:pass
    else:raise AssertionError('Conflicting workspace override accepted')
    finally:os.environ['JP_S6C_SIM']=original_environment
    spec_binding,spec,common,execution=load_native();fast=load_fast();checks=['conflicting workspace override rejected before frozen import','frozen SIM REPORT PAYLOAD match explicitly admitted roots']
    ...
    code="import sys,json;sys.path.insert(0,sys.argv[1]);import s6c_common,s6c_execution;print(json.dumps({'common':s6c_common.__file__,'scanner_module':s6c_common.tree_bytes.__module__,'worker':s6c_execution.worker_job.__code__.co_filename,'SIM':str(s6c_common.SIM),'REPORT':str(s6c_common.REPORT),'PAYLOAD':str(s6c_common.PAYLOAD)}))"
    ...
    assert Path(loaded['SIM']).resolve()==SIM and Path(loaded['REPORT']).resolve()==REPORT and Path(loaded['PAYLOAD']).resolve()==common.PAYLOAD.resolve()
