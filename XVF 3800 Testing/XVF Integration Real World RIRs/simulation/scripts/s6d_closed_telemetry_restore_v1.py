"""Root-admitted fresh restore after closed native telemetry; see README_S6D_CLOSED_TELEMETRY_RESTORE_V1.md."""
...
REVIEW_STATUS='ROOT_ACCEPTED_TELEMETRY_RECOVERY_V2_SOURCES_V1'
...
def inspect_review(path,expected_hash,execute=False):
    ...
    return dict(review=a,review_ref=ref,original=original,values=values,known=known,native_proof=proof,output=out)
...
class WindowsServices:
    ...
    def ports_idle(self):
        for port in (8765,8766,8767):
            with socket.socket() as s:s.settimeout(.2);need(s.connect_ex(('127.0.0.1',port)) in (111,10061),'Recorder port busy or idle state unknown: '+str(port))
        return True
    def acquire(self):
        import msvcrt
    ...
    def identify(self):return self.control.identify()
...
def recover_prevalidated(context,services):
    ...
    def checkpoint(label):
        snapshot=services.census();decision=scan_decision(snapshot,context['known'],services.identity);services.ports_idle();path=out/(label+'_PROCESS_SNAPSHOT.json');save(path,snapshot);proof=dict(stage=label,snapshot=bind(path),decision=decision,ports_idle=True,hardware_lock_held=owned);result['process_scans'].append(proof);return proof
    try:
        ...
        keys=set(initial['identity'])-{'USB_BIT_DEPTH','I2S_INPUT_PACKED'};need(set(before)==set(initial['identity']) and all(policy.exact(before[k],initial['identity'][k]) for k in keys),'Device identity differs before setters')
