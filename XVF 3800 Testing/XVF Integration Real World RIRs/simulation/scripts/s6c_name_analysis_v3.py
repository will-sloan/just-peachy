"""Scorer-only S6C identity and retained-name exposure. README_S6C_NAME_ANALYSIS_V3.md."""
...
RATE=16000
POLICY=dict(schema='s6c_name_metrics.v3',source_evidence_expiry_sec=.75,
...
def run(args):
 ...
 scoremap,map_extensions=extensions.gallery_map(mb,scoremap,args.gallery_map_extension,set(scenes))
 registry,registry_bindings=core.registered_candidates(args.registry_extension,args.expected_candidates)
 ...
 inputs=core.verified(core.bind(core.S6B/'INPUT_INDEX.json',core.INPUT_SHA))['rows'];inputrows={(r['case_id'],r['stream']):r for r in inputs}
 ...
 code=[core.bind(Path(__file__)),core.bind(Path(__file__).with_name('README_S6C_NAME_ANALYSIS_V3.md'))]+[core.bind(core.SIM/'scripts'/n) for n in core.CODES]
 ...
 for item in index['rows']:
  ...
  try:
   ...
   result['source_bindings']=dict(prediction=item['result'],support=sb,scorer_gallery_map=mb,scorer_map_extensions=map_extensions,registry_sources=registry_bindings,Q=qb,bank=bankb,codes=code)
 ...
 receipt=dict(schema=POLICY['schema'],status='COMPLETE_REQUESTED_NAME_INDEX' if success else 'PARTIAL_WITH_FAILURES',index=ib,scorer_map=mb,scorer_map_extensions=map_extensions,registry_sources=registry_bindings,enrollment_completion=core.bind(report/'enrollment/ENROLLMENT_COMPLETION.json'),Q=qb,bank=bankb,codes=code,policy=POLICY,
...
if __name__=='__main__':
 import json
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--index',type=Path);p.add_argument('--output-subdir',default='name_analysis_v3');p.add_argument('--require-complete',action='store_true');p.add_argument('--test',action='store_true')
 p.add_argument('--registry-extension',nargs=2,action='append',default=[],metavar=('PATH','SHA256'));p.add_argument('--expected-candidates',type=int,default=184)
 p.add_argument('--gallery-map-extension',nargs=4,action='append',default=[],metavar=('MAP','MAP_SHA','COMPLETION','COMPLETION_SHA'));a=p.parse_args()
