...
from .research_s7_presentation import segment_application_freshness
...
class EdgeSpeechWindow:
    def __init__(self, root: tk.Tk, config: PipelineConfig, *, engine=None) -> None:
        ...
        self._s7_ack_sink = None
        self._s7_pending_acks = 0
        self._s7_columns = None
        self._s7_diagnostic_pending = deque(maxlen=64)
        self._s7_diagnostic_seen = 0
        self._s7_diagnostic_coalesced = 0
        self._s7_diagnostic_last_flush = 0.
    ...
    def _build(self) -> None:
        ...
        if self.engine._s6d is not None:
            ...
            ttk.Checkbutton(transcript_frame, text="Show complete unfiltered transcript", variable=self.s6d_full_view,
                command=self._s7_full_view_action if self._s7_initial_gui_options() is not None else self._render_transcript).pack(anchor="w")
            if self._s7_initial_gui_options() is not None:
                self._build_s7_controls(transcript_frame)
        ...
        if self._s7_initial_gui_options() is not None:
            self._build_s7_columns(transcript_frame)
    ...
    def _poll(self) -> None:
        ...
        s7_budget = 128 if getattr(self.engine, '_s7_presentation_options', None) is not None else None
        s7_consumed = 0
        while not self.engine.events.empty() and (s7_budget is None or s7_consumed < s7_budget):
            ...
            if getattr(self.engine, '_s7_presentation_options', None) is not None:
                self._queue_s7_diagnostic(data)
            else:
                self._append_research(json.dumps(data, ensure_ascii=False))
        if getattr(self.engine, '_s7_presentation_options', None) is not None:
            self._flush_s7_diagnostics(force=self.engine.state in {'COMPLETED', 'FAILED'} and self.engine.events.empty())
        ...
        if self.engine._s6d is not None and self.engine.state in {"COMPLETED", "FAILED"} and self.engine._finalization_thread is not None and not self.engine._finalization_thread.is_alive() and self.engine.events.empty() and not getattr(self, '_s7_pending_acks', 0):
    ...
    def _finish_s6d_close(self):
        if getattr(self, '_s7_pending_acks', 0):return False
    ...
    def _consume_s6d_display(self, payload):
        if payload.get('s7_enabled') is True:
            return self._consume_s7_display(payload)
    ...
    def _render_transcript(self) -> None:
        if getattr(self.engine, '_s7_presentation_options', None) is not None or any(row.get('s7_enabled') for row in self.s6d_rows.values()):
            return self._render_s7_transcript()
        if self.engine._s6d is not None:
    ...
    def set_s7_ack_sink(self, sink):
        """Optional native wrapper collector; called on Tk thread, never through engine._emit."""
        if sink is not None and not callable(sink):
            raise TypeError('S7 acknowledgment sink must be callable')
        self._s7_ack_sink = sink

    def _s7_ack(self, record):
        record = {**record, 'clock_scope': 'Tk receipt/application/idle callback; not physical scanout'}
        sink = getattr(self, '_s7_ack_sink', None)
        if sink is not None:
            sink(record)
        else:
            self._record_s6d_gui(record)

    def _s7_initial_gui_options(self):
        # Widgets are built before _begin_session creates the runtime controller.
        # Configured options here are UI-only; they create no source/evidence epoch.
        options = getattr(self.engine, '_s7_presentation_options', None)
        if options is not None:
            return options
        settings = getattr(self.engine, '_s7', None)
        if settings is not None and settings.presentation_enabled:
            return {'mode': settings.mode, 'ownership_mode': settings.ownership_mode}
        return None

    def _build_s7_controls(self, parent):
        bar = ttk.Frame(parent); bar.pack(fill='x')
        self._s7_mode_var = tk.StringVar(value=(self._s7_initial_gui_options() or {}).get('mode', 'M1'))
        mode = ttk.Combobox(bar, textvariable=self._s7_mode_var, values=[f'M{i}' for i in range(1, 8)], width=5, state='readonly')
        mode.pack(side='left'); mode.bind('<<ComboboxSelected>>', lambda event: self.s7_user_action('mode', self._s7_mode_var.get()))
        self._s7_selection_var = tk.StringVar(value=','.join(self.engine._s6d.selected_profile_ids))
        ttk.Entry(bar, textvariable=self._s7_selection_var, width=22).pack(side='left', padx=4)
        ttk.Button(bar, text='Emphasize IDs', command=lambda: self.s7_user_action('selection',
            [item.strip() for item in self._s7_selection_var.get().split(',') if item.strip()])).pack(side='left')
        self._s7_display_var = tk.StringVar(value='active')
        display = ttk.Combobox(bar, textvariable=self._s7_display_var, values=['active', 'listening', 'dim'], width=9, state='readonly')
        display.pack(side='left', padx=4)
        display.bind('<<ComboboxSelected>>', lambda event: self.s7_user_action('display_state', self._s7_display_var.get()))
        ttk.Label(parent, text='Presentation controls; changing views does not change model workload.').pack(anchor='w')

    def _build_s7_columns(self, parent):
        self._s7_columns_frame = ttk.Frame(parent)
        self._s7_columns = {}
        for name, title, col in [('left', 'Person column 1', 0), ('right', 'Person column 2', 1)]:
            frame = ttk.LabelFrame(self._s7_columns_frame, text=title)
            frame.grid(row=0, column=col, sticky='nsew')
            widget = tk.Text(frame, wrap='word', font=('Segoe UI', 11), state='disabled', width=25, height=9)
            widget.pack(fill='both', expand=True); self._s7_columns[name] = widget
        other = ttk.LabelFrame(self._s7_columns_frame, text='Unresolved / other speakers — chronological')
        other.grid(row=1, column=0, columnspan=2, sticky='nsew')
        widget = tk.Text(other, wrap='word', font=('Segoe UI', 11), state='disabled', height=5)
        widget.pack(fill='both', expand=True); self._s7_columns['other'] = widget
        for col in (0, 1):self._s7_columns_frame.columnconfigure(col, weight=1)
        self._s7_columns_frame.rowconfigure(0, weight=2); self._s7_columns_frame.rowconfigure(1, weight=1)

    def _s7_full_view_action(self):
        self.s7_user_action('full_view', bool(self.s6d_full_view.get()))

    def s7_user_action(self, action, value, *, now=None):
        """Scriptable GUI action; view history never rewrites the internal transcript."""
        state = getattr(self.engine, '_s6d_presentation', None)
        if state is None or not getattr(state, 's7_enabled', False):
            self.s6d_status.set('S7 view action unavailable until an S7 session has started')
            return None
        received = time.perf_counter() if now is None else now
        try:
            record = state.user_action(action, value, now=received)
        except ValueError as exc:
            self._s7_ack(dict(kind='s7_user_action_rejected', session_id=state.session_id,
                action=action, value=value, receipt_monotonic_sec=received, reason=str(exc)))
            self.s6d_status.set(str(exc)); return None
        self._s7_ack(record)
        if action in {'selection', 'mode'} and getattr(self, '_s6d_direction_rows', []):
            self._s6d_direction_rows = []
            self._render_s6d_directions()
            self._s7_ack(dict(kind='s7_direction_invalidated_by_view_action', session_id=state.session_id,
                             action_id=record['action_id'], applied_monotonic_sec=time.perf_counter()))
        self.s6d_full_view.set(state.full_view)
        for row in state.snapshot_rows():
            self._consume_s7_display(row, action_id=record['action_id'])
        return record

    @staticmethod
    def _s7_visible_state(payload):
        return {key: deepcopy(payload.get(key)) for key in ('session_id', 'caption_key', 'utterance_id', 'segment_id',
            'text_revision_id', 'text_version', 'identity_version', 'evidence_ids', 'token_ids', 'token_range',
            'display_version', 'view_revision', 'mode', 'column', 'label', 'display_text', 'known_profile_id',
            'track_id', 'visible', 'visibility_state', 'emphasized', 'display_state', 'full_view',
            'ownership_mode', 'segments', 'accepted_identity_snapshot', 'rendered_token_ids')}

    def _consume_s7_display(self, payload, action_id=None):
        received = time.perf_counter()
        state = getattr(self.engine, '_s6d_presentation', None)
        session = getattr(self, '_s6d_gui_session_id', None)
        if session is None or payload.get('session_id') != session:
            self._s7_ack(dict(kind='s7_gui_rejected', session_id=payload.get('session_id'),
                caption_key=payload.get('caption_key'), receipt_monotonic_sec=received, reason='foreign_or_unstarted_session'))
            return False
        key = payload.get('utterance_id')
        version = payload.get('display_version')
        if not isinstance(key, str) or payload.get('caption_key') != session + '/' + key or type(version) is not int:
            raise ValueError('S7 GUI requires exact caption key and integer display version')
        payload = state.project(payload) if getattr(state, 's7_enabled', False) else dict(payload)
        previous = self.s6d_rows.get(key)
        token = (version, payload.get('view_revision', 0))
        if previous is not None and token <= (previous.get('display_version', -1), previous.get('view_revision', -1)):
            self._s7_ack(dict(kind='s7_gui_rejected', session_id=session, caption_key=payload['caption_key'],
                receipt_monotonic_sec=received, reason='duplicate_or_older_display', version=list(token)))
            return False
        self.s6d_rows[key] = dict(payload)
        while len(self.s6d_rows) > self.engine._s6d.max_display_rows:
            old = next((k for k, row in self.s6d_rows.items() if row.get('final')), None)
            if old is None:raise RuntimeError('S7 GUI active caption capacity exhausted')
            del self.s6d_rows[old]
        self._s7_ack(dict(kind='s7_gui_received', **self._s7_visible_state(payload),
            action_id=action_id, receipt_monotonic_sec=received,
            event_publication_monotonic_sec=payload.get('publication_monotonic_sec')))
        started = time.perf_counter(); self._render_s7_transcript(); applied = time.perf_counter()
        rendered = getattr(self, '_s7_last_rendered_rows', {}).get(key)
        if rendered is not None:
            for segment in rendered.get('segments', []):
                if segment.get('rendered'):
                    segment.update(segment_application_freshness(segment, applied))
        snapshot = self._s7_visible_state(getattr(self, '_s7_last_rendered_rows', {}).get(key, payload))
        self._s7_ack(dict(kind='s7_gui_applied', **snapshot, action_id=action_id,
            receipt_monotonic_sec=received, widget_apply_started_monotonic_sec=started,
            widget_apply_finished_monotonic_sec=applied))
        self._s7_pending_acks = getattr(self, '_s7_pending_acks', 0) + 1
        def flushed():
            try:
                current = self.s6d_rows.get(key, {})
                same = current.get('session_id') == session and (current.get('display_version'), current.get('view_revision')) == token
                self._s7_ack(dict(kind='s7_gui_idle_flush', **snapshot, action_id=action_id,
                    idle_callback_monotonic_sec=time.perf_counter(),
                    flush_status='APPLIED_AT_IDLE' if same else 'SUPERSEDED_BEFORE_IDLE',
                    actual_state=self._s7_visible_state(getattr(self, '_s7_last_rendered_rows', {}).get(key, current))))
            finally:
                self._s7_pending_acks -= 1
        self.root.after_idle(flushed)
        return True

    def _render_s7_transcript(self):
        state = getattr(self.engine, '_s6d_presentation', None)
        rows = [state.project(row) if getattr(state, 's7_enabled', False) else row for row in self.s6d_rows.values()]
        mode = state.mode if getattr(state, 's7_enabled', False) else rows[-1].get('mode') if rows else None
        columns = getattr(self, '_s7_columns', None)
        if mode == 'M5' and columns:
            self.transcript.pack_forget(); self._s7_columns_frame.pack(fill='both', expand=True)
            widgets = columns
        else:
            if columns:self._s7_columns_frame.pack_forget()
            self.transcript.pack(fill='both', expand=True)
            widgets = {'all': self.transcript}
        for row in rows:
            row['rendered_token_ids']=[]
            for piece in row.get('segments',[]):piece['rendered']=False
        for name, widget in widgets.items():
            widget.configure(state='normal'); widget.delete('1.0', 'end')
            widget.tag_configure('emphasized', font=('Segoe UI', 12, 'bold'), foreground='#175c26')
            for row in rows:
                pieces = row.get('segments') if row.get('ownership_mode') == 'supported_prefix_v2' else [row]
                for piece in pieces:
                    target = piece.get('column') if piece.get('column') in {'left', 'right'} else 'other'
                    if (name == 'all' or name == target) and (self.s6d_full_view.get() or piece.get('visible')):
                        if row.get('ownership_mode') == 'supported_prefix_v2':
                            piece.update(segment_application_freshness(piece, time.perf_counter()))
                            piece['rendered']=True
                            row['rendered_token_ids'].extend(piece['token_ids'])
                        span = f'[{row.get("source_start_sec", 0):.2f}s; tokens {piece.get("token_range")}] ' if mode == 'M5' else ''
                        text = span + piece['label'] + ': ' + piece['display_text'] + (' …' if not row.get('final') else '') + '\n\n'
                        widget.insert('end', text, ('emphasized',) if piece.get('emphasized') else ())
            dim = bool(rows and rows[-1].get('display_state') == 'dim')
            widget.configure(background='#e8e8e8' if dim else 'white', foreground='#444444' if dim else 'black', state='disabled')
            widget.see('end')
        self._s7_last_rendered_rows={row['utterance_id']:deepcopy(row) for row in rows}
        if rows:
            self.s6d_status.set(f'{mode} • {rows[-1].get("display_state", "active")} • full internal transcript retained')


    def _queue_s7_diagnostic(self, data):
        """Compact optional diagnostic view; full scientific events stay in journals."""
        payload = data.get('payload') or {}
        summary = '{} {} source={} id={}'.format(
            payload.get('publication_sequence', '?'), data.get('event_type', '?'),
            data.get('source_time_sec', '?'),
            payload.get('event_id') or payload.get('utterance_id') or '-')
        if payload.get('reason'):
            summary += ' reason=' + str(payload['reason'])[:160]
        summary = summary[:320]
        if len(self._s7_diagnostic_pending) == self._s7_diagnostic_pending.maxlen:
            self._s7_diagnostic_coalesced += 1
        self._s7_diagnostic_pending.append(summary)
        self._s7_diagnostic_seen += 1

    def _flush_s7_diagnostics(self, *, force=False, now=None):
        now = time.perf_counter() if now is None else now
        if not self._s7_diagnostic_pending or not force and now-self._s7_diagnostic_last_flush < .25:
            return False
        lines = list(self._s7_diagnostic_pending)
        coalesced = self._s7_diagnostic_coalesced
        started = time.perf_counter()
        header = '[Diagnostic summaries: {} shown, {} coalesced; full events in session logs]'.format(len(lines), coalesced)
        self._append_research(header + '\n' + '\n'.join(lines))
        finished = time.perf_counter()
        self._s7_diagnostic_pending.clear()
        self._s7_diagnostic_coalesced = 0
        self._s7_diagnostic_last_flush = now
        self._s7_ack(dict(kind='s7_gui_diagnostic_batch', optional_UI_only=True,
            shown_summaries=len(lines), coalesced_summaries=coalesced,
            total_consumed_events=self._s7_diagnostic_seen,
            widget_apply_started_monotonic_sec=started,
            widget_apply_finished_monotonic_sec=finished,
            scientific_event_drops_by_diagnostic_view=0))
        return True
