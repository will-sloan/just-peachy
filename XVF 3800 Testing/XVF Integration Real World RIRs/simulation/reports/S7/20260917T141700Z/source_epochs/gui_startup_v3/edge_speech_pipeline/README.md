...
Opt-in S6D delivery, selected-person transcript and direction research is documented in [README_RESEARCH_S6D.md](README_RESEARCH_S6D.md), including current PowerShell and Anaconda/CMD commands, inputs, outputs and model-free tests. It requires explicit settings and an exact v3 profile; ordinary launches retain the historical behavior.

Explicit S6 candidate execution is documented in [README_RESEARCH_PROFILES.md](README_RESEARCH_PROFILES.md). Ordinary launches keep historical defaults; research profiles opt in to supported component and cue settings without changing model weights.
