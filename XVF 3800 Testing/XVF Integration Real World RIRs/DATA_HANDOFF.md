# XVF3800 data handoff — complete uncompressed folder

Consolidated 8 September 2026. **Share this entire `XVF Measurement` folder with the desktop.** The original recordings remain in place. Reference documents, excitation signals, software, metadata and guidance are now together. No archive extraction or restore script is needed for this folder.

## Start on the desktop

1. Copy or share the whole folder, including all subfolders. Allow roughly 15 GB of disk space. If using OneDrive, wait until synchronization completes and make the files available locally before analysis.
2. Open this `DATA_HANDOFF.md`, `RECORDINGS_INDEX.csv`, `PROJECT_CONTEXT.md` and `MEASUREMENT_GUI_GUIDE.md`.
3. Recordings are in `XVF_MEASUREMENT_WORK/experiments/`. Uncompressed excitation assets are in `measurement_app/assets/excitation_v2/`. The original workbook, XMOS PDFs, context and supplied ZIP packs are in `reference_documents/`.
4. Analyse the saved WAVs and metadata directly. The GUI source is included, but running acquisition on another PC needs Python, hardware dependencies and a launcher adjusted for that PC. The current launcher contains the laptop's absolute Python path.

Keep the recorded trial folders unchanged. Put new RIRs and analysis outputs in separate folders. No denoising, source correction or final RIR generation was performed during consolidation. Previously deleted recordings were not recovered from the Recycle Bin.

## Dataset inventory

This folder contains **166 named JPXVF measurement/pilot runs across six room/table labels**, plus 13 request-bearing diagnostic takes and additional hardware/recovery evidence. `RECORDINGS_INDEX.csv` indexes 179 request-bearing folders, including incomplete/failed evidence. Room labels are operator entries; they are not a claim of six independent physical buildings or sites.
...
Five Loeb Caf recordings were explicitly annotated **Background noise present**. They are retained unchanged. Annotation files are `validation/LAST_FIVE_NOISE_REVIEW_20260907/noise_annotations.json` and the GUI copy at `measurement_app/static/recording_reviews.json`. Read `NOISE_REVIEW.md` in that validation folder. Annotations bind run IDs, timestamps and original hashes. They apply to those five reviewed recordings only; other takes may also contain noise.
...
The desktop handoff check is saved in `validation/DESKTOP_HANDOFF_20260908/HANDOFF_CHECK.json`. It verifies the added reference documents, excitation assets and required handoff paths. Earlier complete archive verification and original-manifest audit reports are preserved in that same folder; their scope and snapshot counts predate this consolidation. The original per-run `SHA256SUMS.txt` files remain inside the recording folders. Transfer verification establishes byte identity and completeness against the transfer inventory, not scientific correctness. Never edit frozen takes in place; put new RIRs, plots, QC and notes in a new analysis directory outside the archived trial folders.

This folder includes the GUI source and locally bundled audio/USB dependencies, but it is **not a standalone desktop installer**. The supplied launcher points at the laptop's Python 3.12 path. On a different PC, install/configure an appropriate Python environment and update a separate launcher; audio devices and hardware tools also require setup. Existing recordings can be read and analysed without an attached XVF board. Do not use the copied GUI's delete function or start acquisition until the desktop installation and device selections have been checked.
