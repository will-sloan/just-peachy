...
public static class QueuedTelemetry
{
    ...
    private const byte AecResource = 0x21;
    // Derived from supplied aec_cmds.yaml + generator's dedicated AEC offset70.
    ...
    private static readonly byte[] Commands = new byte[] {75, 80};
    private static readonly string[] Names = new string[] {"AEC_AZIMUTH_VALUES", "AEC_SPENERGY_VALUES"};
    ...
    private static bool[] pending = new bool[2];
    ...
    public static string Inspect(string dllDirectory)
    {
        ...
        bool[] verified = new bool[2];
        for (uint i=0; i<number; ++i)
        {
            byte resource, command;
            int type, rw; uint count;
            get_cmd_id_info(out resource, out command, (UIntPtr)i);
            for (int field=0; field<2; ++field) if (resource==AecResource && command==Commands[field])
            {
                get_cmd_val_info(out type, out rw, out count, (UIntPtr)i);
                int expectedType = field==0 ? 5 : 3;
                if (type != expectedType || rw != 0 || count != 4 || verified[field])
        ...
        if (!verified[0] || !verified[1]) throw new InvalidOperationException("Telemetry commands absent from command map");
    ...
    private static int Read(int field, long cycle, string phase, int attempt, out byte[] data, out long end)
    {
        ...
        int transport = control_read_command(AecResource, (byte)(Commands[field] | 0x80), data, (UIntPtr)17);
        end = Stopwatch.GetTimestamp();
        int status = transport==0 ? data[0] : -1;
        transactions.WriteLine(Json.Serialize(Obj("sequence",transactionCount++,"cycle",cycle,"phase",phase,
            "command",Names[field],"resource",AecResource,"wire_command",Commands[field]|0x80,
            "expected_response_bytes",17,"attempt",attempt,"request_start_qpc_ticks",begin,
            "request_end_qpc_ticks",end,"request_start_monotonic_ns",Ns(begin),"request_end_monotonic_ns",Ns(end),
            "transport_return",transport,"device_status",status,"raw_response_base64",Convert.ToBase64String(data),
            "payload_valid",transport==0 && status==0)));
    ...
    {
        var values = new object[4]; var finite = new bool[4];
        for(int i=0;i<4;++i)
        {
        ...
        var row=Obj("sequence",sampleCount++,"cycle",cycle,"phase",phase,"command",Names[field],
            ...
            "values",values,"finite",finite,"raw_response_base64",Convert.ToBase64String(data),
            "units",field==0 ? "radians" : "vendor_speech_energy_units","same_device_frame_proven",false);
    ...
    private static void Pair(long cycle, string phase, bool sequential)
    {
        ...
        {
            for(int field=0;field<2;++field) if(!complete[field])
            {
                if(attempts[field]>=1000 || (Stopwatch.GetTimestamp()-pairStart)/(double)Stopwatch.Frequency>2)
                    throw new TimeoutException("Bounded telemetry retry limit reached");
    ...
    public static int Run(string dllDirectory, string outputDirectory, double seconds, bool sequential, string stopFile)
    {
        ...
        transactions=Writer(Path.Combine(outputDirectory,"transactions.jsonl"));
        samples=Writer(Path.Combine(outputDirectory,"samples.jsonl"));
        transactionCount=sampleCount=retries=cycleCount=0;
        firstTimes=new long[2];lastTimes=new long[2];fieldCounts=new long[2];pending=new bool[2];
        intervals=new List<double>[] {new List<double>(),new List<double>()};
        ...
        bool passed=error==null && cleanup==0 && !pending[0] && !pending[1] && cycleCount>0 && fieldCounts[0]==cycleCount && fieldCounts[1]==cycleCount;
        var result=Obj("status",passed?"PASS":"FAIL","mode",sequential?"sequential_official_transport":"queued_distinct_reads",
            ...
            "measurement_elapsed_s",start==0?0:(end-start)/(double)Stopwatch.Frequency,"requested_seconds",seconds,
