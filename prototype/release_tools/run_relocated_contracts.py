...
selected=['test_people.py','test_lifecycle.py','test_file_tap.py','test_enrollment_cleanup.py',
          'test_live_stop_ownership.py','test_live_pipeline_bridge.py']
rows=[]
...
summary=dict(status='PASS' if result.wasSuccessful() else 'FAIL',tests_run=result.testsRun,
    ...
    adaptation='Only test source-root expressions rebound to the immutable installed release; original assertions preserved.')
