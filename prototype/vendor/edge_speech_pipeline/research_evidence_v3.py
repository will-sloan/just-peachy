            ...
            if audio.size:
                ...
                while pending.size >= step:
                    ...
                    if since_seg >= seg_step:
                        ...
                        views = engine._s7_model_call("segmentation", end, models.segment,
                            np.pad(rolling, (160000-len(rolling), 0)), include_posteriors=True)
                    ...
                    for diagnostic in candidates:
                        ...
                        if not diagnostic["admitted"]:
                            continue
                        started = time.perf_counter()
                        vector = engine._s7_model_call("embedding", end, models.embed, rolling[-diagnostic["samples"]:])
