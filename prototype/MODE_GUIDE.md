...
| ID from SELECTED names - Closed group (Always assign) ◇ | Valid fresh voice chooses its best selected UUID; rejected open-set names show **· assumed** | Outsiders may be misnamed. One selected person is a user-assumed label. Missing/stale/overlapping voice never forces a name. |
...
enrollment/reference. Speech without eligible clean, fresh voice/association
keeps its words pending/unavailable; silence is never named. Overlap annotations
are not word-by-word source certainty. See `docs/MODE_SEMANTICS.md` for exact gates.
