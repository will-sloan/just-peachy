...
class LiveSpatialProvider:
    def __init__(self, tap, tracker_config, *, enabled=False, display=False, clock=time.perf_counter):
        self.tap, self.config = tap, tracker_config
        self.enabled, self.display, self.clock = bool(enabled), bool(display), clock
        self.origin = None; self.live = None; self._lock = threading.RLock()
    ...
    def evidence_for_window(self, source_start_sec, source_end_sec, available_at_sec):
        with self._lock:
            ...
            valid = bool(connected and row and row['mapped_end'] >= source_start_sec
    ...
    def observe_decision(self, payload):
        ...
        if self.enabled and (cue.get('qualified_bearing_deg') is None or cue.get('reliability', 0) <= 0):
            return
        identity = decision.get('identity') or {}
        profile = identity.get('known_profile_id') if identity.get('naming_state') == 'confirmed' else None
        name_stamp = now if identity.get('query_executed') else identity.get('name_evidence_available_at_sec')
        if not isinstance(name_stamp, (int, float)) or not 0 <= now-name_stamp <= 2.:
    ...
    def snapshot(self):
        ...
        return dict(state=raw.get('state', 'OFF'), message=message,
