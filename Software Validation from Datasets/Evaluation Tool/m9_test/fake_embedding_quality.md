...
- `python scripts/embed_speaker_folder.py m9_test/audio --backend fake --output m9_test/fake_embeddings.jsonl --report m9_test/fake_embedding_quality.md --dimension 8 --min-duration-sec 0.1 --labels-csv m9_test/labels.csv --run-id m9_fake_local`

## Validation Checks

- Dimension checks: successful dimensions=[8]
- Normalization checks: min_norm=1.0000, max_norm=1.0000
- Serialization checks: wrote 3 row(s) to m9_test/fake_embeddings.jsonl
- Runtime RTF: mean=0.0000, max=0.0000
- Memory: max_cpu_memory_mb=326.95
