[CmdletBinding()]
param(
    [ValidateSet('Audit','Prepare','Validate','Plan','Smoke','RunDevelopment','Status','Stop','Analyze','Freeze','Collect','ValidateFrozen')]
...
$Monitor = Join-Path $ToolRoot 'scripts\monitor_hybrid_speaker_attribution_product_v2.ps1'
...
if ($Action -eq 'RunDevelopment' -and $Background) {
    ...
    $arguments = @(
        ...
        '-ParallelBackends', [string]$ParallelBackends,
...
if ($Action -eq 'RunDevelopment' -and $OpenMonitor) {
...
$command = switch ($Action) {
    'RunDevelopment' { 'full' }
    'ValidateFrozen' { 'validate-frozen' }
    default { $Action.ToLowerInvariant() }
}
$arguments = @($Launcher, 'hybrid-product-v2', $command)
if ($Action -in @('Smoke','RunDevelopment')) {
    $arguments += @('--parallel-backends', [string]$ParallelBackends)
}
if ($Action -in @('RunDevelopment','Analyze')) {
...
try {
    & $Python @arguments
    if ($LASTEXITCODE -ne 0) { throw "Hybrid Product V2 action failed with exit code $LASTEXITCODE" }
}
finally {
    Pop-Location
}
